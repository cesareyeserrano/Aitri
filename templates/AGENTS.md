...
- **`verify-run` says the runner "did not finish" / was killed?** That is a runner **kill** (it hit the time limit, or emitted more output than the capture buffer), NOT a failing suite — Aitri does **not** write `04_TEST_RESULTS.json` and does **not** flip `verifyPassed`, so nothing regressed. If your suite legitimately runs long (large integration/e2e/.NET suites), raise the ceiling: set `test_runner_timeout_ms` (milliseconds, default `900000` = 15 min) in `04_BUILD_REPORT.json`. If it was killed for output volume, reduce runner verbosity. A genuine test failure is a clean non-zero exit and is still recorded normally.
