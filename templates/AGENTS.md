...
For tests that genuinely cannot be automated (manual QA, external systems): `aitri tc mark-manual <TC-ID>` sets the TC's `automation` field to `manual` so the e2e coverage gate accepts it without an automated runner. When **every** Phase-3 TC is manual, the project has no automated runner by design — `complete build` then accepts a `04_BUILD_REPORT.json` with no `test_runner`/`test_files`, and you verify each TC with `aitri tc verify <TC-ID> --result pass|fail --notes "..."` (a human at a terminal can instead run bare `aitri tc verify` for a guided checklist that walks each pending manual TC). If an **automated** TC ran but Aitri still could not parse its result, record it against the evidence file instead of re-typing it manual: `aitri tc verify <TC-ID> --result pass|fail --notes "..." --evidence <path-to-result-file>`.
