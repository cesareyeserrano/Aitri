...
- **`approve` is a human-review checkpoint, not a rubber stamp.** Every `aitri approve <phase>` prints the artifact summary and the phase's Human Review checklist. Two modes, set by the project:
  - **Default (`humanApprovalGate` not set):** you MAY run `approve` in agent mode, but ONLY after presenting the artifact summary + checklist to the user and getting their explicit confirmation. Never chain `approve 1 → 2 → 3 → 4 → 5` autonomously — relaying the checkpoint to the human is the point.
