...
- **Phase 4 is plan-first, never build-everything-then-reveal.** On a fresh build the briefing's Plan-First Build Protocol applies: write `BUILD_PLAN.md` (FR clusters + order + status — a working file, not a pipeline artifact), present the plan to the user BEFORE any code, implement cluster by cluster (skeleton → persistence → hardening within each), and checkpoint at each cluster boundary — pause and show progress when a human is present; record-and-continue when autonomous. **On re-entering Phase 4 in a later session, read `BUILD_PLAN.md` first** — it carries the execution state the briefing does not. A mid-build correction that changes the SPEC routes through the pipeline (never silently absorb it), and after ANY upstream re-open you re-run `aitri run-phase 4` for a fresh briefing — never resume from a stale briefing + old plan. Debug re-entry (failing tests) skips the protocol: minimal fixes, no re-planning.
