...
- A TC is credited ONLY from the `test_runner` command's output, the auto-detected e2e run, an explicit `--results` JUnit-XML/TRX file, or recorded `tc verify` evidence. A test that runs only inside a `quality_gates` command (e.g. a separate component-test runner declared as a gate) is judged by exit code — its TC ids are NEVER parsed, and those TCs will report as skipped and block verify-complete. Plan every TC so its test executes under the single `test_runner` command (or the e2e runner / a results file). One TC id per test title — a title naming two TCs credits only the first
- **A test NEVER re-runs the suite it lives in.** Do not author a TC whose test shells out to the project's own `test_runner` (or its build) over a scope that includes the running suite — "the suite still passes", "the neighbouring tests are still green", "coverage did not drop" are NOT test cases. They are already proven by those tests running in the same pass, and by the runner's own exit code, which `verify-run` reads. What the nested run adds is only cost and risk: it multiplies memory by the worker pool (a pool per level), and the moment its scope includes the file it is written in, the suite launches itself, again, without end — a real project lost its machine to exactly that, from a single test. If a "nothing else broke" commitment needs a mechanical owner, it is the runner's exit code or a `quality_gates` entry, never a TC. *Legitimate and unaffected:* spawning a different program to assert its behavior — a gate script, a linter, a CLI under test, a single scoped probe file that must fail — which is a subprocess of another tool, not the suite re-entering itself
