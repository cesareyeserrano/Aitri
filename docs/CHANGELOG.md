...
- **AUDIT-REF-0706 — `audit requirements` → `approve 1` no longer self-blocks.** The auditor briefing itself taught citing the seed by filename (`auditCoverage.md` report format), and the approve-1 absorption preflight then blocked on that mention in `AUDIT_REPORT.md` — the recommended flow produced the blocker on both Hub features this week. `AUDIT_REPORT.md` joins the classifier's skip-list (a point-in-time advisory record: the seed DID exist when the audit ran; nothing consumes the report at runtime), and the template now says "the seed brief", never the filename. The gate is intact for every artifact where a seed ref actually breaks post-absorption.
- **VERIFY-TC-VISIBILITY-0706 — TC-credit channels stated up front, and multi-id titles warn.** (a) Phase 3/4 briefings now state plainly that a TC is credited ONLY from the `test_runner` output, the auto-detected e2e run, `--results` XML, or `tc verify` evidence — a `quality_gates` command is judged by exit code and never parsed for TC ids (a Hub build learned this only when verify-complete blocked). (b) `verify-run` warns on stderr when one runner-output line carries several planned TC ids whose extras were credited nowhere else in the run — a single line credits only its first id (deliberate: one assertion set must not bless N TCs, e.g. a negative TC credited by a happy-path run; the silence was the defect, not the drop). New `extractAllTCIds` (tc-id.js) + `findMultiTCIdLines` (verify-parsers.js). Honest scope: stdout/e2e channels; a multi-id XML testName still drops silently (briefings state the rule).
- **E2E-PIPELINE-0706 — `validate --json` `allValid` honors the absorbed brief.** The predicate required literal `exists`, and IDEA.md is archived at approve 1 by design — so `allValid` was a permanent false-negative for every completed project (a shipped consumer misread it). Now `(exists || absorbed) && approved && !drift`; `exists` stays literal; `deployable` remains the deploy verdict.
- **E2E-PIPELINE-0706 — NFR category vocabulary gains `Observability` and `CI/CD`.** The Phase-1 briefing mandated covering both operational categories while its own enum line forbade naming them — steering agents to `Regression`, the one value with hard-MUST gate side-effects. The enum now names them, with an explicit "do NOT file these under Regression" reservation and guidance to write CI/CD acceptance criteria as structural assertions on the workflow file. `isMustRequirement` unchanged (new values carry no MUST side-effect — pinned by test); the legacy-upgrade category vocabulary follows suit.

Also in this release window (previous commit, no bump): the smoke E2E layer now exercises the working-tree binary instead of the globally installed one; CI triggers on every branch and gains a lib/bin coverage floor (90% lines / 80% branches) — the same coverage gate Aitri offers consumers, dogfooded; `test`/`test:all` switched to glob discovery after the hand-maintained list drifted (TEST-HARDENING-0707).

`AUDIT-REF-0706` / `VERIFY-TC-VISIBILITY-0706` / `E2E-PIPELINE-0706` · 15 new tests · contract docs: VALIDATE_JSON.md + ARTIFACTS.md updated, integrations CHANGELOG entry `— additive`.
...
- **New contract document `docs/integrations/VALIDATE_JSON.md`** — `aitri validate --json` has promised its shape to consumers since the early Hub era ("legacy schema preserved for Hub and external consumers") but the contract had no home; the shape was only recoverable from scattered integrations-CHANGELOG entries. Documented in full (verified against `emitJson`): per-artifact entries incl. the additive `absorbed`/`verifyPassed`/`resultsBinding` fields, `allValid` vs `deployable` semantics, `deployFiles`, `setupCommands`, and the `--ci` exit-code gate. Wired into `test/release-sync.test.js` (five headers now enforced) and the README surface/contract tables, alongside a new surface row for `aitri export traceability`.
- **Code fix: the `artifact_missing` next-action's `severity` is `"critical"`, not `"blocker"`** — rc.149 shipped a value outside the documented `info|warn|critical` enum; a consumer switching on the contract would silently drop the most urgent action class. A test now pins every emitted `nextActions[].severity` to the enum.
- **Doc-drift corrections (docs only, code unchanged; full detail in `docs/integrations/CHANGELOG.md` rc.157):** SCHEMA.md's artifact map used a nonexistent `"4r"` key for the review phase (real key: `"review"` — the doc's own `hasDrift` sample missed review-phase drift); `driftPhases` element type (numbers on disk for core phases); `verifyPassed`/`verifySummary` reset/clear sites completed (required quality gate, reconcile-pending, cascade — incl. the six-field cascade wipe); event payloads and `lastSession` writers completed. ARTIFACTS.md still described the pre-rc.62 "minimum 2 e2e" rule (it is stack-aware: backend-only accepts `integration`); claimed `export traceability` reads `requirement_compliance[].title` (it reads titles from `01_REQUIREMENTS.json`); omitted the `complete 5` run-binding tamper refusal, the `files_created`/`files_modified` string-path guard, and the whole B8 blocking-bug evidence contract. STATUS_JSON.md ladder gaps (P2 `artifact_missing` trigger, P4 suppression under blocking bugs) and `verifySummary` provenance.
- Also restored the integrations-CHANGELOG `## v2.0.0-rc.151` heading that the rc.154 commit had accidentally replaced (the BUILD_PLAN.md entry was left under the wrong version).

`INTEG-0704` · one visible output change (`status --json` severity value); no schema shape change; five contract docs reconciled to code truth.
...
- **D4 — the UX-routing decision has a single owner.** `approve.js` (the `approve requirements` next-action) and `snapshot.js` (the status/resume ladder) each inlined `['ux','visual','audio'].includes(fr.type?.toLowerCase())` to decide whether the UX phase must run before Architecture — snapshot's own comment said "Mirror the exact detection approve.js uses", and one divergence was already caught (ADV-0622-29: approve steered to `run-phase ux` while `status` pointed straight at Architecture). Both now call one `hasUxRequiringFr(reqs)` in `lib/requirements.js`; a test pins that the literal type-list lives only there. Behavior-preserving for well-formed inputs (existing routing-matrix tests unchanged); the type guard turns a throw on a hand-authored non-string `fr.type` into a clean `false` (never loses a real UX steer). Separately, the git-HEAD baseline probe that `approve` phase-4 inlined (duplicating `stampReconcileBaseline`) is now the single-owner pure helper `computeReconcileBaseline`. **Scoped honestly:** the phase-progression ladder itself is NOT delegated wholesale — `snapshot`'s ladder has more states than `approve`'s inline version (e.g. the all-skip verify-run no-op guard), so full delegation could not be behavior-preserving; the confirmed divergence class (the mirrored UX detection) is what the dedup closes.
- **D5 — the intent-coverage audit is wired into the Phase-1 flow, honestly.** `aitri audit requirements` (the `coverage_map` diff) has repeatedly caught real dropped needs, but it ran on-demand only and typically late, when a drop costs a pipeline re-open instead of a JSON edit. Now: `complete 1` recommends it as a FRESH-session relay step; `approve 1` flags it when it has not run for the current requirements (root + feature); `resume`'s nudge switched from a file-mtime comparison to the same signal. **Freshness is by content hash, not mtime** — new additive `.aitri.coverageAuditReqHash` (sha256 of `01_REQUIREMENTS.json`, stamped at invocation). mtime was broken three ways: `AUDIT_REPORT.md` is shared by three audit types (a security audit would fake requirements freshness), git clone resets mtimes (the exact ADR-048 failure), and same-content regeneration bumps it. One shared predicate `requirementsAuditState` is the SSoT for all three surfaces. **NOT a gate** (deliberate): the audit is agent-performed, so a blocking "no approve without audit" would only verify presence of a file the same agent wrote. **Honest ceiling:** the stamp is written at invocation — Aitri never verifies the audit was actually performed or the section is meaningful; this is placement + visibility, not proof.
- **The adversarial pass on D4** confirmed no defect and caught one over-claim in a code comment (the type-guard "preserves the routing outcome" is only exact for an all-malformed FR set; a mixed set moves `false→true` toward correctness) — comment corrected.

`UPLAN-0703` · new additive `.aitri` field + visible output at `complete 1`/`approve 1`/`resume`; D4 is behavior-preserving internal dedup.
...
- **C1 — `fr.description` reaches downstream phases; `project_summary` gets its first consumer.** `extractRequirements` forwarded `id/title/priority/type/acceptance_criteria` but never `description` — the richest behavioral prose in the PRD reached no downstream phase (architecture, tests, and build worked from titles). Forwarded now (same shape as the rc.140 `nfr.acceptance_criteria` fix). `project_summary` (North Star KPI / JTBD / guardrails) had ZERO consumers — Phase 2 now reads it from the RAW artifact on disk and renders a dedicated "Product intent" section (Phase 2 only; KPI/guardrails genuinely inform design trade-offs, everywhere else is prompt bloat). It deliberately does NOT travel through the extract: run-phase applies the producer transform before any consumer option could fire — the first cut's opt-in parameter was a no-op in the real flow, caught by the adversarial pass and pinned by a composition-level test that runs the two passes exactly as production does. The phaseUX persona fallback pointed at the stripped fields — now points at the artifact on disk.
- **C2 — client context renders INSIDE the briefing, before the instructions.** The ADR-066 "authoritative, injected-in-full client material" printed AFTER the Delivery Summary, the `Run: aitri complete N` line, and the Human Review checklist — an agent that stops at the instruction block never saw the client's spec. New `{{CONTEXT_ASSETS}}` placeholder in all six context-consuming templates (discovery/1/ux/2/3/4), positioned before Output/Instructions; the after-briefing print survives only as a defensive fallback. Stale positional claims fixed ("context above"/"listed below the briefing"); the "(no upstream artifact is approved yet)" parenthetical now checks the actual state.
- **C3 — persona/template division rule, and the three live drifts fixed.** Rule written down (`lib/personas/README.md` + ARCHITECTURE): personas carry role + judgment heuristics ONLY; ALL mechanics live in the template; a rule never lives in both. Fixed: `qa.js` contradicted `tests.md` twice (banned multi-FR TCs that `frs:[...]` sanctions; demanded ac_id that string-AC projects must not invent); `devops.js` restated `deploy.md`'s compliance table and had already drifted (dropped `production_ready` from a row); `pm.js` hardcoded the root no_go_zone floor (wrong in feature scope). The unconditional 375px viewport mandate (tests.md gates, build.md self-eval, phaseUX summary) became declared-medium-conditional — a kiosk/TUI/desktop product no longer imports mobile viewports (principle 4).
- **C4 — floors are floors, not targets (anti-anchoring).** `requirements.md` hardcoded the root floors ("Min 5 FRs / Min 3 NFRs / ≥3 no_go_zone") — a feature agent read them against a gate requiring 2/1/1 and padded with filler. Floors now render per scope ({{MIN_FR}}/{{MIN_NFR}}/{{MIN_NGZ}}) from a `phase1.js#scopeFloors` SSoT shared with the gate. And the framing changed on operator instruction — a printed minimum anchors the agent to produce exactly the minimum: the template now states the floor only rejects an empty artifact, **sizing comes from the seed via the coverage_map**, stopping at the floor when the seed implies more is silent scope loss, padding is still forbidden, and an honest below-floor decomposition splits coarse FRs rather than inventing filler. (Gate-side advisory-with-confirm for a genuinely-below-floor root product: deferred until a real project hits the floor honestly.)
- **C5 — ux/discovery print Human Review checklists.** Both templates lacked the `## Human Review` section, so `approve ux`/`approve discovery` printed NO checklist (the plumbing existed; the weakest-gated phases were the only ones without one). Added: UX fidelity-to-provided-design + FR coverage + no invented flows; Discovery problem-grounding + no solutioneering.
- **C6 — build/deploy contradictions and stack assumptions removed.** One completion bar (debug-mode "re-run only the failing TCs" vs DoD "full suite green" → targeted re-runs are a debugging step, the DoD is the exit); env-var items conditional on the declared config mechanism (three positions in one file before); CI checklist item names the DECLARED platform's config (a GitLab project no longer ships a dead `.github/workflows/ci.yml`); deploy verification is stack-neutral ("do not install dependencies", was "npm install"); the deploy Delivery Summary leads with the problem levels instead of `production_ready` counts.
- **C7 — `test/rendered-briefing.test.js` pins the briefing layer.** Renders every core briefing per scope and locks: no raw `{{...}}` tokens AND no silent-empty floor renders (render() replaces a missing key with '' by design — "≥ FRs" would ship green); the assets block precedes Output/Instructions per phase AND on phase-1 re-runs; root floors ≥5/≥3/≥3 vs feature ≥2/≥1/≥1; the known persona-drift vocabulary stays dead; ux/discovery carry the checklist and `extractHumanReview` (the approve-time print source) returns them. A run-phase-level integration test pins the composed ordering + no-double-print.
- **The adversarial pass found TWO ship-blockers in the first cut, both falsifying the release's headline claims; fixed here:** (a) the `project_summary` forward was dead in the real flow — run-phase applies the producer transform first, so Phase 2 re-extracted an already-stripped string; all three new tests passed because they asserted the wrong layer → disk-read mechanism + composition-level test; (b) `{{CONTEXT_ASSETS}}` sat inside `{{#IF_IDEA_MD}}`, so every phase-1 RE-RUN (the reject→refine loop) dropped the client material back to the after-checklist fallback → moved outside the conditional + re-run render test. Also from the pass: tests.md still contradicted ITSELF on ac_id (the checklist + intro mandated unconditionally what the ID rules make conditional) — the drift had moved persona→template, not died; fixed at both sites.

`UPLAN-0703` · no schema change; briefing output changes are the release.

---

## [2.0.0-rc.149] — 2026-07-03 — state integrity: unknown is never clean, corrupt is never reset, de-blocking carries evidence (UPLAN-0703 Phase B, B4–B8, ADR-070)

The audit's second finding cluster was a defect CLASS: failure paths that degrade to a reassuring answer. Five verified instances, one principle applied to all.

- **B4 — reconcile: an unreachable baseline refuses; entering pending clears the stale verify verdict.** The git-path failure fell through to the mtime comparator with a git SHA as the "timestamp" (`new Date(sha)` = NaN → `{files:[]}` → "✅ No code changes detected" + `resolved`) — a false CLEAN exactly when the tool knows least. Now: refusal with `aitri reconcile --init` recovery (`--init` may re-baseline over an unreachable ref; a reachable one stays clobber-protected). And detecting off-pipeline changes clears `verifyPassed` (+ new `reconcile-pending` event) — `--resolve`'s "tests still pass" gate no longer accepts a verify that PREDATES the drift. (`lib/commands/reconcile.js`)
- **B5 — `loadConfig` reads only `LOCAL_FIELDS` from `.aitri.local`.** The `{...raw, ...local}` merge let ANY key in the per-machine file shadow committed team state — and the next `saveConfig` PROMOTED the local value into the committed file. Shared keys found in the local file are now ignored with a warning naming them. (`lib/state.js`)
- **B6 — every `.aitri` parse failure refuses (G-4 extended).** A malformed non-conflict `.aitri` returned DEFAULTS; the next state-mutating command saved it, wiping approvals/hashes silently. Now: refuse with git-restore guidance (backup still written). The read-only snapshot keeps the tolerated `parseError`-flagged path for FEATURE configs; corrupt `.aitri.local` stays lenient. (`lib/state.js`)
- **B7 — `health.deployable` requires approved artifacts to exist on disk.** Deleting an approved artifact left status/resume/`status --json` green while `validate` said MISSING. New `artifact_missing` deploy-gate reason + a priority-2 next-action; the `approved` phase label is deliberately unchanged (approval is a state fact, existence a disk fact — both are now reported). (`lib/snapshot.js`)
- **B8 — bug lifecycle: de-blocking carries evidence; unreadable bug state refuses.** `bug fix` on a BLOCKING bug (critical/high, active) requires `--resolution` or `--tc`, and `bug close` on a blocking bug requires `--resolution` (non-blocking bugs keep the light path on both); `bug verify` refuses a never-fixed bug; a malformed `BUGS.json` refuses every bug command (a `bug add` — or verify-run's interactive auto-registration — would have OVERWRITTEN it) and every bug-reading gate (`verify-complete`, `reconcile --resolve`, `validate --ci` across root + features); unknown severity/status values (`P0`, `blocker`) warn once per file. (`lib/commands/bug.js` + gate wiring)
- **The adversarial-over-the-diff pass found a SHIP-BLOCKER in the first cut + four follow-ups, all fixed here:** (a) the malformed marker was an in-band string key — verify-run's auto-registration would serialize it INTO the file, destroying the bugs and poisoning the valid JSON so every gate false-refused forever → out-of-band Symbol + guard on the sibling writer; (b) the verifyPassed-clear fired on every reconcile re-run, destroying a verify that postdated the drift and churning the 20-capped event log → transition-only; (c) the B6 refusal leaked a raw stack trace → bin guidance branch; (d) `validate --ci` checked only the root's BUGS.json while the aggregate reads every pipeline → all pipelines checked; (e) bare `bug close` bypassed the fix-evidence gate → close on blocking requires `--resolution`.
- Docs: `docs/integrations/{SCHEMA,STATUS_JSON,CHANGELOG}.md` (marked `— breaking`: the `bug fix`/`bug close`/`bug verify` CLI contract), `docs/{DECISIONS(ADR-070),ARCHITECTURE,FLOW_INVARIANTS}.md`, `templates/AGENTS.md` (bug evidence contract, reconcile recovery, verifyPassed-clear). Tests: 19 added across reconcile/state/snapshot/bug/validate/smoke; 3 pinning tests flipped with rationale (local-precedence, malformed-resets, malformed-parseError).

`UPLAN-0703`

---

## [2.0.0-rc.148] — 2026-07-03 — the results file is trusted only when bound to a run (UPLAN-0703 Phase B, B1–B3, ADR-069)

The UPLAN-0703 audit found five one-step bypasses of the verification gate; three are closable mechanically with zero new dependencies. This closes them — converting "recorded not asserted" from slogan to mechanism on the deploy gate the whole harness rests on. **B1 is breaking-ish** (a pre-rc.129 stamp-less project is blocked once → one `verify-run` re-run); it lands now, inside the rc window, before v2 promotes to stable.

- **B1 — `verify-complete` now REQUIRES the run-binding stamp (breaking-ish).** The run-binding was `if (verifyResultsHash && hash !== stamp)` — an **absent** stamp passed by design. The cheapest audit bypass was a hand-written, internally-consistent green `04_TEST_RESULTS.json` that no test ever produced. Now an absent stamp is rejected ("no verify-run recorded — run `aitri verify-run`"), and a mismatched stamp is rejected as before. A pre-rc.129 project with a real results file re-binds by running `verify-run` (or `verify-run --results <file>`) once. (`lib/commands/verify.js`)
- **B2 — `tc verify` holds the same line as verify-complete: stamp required AND matching.** `tc verify` applied one entry then re-stamped the whole file — so a hand-edit of every `fail`→`pass` plus one legitimate `tc verify` re-blessed the tampered file (laundering), and a stamp-less hand-fabricated file could be blessed the same way (fabrication — the adversarial pass showed this defeated B1 in one extra command). It now refuses a stamp-less file ("no verify-run recorded" — `verify-run` seeds + stamps the all-manual project since rc.129, so no sanctioned stamp-less file exists) and a file edited since its stamp, on both the scriptable path and the guided loop. (`lib/commands/tc.js`)
- **B3 — one SSoT binding helper, re-checked on every read surface.** New `lib/state.js#verifyResultsBinding(dir, config)` → `bound | mismatch | no-stamp | missing-file`. `snapshot` computes it once (`verify.resultsBinding`); `computeHealth.deployable` blocks on `mismatch`/`missing-file` (new reasons `results_tampered`/`results_missing`); `validate` text/`--json` and `status --json` report real drift instead of hardcoded `drift:false`; `phase5.validate` refuses to build a compliance proof over a `mismatch`. So a post-run rewrite of the results file no longer hides behind the sticky `verifyPassed` flag on any surface. This also repairs the "single snapshot SSoT" claim — `validate` had drifted its own hardcoded logic. Additive to the JSON contracts (`resultsBinding` field + new reason types).
- **The adversarial-over-the-diff pass found a SHIP-BLOCKER in the first cut + three follow-ups; all fixed in this release:** (a) B2's first cut only guarded *mismatch* — a stamp-less fabricated file could be blessed via `tc verify` (→ stamp now required, above); (b) `verify-complete`'s stub-known-gap rewrite re-stamped in memory but persisted only at function end, so any later gate exit stranded the project falsely "tampered" on every surface → the stamp is persisted at the rewrite; (c) B3 gated only the root — a terminal feature (5/5, verify passed) with a rewritten results file stayed invisible → new `feature_results_tampered` deploy reason; (d) `hashResultsFile` only trimmed EOF whitespace, so a per-line trim-trailing-whitespace hook read as "tampered" → per-line strip added (hash-stable for standard JSON; a re-indent remains a documented mismatch, one `verify-run` recovers).
- **Honest ceiling (stated, not overclaimed):** these bind the results file to a *recorded* run; an agent that rewrites BOTH the file and `.aitri#verifyResultsHash` (both writable) still passes. The gain is raising the forgery cost from a trivial/accidental pass to a deliberate edit of a state field the agent is never told to touch — defense-in-depth on the rc.129 run-binding, not a new guarantee.
- Docs: `docs/integrations/{SCHEMA,STATUS_JSON,CHANGELOG}.md` (CHANGELOG marked `— breaking`), `docs/{DECISIONS(ADR-069),ARCHITECTURE,FLOW_INVARIANTS}.md`, `templates/AGENTS.md`. Tests: 12 added (B1 flip + fabricated-file block; B2 fabrication + laundering refusals + honest-flow-passes; B3 bound/mismatch/missing/no-stamp/feature-tampered across snapshot-health, validate --json, phase5.validate; per-line whitespace canonicalization).
...
- **F1 — the build phase's design reference is now FORM-AGNOSTIC (`lib/commands/run-phase.js`).** When a UI is built (a `01_UX_SPEC.md` exists), the build briefing surfaced only extension-matched *visual* assets (`png/pdf/…` + `.html`) from `idea_context/` as "open these" pointers — so a readable `.md`/`.txt` design doc reached the builder in **no form** (not injected, not pointed at, no notice): the intake-gap class one hop past where rc.137/AUDIT-0630-B closed it. The block now lists **every** context asset as a pointer regardless of extension — a design/spec reference can arrive in any form. Still pointer-only (no content injected → staleness-safe, the reason build excludes raw context) and still gated on a UX spec (backend-only builds keep the deliberate "no context at build" default). `ADOPTION_AUDIT.md` is excluded (it has its own framed block). Heading renamed "Visual reference" → "Design reference".
- **F2 — `nfr.priority` is forwarded by `extractRequirements` / `extractRequirementsForCompliance` (`lib/phases/context.js`), for parity with the FR arm.** This is a **symmetry cleanup, not a live-gap fix**: the canonical NFR schema has no `priority` (NFR MUST-ness is `category:"Regression"`, already forwarded), so the field is `undefined`/omitted in the common case. It only carries signal for an author who hand-sets `priority:"MUST"` on an NFR — previously the FR arm forwarded priority while the NFR arm dropped it, hiding that explicit-but-off-canonical MUST from the architect/QA at design time. Additive; coverage was never at risk (the phase-3 gate re-reads priority from disk).
- **F3 (held, not a defect):** the Phase-1 re-run asset framing "no upstream artifact is approved yet" is accurate — Phase 1's only prior is the IDEA seed / optional discovery, neither an approved pipeline artifact — so there is nothing to correct.

Normative reference produced by the sweep: `docs/Aitri_Design_Notes/_flow-invariants-2026-07-02.md` (local-only) — per-phase input-delivery/gate model future audits validate conformance against. +3 net tests (`test:all` 1894).
