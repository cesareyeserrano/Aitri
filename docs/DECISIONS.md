...
**Status:** ACCEPTED — shipped as canary 2.0.2-rc.1; promotes to 2.0.2 on publish after owner field-validation. Work items `W3 PERSONA-ROLE-EVIDENCE-0711`, `W4 COVERAGE-SIGNAL-0711`, `W5 PROMPT-EVAL-PINS-0711` (spec: `_research-adoption-design-0711.md`; one combined ADR at ship time per its §3). Closes the RSRCH-ADOPT-0711 executable thread — W2 stays owner-deferred (2026-07-11).
...
**Decision:** (1) ROLE = function-in-pipeline + downstream audience, never an expertise claim — "You are a Senior X" → "You are the X", Title-Case persona titles preserved (two tests pin them as the briefing's title source). Rule codified in `lib/personas/README.md`. (2) One caveat sentence next to the `coverage_goal` schema literal in `templates/phases/tests.md`: line coverage does not predict fault detection; the mutation score is the honest signal (declared as a Phase-4 quality_gate). (3) Pins in `test/rendered-briefing.test.js`: a negative canary on every exported persona `ROLE` (`/\b(senior|world-class|expert)\b/i` — scoped to ROLE, not whole files) + the W4 sentence + the existing W1 pin. W2 gets no pin — its content is unshipped.
...
Honest ceiling unchanged: an agent can still record a bogus skip reason — the warn makes the skip *loud*, not impossible. +3 tests (`complete.test.js`), AGENTS.md updated same commit.

## ADR-080 — 2026-07-17 — Feature increments inherit the parent product's standards; the standards-amendment mechanism is deliberately deferred (GOVERNANCE-0717)

**Status:** ACCEPTED — shipped as canary 2.1.0-rc.1 (promotes with the combined pack). Design session artifact: `docs/Aitri_Design_Notes/_governance-design-0717.md` (panel: scope + kill + inline mechanics; every load-bearing panel claim re-verified against code before acceptance).

**Context:** field case (adopted dashboard, 2026-07-17): every GUI feature re-invented product structure — a standardized indicator card rebuilt differently per feature, corrected only by the human in chat. Four verified gaps: features inherited ONLY parent FRs (`run-phase.js:146-151`), root UX/system-design artifacts were never injected into feature phases (parent context = pointer-only, TPA-4 — the bare-path bet ADR-066 records as field-falsified); `adopt scan` captured zero conventions; chat corrections had no destination; and (found by the panel) the completed-pipeline re-run guard excluded `'ux'` while its cascade wipes 2–5 + verify state.

**Decision (G1–G4):** (1) feature `ux` / phase-2 briefings inject the root spec's standards SECTIONS (anchored extraction — Design Tokens + Component Inventory; System Architecture + Implementation Approach), capped with a loud pointer fallback, stale-labeled on root drift, with the reuse/justify-deviations mandate — and the UX authority ladder amended in BOTH carriers (persona + template) to slot the parent standard between feature visual FRs and the archetype (a mandate block outside the ladder = ADR-065's inversion mirrored — kill-review condition). (2) `adopt scan` audits gain `#### Conventions Observed` (standards to follow, EXCLUDING Priority-Action defects), which also serves as G1's fallback when no root UX spec exists — the motivating adopted case. (3) Corrections: feature-local by default; promotion to product standard = deliberate human amendment of the root spec, named in AGENTS.md so the friction is a visible signal. (4) The re-run guard covers every phase whose `CASCADE_DOWNSTREAM` hits approved core phases (`state.js#cascadeDownstreamOf`).

**Rejected / deferred, with reasons:**
- **Automated amendment routing (original B3) — CUT by the panel:** re-opening approved root ux invalidates root 2/3/4/5/review + verify state (`state.js:641`, `run-phase.js:217-230`) — correct integrity semantics for spec changes, ruinous economics for additive registry growth; a mandate pointing agents there punishes compliance. An "additive amendments don't cascade" carve-out was rejected: "additive" is an honor-system judgment and breaks the TPA-1 integrity model.
- **CONVENTIONS artifact (alternative A) — deferred complement, not killed:** an off-pipeline, append-without-invalidate registry (BUGS/BACKLOG lifecycle class) is the honest long-term home for a growing standards registry, but its lifecycle has zero observed traffic to design from — building now is design-by-imagination. **Trigger (panel-corrected — "first amendment attempt" was suppressed by the very economics above):** recurring reuse-corrections in chat, or a product-wide standard with no home, within the field project's next 2–3 features. When it fires, A inherits G1's injection mechanics wholesale; the open design question is the three-masters split (CONVENTIONS vs root Design Tokens vs feature mockup).
- **A conformance gate ("did you reuse the component") — not attempted:** mechanical judgment Aitri lacks; presence-theater class (ADR-066 panel).

**Trade-off / honest ceiling:** injection ≠ compliance (ADR-066 class) — the standard now reaches the agent loudly, deviations are checkable at the approve gate (new checklist line), but obedience is honor-system. The injected inventory reflects the root spec AS APPROVED — components added by later features are absent until an amendment or the deferred registry; bounded by the freshness label + check-the-codebase instruction. Briefing size grows at feature spec phases (24KB/block cap). **Objection recorded (kill review):** B ships the cheap half (injection) and defers the load-bearing half (a living registry) — accepted knowingly; the deferral is evidence-gated, not silent.

**Impact:** High (feature briefing inputs, authority ladder, adopt template, re-run guard). **Value to produced software:** 9 — prevents the observed defect class directly (tier 1). **Severity:** Moderate.

## ADR-081 — 2026-07-17 — BUILD_PLAN stays a markdown working file with stable EP-ids; the machine surface is an advisory status projection — BUILD_PLAN.json and `verify-run --epic` REJECTED (PLAN-ARTIFACT-0715)

**Status:** ACCEPTED — shipped 2.1.0-rc.2. Design session B artifact: `docs/Aitri_Design_Notes/_plan-artifact-design-0717.md` (panel: kill + scope/mechanics, independent and CONVERGENT — both killed the implementer's own initial proposal on verified code evidence; owner's governing requirement: control over partial deliveries instead of big-bang).

**Context:** the owner asked for a more robust, better-structured plan with clear epic IDs, consumers = human checkpoints + Hub + Aitri. The implementer's initial design — BUILD_PLAN.json as a schema'd Hub-consumable + `verify-run --epic` for partial verification — died in panel on four verified facts: (1) `verify-run` refuses before `approve 4` (verify.js:232-241) — partial deliveries happen DURING Phase 4, so any verify-run flag is at the wrong layer; (2) the declared runner runs whole-suite, TC attribution is parse-time — stack-agnostic run-time filtering does not exist and per-runner filter injection violates principle 4, failing silent; (3) 04_TEST_RESULTS.json is wholesale-overwritten and run-bound (`verifyResultsHash`) — partial/merged results are the laundering surface ADR-069 closed; (4) an agent-authored JSON contract with no validator is a THIRD artifact class (the CLAUDE.md warning sign), promotes the highest-write-frequency hand-edited file to trusted, and freezes skeleton evolution that was exercised freely rc.151→rc.170 precisely because non-contract.

**Decision:** (1) The plan stays markdown, working-file class — with mandatory stable epic ids in the skeleton: `EP-NN`, assigned once, NEVER renumbered, per plan generation (a fresh plan after an upstream re-open starts a new id space — false-continuity guard). (2) `lib/build-plan.js` — a TOLERANT advisory reader (degrade-to-null; also accepts the rc.170 `Epic N` heading): correct posture because nothing gates on it; a gating reader would need refuse (BUGS.json precedent). (3) `status` shows advisory epic progress while Phase 4 is in flight; `status --json` gains the additive display-only `buildPlan` field (STATUS_JSON.md; consumers MUST NOT gate on it). Traceability is derivation-only: epic→US→FR→TC through the HASHED artifacts; no authored key is trusted.

**Deferred with recorded blueprint — mechanical mid-build epic verification:** a lightweight command (natural home: extend `checkpoint`) that runs the project runner, parses with the EXISTING parsers, derives the epic's TC set from `Delivers` via the hashed 03_TEST_CASES.json, and writes `epicVerifications` to `.aitri` (Aitri-written — no tamper surface; Hub reads via SCHEMA.md), never touching 04_TEST_RESULTS.json / verify state. Open questions for that session: checkpoint semantics, plan-generation marker, feature-scope doubling. **Trigger:** the owner asks for harness-verified (not agent-presented) epic completion after using EP-ids + status progress, or a real project needs the mechanical seal. Until then, mid-build control = the build protocol's existing mandate (agent runs the epic's TCs and presents output at each boundary) + the new between-session visibility.

**Trade-off:** the progress display trusts an agent-maintained file — accepted because it is display-only and stated as such in the contract; the mechanical seal is the deferred half. **Objection recorded (kill review):** "harness-verified epic done" keyed off any agent-editable set would certify less than it reads — which is exactly why the deferred design keys off the hashed Phase-3 artifact and writes through Aitri only.

**Impact:** Low-Medium (template grammar, one pure lib, status projection, one additive contract field). **Value to produced software:** 7. **Severity:** Low (advisory throughout).

## ADR-082 — 2026-07-22 — security threading is enforced, not honored: decision gate at Phase 1, conditional design-body gate at Phase 2, verify gap nudge — a security PHASE is rejected (SEC-THREADING-0722)

**Context.** The owner asked whether security should get a UX-style activation. Investigation first corrected the premise: the UX "activation" is conditional steering, not a blocking route (`run-phase.js` enforces nothing; `01_UX_SPEC.md` is an optional phase-2 input). Security's architecture — threading through phases + post-build `audit security` (ADR-051), no phase — matches the industry consensus (SDL/SAMM/shift-left: embedded activities, not a toll-booth stage) and is kept. But four links of the thread were prompt-only where their siblings are mechanical, each a verified defect path: (1) the mandated Phase-1 security applicability decision was unenforced — a project that FORGOT security was indistinguishable from one that decided it away, and `buildSecurityNfrSummary` returning null in both cases silenced the entire downstream chain (resume audit nudge, audit briefing context, build-template gate expectation); `resume.js` even documented a "Phase-1 forced decision" that did not exist. (2) `## Security Design` passed `complete 2` with an empty body while sibling Technical Risk Flags has a body check. (3) build.md declares a security quality_gate "EXPECTED" with a technical_debt reason on omission — verify never checked either, while UI FRs DO get the smoke-gap nudge. (4) tests.md's "security FR → Unit AND Integration both MUST" is unenforced. Same class as REG-GATE-0621: prompt mandates without mechanical backing get dropped.

**Decision.** Ship (SEC-THREADING-0722, 2.1.0-rc.7): a SSoT for security-NFR classification in `lib/requirements.js` (category predicate + terminator-hardened exclusion idiom `"Not applicable: …"/"N/A"/"No aplica: …"`, shared by all consumers so gates and nudges cannot drift); a blocking Phase-1 gate requiring the decision (≥1 Security-category NFR, promise or explicit exclusion; feature increments exempt unless they declare a security-typed FR); a blocking, conditional, fence-aware Phase-2 gate (active security NFRs ⇒ non-empty Security Design body; fails open on unreadable requirements); an advisory verify nudge (active security NFRs + no security-looking gate + no technical_debt record; fires for manual-only projects too — a scanner needs no test suite); template deepening (NFR→mitigation mapping + trust boundaries in Security Design — prompt-level by design, regex-over-prose is the fence-blind failure class); reserved technical_debt id `SEC-GATE` documented as contract.

**Rejected.** (a) A security phase / threat-model artifact / blocking route — industry anti-pattern, no artifact dependency (UX's justification), third agent-authored artifact class without a validator (the BUILD_PLAN.json argument, ADR-081). Re-open trigger: a real project where a post-build audit finding would demonstrably have been caught by a pre-build step the threading misses. (b) **The Phase-3 mechanical TC-type gate (unit+integration per security FR) — KILLED by the adversarial panel as theater; do not re-propose:** `phase3.js:221-233` already records that a type-forcing gate made agents mislabel TC types to clear it, corrupting the taxonomy — and a relabeled "integration" TC would newly satisfy the backend critical-flow floor it shouldn't. The gate would force metadata, not content. The prompt-level rule in tests.md stays. (c) Blocking verify on a missing security gate — false-block risk when security coverage lives outside quality_gates (CI, lint plugins).

**Objections recorded.** Kill review's strongest surviving argument: the apparatus mechanically guarantees only that a line exists, a section is non-empty, a nudge prints — real security still rests on the agent and the human, one level down; a filler "Not applicable: internal tool" on a PII-handling service manufactures false confidence at the very review layer that must catch it. Accepted with eyes open: this is the same trade no_go_zone and idea_provenance made — make omission visible and vetoable, not impossible — and the exclusion line is at least reviewable where silence was not. Mechanics review contributed: terminator-hardening of the idiom (mis-exclusion silently disables gates — the unsafe direction), fence-aware extraction for the new body check (FB-EXTRACT-FENCE-0718 lesson), fail-open ctx reads, name-only generic tokens in the gate heuristic, and the deliberate `buildSecurityNfrSummary` behavior change (one malformed entry no longer suppresses the whole summary) pinned in tests.

**Trade-off.** Every root project now records one security line even when security is irrelevant (the no_go_zone cost, accepted); stricter `complete 1/2` can newly fail a legacy project on re-complete — intended teaching, messages name the `--check` probe and both compliant shapes. Feature ceiling stated honestly: a feature that touches security WITHOUT declaring a security-typed FR gets only prompt-level G1 inheritance (cross-tree parent reads from a phase validator rejected as new coupling).

**Impact:** Medium (three validators + one nudge + SSoT module section; no schema/artifact-name change — the exclusion idiom and SEC-GATE become documented contract, additive). **Value to produced software:** 8. **Severity:** Moderate.

## ADR-068 Addendum — 2026-07-22 — the killed-run rule's boundary: a killed AUXILIARY e2e dispatch persists the finished unit run (FB-E2E-KILL-0722)

**Context.** ADR-068 established: a killed test runner "did not finish" — refuse without writing, because parsing a truncated run's output fabricates results and flips `verifyPassed` on a phantom. Field report (T-Ledger, 2026-07-22) exposed a boundary case the rule over-covered: the Playwright AUTO-RUN dispatch (verify.js, auto-detected via playwright.config) runs AFTER the main runner has already finished and parsed cleanly. Its kill invoked the same refuse-everything path, discarding 211/211 finished green unit results and deadlocking verify → verify-complete → reconcile --resolve on a project whose e2e could not run in that environment. The documented escape for exactly that situation (mark e2e TCs `automation:"manual"` + `tc verify --evidence`, taught by the e2e gate's own message) was unreachable: it requires a bound results file, which the kill destroyed.

**Decision.** Boundary refinement, not reversal: the MAIN runner's kill keeps ADR-068 behavior verbatim (its output is truncated mid-run — nothing trustworthy exists). A killed e2e DISPATCH now persists the results: the finished unit results are written, every e2e TC records `skip`, and none of the dispatch's partial output is parsed (zero false-pass surface). Additive `04_TEST_RESULTS.json#e2e_run {runner, status:"killed", reason}` records the kill. The Z1 stale-seal predicate gains the case: units-green + e2e-killed resets `verifyPassed`, so `reconcile --resolve` cannot ride an earlier full-green seal. The kill message teaches the two real exits (raise `test_runner_timeout_ms`; or mark-manual + evidence-verify from an environment where e2e runs).

**Rejected (adversarial design pass, do not re-propose):** a TTY-gated `verify-run --no-e2e` flag with a "provisional seal" refused at verify-complete/phase-5. Three independent kills: (1) it dead-ends on its own backstop — refusing at verify-complete never sets `verifyPassed`, and both `reconcile --resolve` and `run-phase 5` key on that field, leaving the motivating project exactly as blocked; (2) the mechanical e2e-debt gate it adds already exists — the verify-complete e2e coverage gate hard-blocks until an e2e TC passes (runner or evidence-verified manual), discharged only by evidence, never by acknowledgment; (3) the TTY gate breaks CI (non-TTY, where "e2e unrunnable here" is common) and inverts calibration (`tc verify --result pass`, a strictly stronger override, is deliberately scriptable). Also rejected: parsing the killed dispatch's partial output (false-pass surface), and a separate `e2e_timeout_ms` (deferred until the per-run wait proves painful in the field).

**Trade-off.** A results file no longer implies every declared runner finished — readers key on `e2e_run` for the killed-dispatch case (integrations CHANGELOG, additive). The phantom-regression protection ADR-068 exists for is preserved: nothing fabricated is recorded, and the reset of `verifyPassed` reflects a true state (current run lacks e2e evidence).

**Impact:** Low (one dispatch branch + one predicate arm + additive field). **Value to produced software:** 7 (unblocks honest unit sealing without weakening the e2e evidence requirement). **Severity:** Moderate (the defect it fixes deadlocked a real consumer's verify chain).

## ADR-083 — 2026-07-22 — Aitri records the declared release identity; it never versions the product (RELEASE-VERSION-0722)

**Context.** Owner observation: consumer projects carry no real versioning through Aitri, and there was no defined way for the team to tell Aitri what version a release is. Code verification found the mechanism half-existed and was dead: `05_TRACEABILITY.json#version` is REQUIRED by the phase-5 validator, but the deploy template gave no provenance instruction (`"version": string — e.g. "1.0.0"` → agents write filler forever, never tied to the product's manifest or any tag) and NO surface consumed it (status/snapshot/export read zero bytes of it). A required field with no consumer and no provenance is the CLAUDE.md "field for completeness" warning sign, already shipped.

**Decision.** Make the existing field honest; do not expand Aitri's role. (1) The deploy template scripts the interaction — the AGENT asks, the TEAM decides (the idea_provenance pattern): read the product's own package descriptor and propose it; or ask the team for the version and their scheme; on re-releases propose the bump from what changed and let the human confirm; never invent filler — unconfirmed values are marked `version_source: "assumed"`. (2) New optional-additive `version_source` (`"manifest" | "team" | "assumed"`), vocabulary-gated when present, omitted = valid legacy artifact. (3) Consumers: `status` release row, additive `status --json#release`, the `approve 5` human checkpoint displays the sealed version and warns loudly on `"assumed"`, and each `approve 5` appends `{version, version_source, at}` to the new first-class `.aitri#releaseHistory` (capped at 50) — the durable cross-cycle release record. The phase-5 `approved` event also carries the payload, but the 20-entry event log evicts a full cycle's events, so the event alone cannot serve as history (skeptic finding pre-commit; same eviction lesson that made `lastVerifyRun` first-class in rc.25).

**Rejected (do not re-propose without new evidence):** (a) Aitri deciding or bumping versions — semver semantics are business judgment, the team's; (b) mechanically reading or editing the product's package descriptor — parser-per-stack burden and Aitri never touches product files (out of identity; re-open only if field evidence shows declared-vs-manifest mismatches causing real harm, and then as a read-only cross-check); (c) an `aitri release` command that tags — git releases are the team's workflow (single-agent-sequential, git-mediated).

**Honest ceiling.** Aitri cannot verify that the declared version is true — it guarantees the version was asked, its source declared, surfaced at the human checkpoint, and remembered across cycles. Provenance-honesty, not proof; the same trade as idea_provenance.

**Impact:** Low (template + one optional field + display surfaces + event payload; all additive). **Value to produced software:** 6 (release identity tied to verified state; the "1.0.0 forever" record lie removed). **Severity:** Low (advisory surfaces; the only gate is vocabulary-when-present).

## ADR-084 — 2026-07-31 — no sanctioned code-first burst mechanism: the discipline's answer to an owner override is protest, not permission — command, convention artifact, and prose tiers ALL REJECTED (CODE-FIRST-BURST-0729)

**Status:** ACCEPTED — nothing shipped, no bump. Study artifact (local): `docs/Aitri_Design_Notes/_code-first-burst-design-0731.md` (3-adversary panel: technical kill + identity skeptic + minimal-alternative advocate, claims verified against code; then owner review, convergent).

**Context:** field evidence (2026-07-29, consumer-project transcript, feature `transferencias`): mid-refinement the agent proposed the spec-first re-open; the owner overrode — "código primero, papeles después." Aitri has no sanctioned middle path (reconcile teaches refactor→resolve / functional→pipeline), and the decisions fixed during the window lived only in the chat (no durable surface: `lastSession`/`reconcileState`/`sessionContext` are all per-machine, state.js:21). The study evaluated four designs of descending surface: (v1) `burst open|note|close` command + shared `.aitri` field; (A′) `BURST_LOG.md` convention artifact + presence detection + gate refusals + a Phase-1 injection channel; (prose) an AGENTS.md protocol / generalized decision-capture line; (owner's alternative) a hard gate blocking forward progress (`feature init`/`run-phase`) while reconcile is pending.

**Decision: reject all four.** The field case shows the discipline WORKING, not failing: the agent proposed spec-first because the teaching is binary; the owner's override is human authority, already inside the validation model; and for the whole window the system told the truth — suites red, `reconcile: pending`, verify unpassable, deploy gate closed (`reconcile_pending` + `verify_not_passed` + drift are deploy-blocking today, snapshot.js:812). The deferral happened UNDER PROTEST, and the protest is the mechanism: it is precisely the unsanctioned-ness that made the override a conscious, expensive, owner-initiated act. Every tier — verb, artifact, or teaching paragraph — converts that protest into permission and normalizes the red it claims to bound. The agility valves already exist (`reconcile --resolve` for maintenance; `aitri feature` as the light loop), and the hard wall already sits at the exit.

**Per-tier kill grounds (recorded so a future session does not re-derive them):** v1 — `approve 4` is an ungated exit that advances the reconcile baseline cross-scope (approve.js:678-687); a separate close command shreds mid-window work via unconditional cascade; TTY-gating the open sits on a non-TTY chat moment; `baseRef` had no consumer. A′ — mechanically sound, but its machinery guards two UNverified claims (bursts recur; taught agents fail capture discipline) at the price of a permanent artifact-name contract; N=1 with zero materialized loss. Prose — its only real consumer is the unsanctioned window itself; every other decision moment already has a landing surface (`## Resolutions`, Delivery Summary blocks, the artifacts). Forward-progress gate — prevents no evidenced defect (no field case of drift buried under new work), global lock with team-scale false positives, and Aitri is passive: it cannot block code edits, only its own commands, so the gate bites at return-time — when the operator is already normalizing.

**Re-open trigger (one, shared):** a second independent field instance where the EXISTING protest mechanism demonstrably failed — a window that rotted unbounded, buried drift under new features, or lost an owner decision despite visible red/pending/closed-exit. The evidence names the exhumation: decision lost → A′'s capture surface; drift buried → the forward-progress gate; rot despite visibility → reconvene on the full space (all four designs remain mapped in the study doc).

**Trade-off:** the decisions fixed during a future unsanctioned window remain chat-vulnerable until its close — accepted: the evidenced instance lost nothing, agents are already taught the capture principle at every artifact surface, and closing that residual channel is exactly what the trigger detects if it ever materializes. **Objection recorded (technical panel + implementer's own v1/A′):** the durable-surface gap is real and code-verified; rejecting the fix is a bet that the protest mechanism plus agent discipline suffices — if the trigger fires, that bet lost and this ADR routes the response.

**Impact:** none in code (decision-only). **Value to produced software:** prevention — protects the discipline signal (unnormalized red) that consumer projects' honesty depends on. **Severity if wrong:** Low — the trigger converts a wrong bet into a routed, pre-designed response.

## ADR-085 — 2026-08-05 — the verify verdict is bound to the code tree it certified (`verifyRanRef`); the multi-team concurrency layer is rejected — merge honesty, not merge management (MULTI-TEAM-0722)

**Status:** ACCEPTED — shipped 2.2.0-rc.7. Study artifact (local): `docs/Aitri_Design_Notes/_multi-team-study-0804.md` (reproduce-first on a real git repo with the installed CLI + full 3-adversary panel: technical kill × identity × evidence; owner-mandated FULL treatment).

**Context.** The last open study asked whether Aitri needs a concurrency layer (ownership, role auth, locks) for multi-agent/team work. Reproduction answered decision 2 empirically: feature dirs merge clean (the sub-pipeline IS the natural concurrency unit); G-4 and refuse-don't-reset behave as designed; but **the verification spine was not merge-aware** — after merging two feature branches (including a hand-resolved conflict in shared code), the sticky shared `verifyPassed` reported "deployable Ready" on a tree NO verify run had seen, and the per-machine reconcile baseline auto-stamped fresh clones at merged HEAD, burying the window (study R3). Reachable SOLO today: all three consumer repos use PR merges carrying `.aitri` paths, Ultron merges git worktrees (`.aitri.local` is gitignored — every worktree/clone auto-stamps), dependabot merges behavioral code with no verify. The kill pass added the single-machine hole: `detectUncountedChanges` surfaces a count but never blocks; only `reconcileState: 'pending'` blocks, which requires someone to happen to run reconcile.

**Decision.** (1) **Bind the verdict to the tree** — `verify-run` stamps `verifyRanRef` (HEAD) + `verifyRanDirty` (uncommitted behavioral files, reconcile allowlist) as SHARED fields; the snapshot derives `verify.refState`: `fresh` (no behavioral diff ref..HEAD — docs-only commits stay fresh) / `stale` (behavioral change since the run — deploy-blocked, `verify-complete` refuses) / `unreachable` (certified commit gone: rebase/squash — refuse-not-pass, B4 doctrine) / `dirty` (ran over uncommitted code — machine-local, non-portable warn, taught exit = commit-then-verify, the same clean-tree rule `reconcile --resolve` enforces) / `unbound` (non-git or pre-rc.7 stamp — explicit degradation to prior behavior, never a block). New deploy reasons `verify_stale_ref`/`verify_ref_unreachable`; a P5 ladder rung routes the exit (visible-deadlock guard); STALE-VERIFY-1's calendar exemption comment rewritten (code staleness now has its own tracked signal). ADR-069's register, extended from the results FILE to the code TREE: raises the cost of a stale pass; does not prevent a deliberate `.aitri` edit. (2) **The concurrency layer is REJECTED — tombstone confirmed by all three panels.** No ownership, locks, role auth, or per-team state: nothing reproduced needs them; git already mediates. Identity sentence: *Aitri remains single-agent-sequential and per-checkout passive; branching and merging stay the team's git workflow. Aitri's contribution at a join is honesty: merged-but-unverified code can never read deployable — it demands re-verification; it never recognizes, waives, or coordinates.*

**Rejected, with grounds (do not re-propose without new evidence):** shared `lastSealedRef` seal (M2) — a scalar cannot represent a merged DAG (re-creates the false-drift it fixes), reintroduces guaranteed shared-file conflicts on a load-bearing field, dead-ends under squash-merge, and "recognizing sealed commits" converts reconcile's TRUE post-merge protest into a re-verification waiver — fail-open, the protest→permission inversion ADR-084 refused. Trigger: a real parallel-branch project documents a re-classification tax; any revival must be a union-mergeable seal SET with its own kill pass. · saveConfig write-skip "fix" (M3) — premise falsified (the skip works; the study's R1 was a fixture artifact — correction recorded in the study doc; 115-commit field scan found zero churn). · Ownership/locks/roles — trigger: a real multi-team failure git + the merge-aware spine cannot express.

**Corrections recorded (evidence discipline):** study R1 ("guaranteed root `.aitri` conflict on parallel first-inits") RETRACTED — canonical bases hold the write-skip; R4 (post-merge drift flag on another branch's sealed work) RECLASSIFIED as correct fail-safe behavior, deliberately NOT suppressed. Residuals with the tombstones: feature-level `verify.passed` stays sticky/unbound (root re-verify covers the union); parallel root `BUGS.json`/`BACKLOG.json` appends collide loudly (refuse-don't-reset, manual dedup); GATE-SCOPE-BLIND-0805 (root `verify-complete`/`--resolve` blind to feature blocking bugs that the snapshot deploy gate aggregates — panel find, its own backlog item).

**Trade-off:** one more mechanical gate in the loop — agents that verify before committing now get a stale block on the next commit; the taught discipline (commit-then-verify) makes the linear flow pay zero. Dirty verdicts remain locally usable (warn, not block) to keep the inner loop cheap. **Impact:** High (deploy gate + verify-complete semantics, 2 additive shared fields, ladder rung, AGENTS.md discipline). **Value to produced software:** 8 — kills a reproducible silent false-pass at the deploy gate, the exact class the tool exists to prevent, unreportable by construction if left (a victim ships believing the gate passed). **Severity:** Critical (silent false-pass class).

### ADR-085 — Addendum 1 (2026-09-08, 2.2.0-rc.11) — the frame normalization is promoted to a shared module; reconcile's three git readers are migrated (RECONCILE-FRAME-0908)

**Context.** M1 introduced path-frame normalization (`gitShowPrefix` + `isBehavioralInFrame`) because git answers in REPO-relative paths while `isAitriStatePath()` matches PIPELINE-relative prefixes. That work was wired into the verify-ref binding ONLY. Its own header named the consequence for the other readers — a livelock — and left it standing. A field report of a *different* (non-existent) loop between `verify-complete` and `reconcile --resolve` triggered an adversarial pass that reproduced the real one: wherever the frames diverge (project root = repo subdir, or a feature pipeline under `aitri/features/<name>/`), Aitri's own `.aitri` and artifacts read as project code, so `reconcile` cleared `verifyPassed` over Aitri's own writes, `--resolve` then demanded a verify, verify rewrote `.aitri`, `--resolve` demanded that commit, and the commit was fresh drift. Non-terminating; `run-phase deploy` unreachable. Reproduced end-to-end with the installed CLI; control run at the repo root terminates.

**Decision.** Promote the normalization to `lib/git-frame.js` as the SSoT — and make that module the **owner of every git invocation that yields paths** (`gitDiffNames`, `gitPorcelainPaths`, both `-z`), not just of the predicates (`isBehavioralInFrame`, `isAitriStateInFrame`, `toPipelinePath`, `gitShowPrefix`, `verifyRefRootCtx`). Every git-path reader is migrated: `reconcile.gitChangedFiles`, `reconcile.uncommittedBehavioralChanges`, `snapshot.detectUncountedChanges`, `captureVerifyRef`, `verifyRefFreshness`, `bug.gitDiffFiles`. Emitted paths become pipeline-relative (the frame `mtimeChangedFiles` already used, and the only valid pathspec from the project dir — the briefing's per-file diffs had been silently rendering empty for subdir projects); out-of-subtree paths keep the repo-relative form anchored with git's `:/` prefix and stay conservatively behavioral, preserving M1's parent-code visibility. `pendingFiles` carry an additive per-machine `pendingFilesFrame: "pipeline"` marker; an unmarked (pre-rc.11) set is trusted for growth detection only where the frames provably coincide (repo root) and otherwise backfills without clearing.

**Adversarial pass on the first cut (three corrections folded before commit).** (D1, ship-blocker) the diff readers parsed `git diff --name-only` text, which C-quotes non-ASCII paths under the default `core.quotePath`; a `café` feature's own `.aitri` came back quoted, failed every prefix match, and re-opened the class — with the new `:/` anchor turning the silent miss into a git `fatal` swallowed by the diff renderer. Resolution: `-z` everywhere, no quote parser exists. (D2) the migration guard was a permanent "known under either frame" alias that under-fires forever (a subtree dir named like the project basename masks real growth — a false negative in the stale-proof guard itself) and missed out-of-subtree entries. Resolution: the frame marker above. (D3) `bug.gitDiffFiles` was a fourth unframed reader. Resolution: migrated. Rejected in the same pass: re-framing `lastSession.files_touched` (display-only, no filter, no defect — left as is).

**Also decided (the report's own proposal, REJECTED).** The reporter asked that both messages name "re-derive Phase 4" as the escape from the perceived loop. Refused: the perceived loop terminates (reproduced — one extra verify pass), the cheap exit is already named in three places, and Phase-4 re-derivation cascades and re-stales every feature verified against the old baseline — the "treadmill" the same project had already recorded as a cost. Teaching the expensive route as canonical would have converted an accidental cost into the documented one.

**Trade-off:** one more module in the read path of three hot commands, a `:/`-prefixed path form that is unusual to read in a briefing, and one cycle of undetected growth for a subdir/feature project that was ALREADY pending at upgrade time (it was in the livelock anyway) — accepted over leaving six readers each free to filter an unframed or quoted path. **Impact:** Medium (three readers + one new module; no schema change — `pendingFiles` is per-machine `.aitri.local`). **Value to produced software:** 8 — an entire class of project (monorepo subdir, and every feature pipeline in the DEFAULT contained layout) could not reach deploy. **Severity:** Critical (non-terminating gate cycle, no operator-visible exit).

### ADR-085 — Addendum 2 (2026-09-11, 2.2.0-rc.15) — the verify ref is read before the dispatches, not after them (VERIFY-REF-PRERUN-0911)

**Context.** Found while analyzing FB-VERIFY-ONCE-0911 (T-Ledger, ~9 min per feature `verify-run`). `captureVerifyRef` ran at stamp time, AFTER the test runner, the Playwright auto-run and every quality gate. Any commit landing in that window (the operator, a second agent session, a merge into the same checkout) became `verifyRanRef` although no dispatch had run against it. `verifyRefFreshness` then diffed that ref against HEAD, found nothing and read `fresh`: a silent false-fresh in exactly the class ADR-085 exists to close, with a window that grows with suite cost. The dirty flag had the mirror blind spot: code uncommitted at start, then committed or reverted mid-run, left a clean post-run read. Reproduced at command level by a runner that commits while `verify-run` is running.

**Decision.** Read `{ref, dirty}` once before any dispatch and once at stamp time. Stamp the PRE-run ref, the tree the dispatches started from. `verifyRanDirty` = dirty at start OR dirty at end, and a failed end read counts as dirty (unknown is not clean). A moved HEAD needs no new field or special case: freshness already diffs ref..HEAD, so a behavioral mid-run commit reads `stale` and a docs-only one stays `fresh`. `verify-run` announces the move and names the bound commit. If exactly one of the two reads fails, the stamp still binds — to whichever ref was read — as dirty: unknown is not clean, at either end. Only when both fail (not a git project, or no commit yet) is there no stamp (`unbound`, the documented degradation).

**Adversarial pass (folded before commit).** (1) `gitPorcelainPaths`/`gitDiffNames` ran with Node's 1 MiB default output buffer. A tree with a few thousand untracked, non-ignored files (a leftover report or coverage tree) threw ENOBUFS, `captureVerifyRef` returned null, and every `verify-run` silently deleted the stamp — reproduced. Both readers in `lib/git-frame.js` now carry a 256 MiB ceiling, which also covers reconcile's and the snapshot's use of the same helpers. (2) A failed end read was reported as "UNCOMMITTED behavioral changes"; it now has its own message. (3) The first cut left a failed START read `unbound` (no gate effect, silent) while a failed end read stamped dirty; the asymmetry is removed as above. (4) Fixture commits in the two git-fixture suites this change touches (`verify-ref-binding`, `reconcile-frame`) disable `commit.gpgsign`, as the snapshot and reconcile suites already did. Named and not acted on: an explicit `--results` FILE older than the run start is still read, with a runtime warning (ADV-0622-11) — pre-existing attestation semantics, unchanged here.

**Second pass, on the folded delta.** The higher ceiling changes two other readers, both in the intended direction. `reconcile --resolve`'s working-tree guard (`uncommittedBehavioralChanges`) swallowed the overflow and returned no files — fail-open — so a large untracked tree let the resolve through; it now reports it, which means a leftover, non-ignored report or coverage tree blocks `--resolve` like any other uncommitted behavioral file. The refusal used to print every path, so its list is now capped at 50. `verifyRefFreshness` on an overflowing diff returned a false `unreachable`; it now returns the real `stale`. Named, not acted on: `bug close` can record a very long `files_changed` when the fix and close commits are far apart.

**Trade-off:** one extra set of cheap git reads (`rev-parse` ×2 + `status`) per `verify-run` (milliseconds against a multi-minute run); a run with a behavioral mid-run commit now ends stale and must be repeated, which is the honest cost. Residual, named and not closed: a tree edited and restored entirely within the run, with no commit, is invisible to both reads. That is the ceiling of two-point sampling; closing it would need watching the tree for the run's duration, rejected (no daemon, WATCH-0718). **Impact:** Low (one command; no schema shape change — `verifyRanRef` now matches its documented meaning). **Value to produced software:** 7 — removes a silent false-fresh from the deploy gate. **Severity:** Critical class (silent false-pass), narrow trigger.

## ADR-086 — 2026-08-05 — root mechanical gates aggregate blocking bugs across all scopes; feature gates stay scope-local (GATE-SCOPE-BLIND-0805)


**Status:** ACCEPTED — shipped 2.2.0-rc.8. Origin: MULTI-TEAM-0722 panel finding, then an adversarial check that REVERSED the implementer's own forming recommendation (warn + teach) on verified code facts.

**Context.** Root `verify-complete` and `reconcile --resolve` read only their own scope's `BUGS.json` (`getBlockingBugs(dir,…)`), while the snapshot deploy gate aggregates cross-scope. An in-progress critical FEATURE bug therefore let root verify-complete flip `verifyPassed` and `--resolve` stamp the baseline while status said blocked — and the divergence had teeth: `health.deployable` is display + optional CI only (no pipeline command refuses on it); the mechanical door to Phase 5 is `verifyPassed` alone (run-phase.js), and `approve`/`complete` check no bugs — so the full sealing path (`run-phase 5 → complete 5 → approve 5` → `releaseHistory`) was mechanically open over a fleet-critical bug. Three witnesses already declared the global contract: AGENTS.md's unqualified teaching (since v2.0.0-rc.2), the deploy spec comment ("no blocking bugs (global — features can block root ship too)"), and rc.110's fix intent ("matches the snapshot deploy gate and AGENTS.md"); only the input set disagreed. Same defect family as rc.110's status-axis divergence — the repo's ruling there was align, don't annotate.

**Decision.** (1) New `getBlockingBugsAllScopes(rootDir, config, err)` — root + every feature scope through the shared `isBlockingBug` predicate (AUDIT-0629-G SSoT), B8 per scope (an unreadable BUGS.json refuses; a parse failure must not silently drop a scope's blockers). (2) ROOT `verify-complete` and `reconcile --resolve` gate on that set; refusals carry scope provenance and the owning scope's fix command. (3) **Feature gates deliberately stay scope-local** — root bugs never block a feature's own verification (the WIP-feature asymmetry the snapshot already encodes); naively globalizing `getBlockingBugs` for every caller was the implementation trap, avoided. (4) `--resolve`'s zero-changes early return stays ungated (deliberate: an empty-diff baseline advance asserts nothing about unverified code; gating it would refuse a harmless bookkeeping no-op) — pinned by comment; a WARN-instead-of-block design was REJECTED as the protocol's "honor system where the system had deliberately eliminated it" (blocking bugs have zero epistemic ambiguity, unlike the MUST-NFR-skipped advisory whose ambiguity is documented).

**Objection recorded (skeptic, accepted as residual):** severity-inflation gaming — teams learning that a "high" feature bug freezes root gates may under-register real blockers as "medium"; severity-at-registration has no audit trail (unlike `mark-manual`'s `downgraded_from`). Weighs less than leaving the sealing path open: the same incentive already exists for same-scope bugs, and audited exits exist (`bug close --resolution`, `bug fix` + `verify`, `feature discard`). If field evidence shows the gaming channel is real, a severity-change audit trail is the response, not gate weakening.

**Residuals recorded (implementation adversarial):** (a) `approve 4` (root and feature — the feature form advances the ROOT baseline, approve.js:686) stamps `reconcileState` with no bug gate — a TTY/human-gated route to the same baseline stamp `--resolve` now guards; left as-is deliberately (human checkpoint), named here so a future gate discussion starts from the record. (b) Behavior change: root gates now REFUSE when any feature `.aitri` is conflicted/malformed (G-4 propagates through the helper's loadConfig; previously the root gate simply never read feature state) — correct direction (a conflicted scope could hide blockers), visible asymmetry vs the snapshot's tolerateConflict rendering, documented in the CHANGELOG.

**Impact:** Medium (two gate input sets + one helper + teaching; no schema change). **Value to produced software:** 8 — closes a mechanically open ship path over fleet-critical bugs. **Severity:** Critical (silent false-pass class, same family as ADR-085's R3).

## ADR-087 — 2026-09-02 — host-CI run state becomes visible in the operator loop: a read-only `gh`-orchestrated advisory in plain `validate` and the security audit — never a daemon, never a block (CI-VISIBILITY-0906)

**Status:** ACCEPTED — shipped 2.2.0-rc.9. Owner-requested with two same-day field cases.

**Context.** Two consumer projects carried RED security workflows unattended — one (govulncheck gate) since June, one (scheduled npm-audit job) failing on main — while the operator's terminal loop (status/resume/validate) showed nothing: the gates fired correctly in Actions, but their state lived only in a browser tab nobody watches. The mitigation hierarchy: (1) the MECHANICAL answer already existed — declaring the same checks as project `quality_gates` puts them in the verify spine (both projects had NOT declared them; both now do); (2) rc.6's posture step checks CI *configuration*, not run *results*; (3) this ADR adds the missing visibility layer.

**Decision.** New `lib/host-ci.js`: `readHostCiStatus()` — read-only probe through the PROJECT'S OWN `gh` (`git`→branch, `gh run list --branch <default> --json`, 8s timeout), judging each workflow by its latest COMPLETED run (in-progress ≠ verdict); silent-with-reason degradation (`no-git`/`no-gh`/`gh-failed`/`no-runs` — never an error, never an exit-code change). Surfaced: (a) **plain `validate`** renders the advisory (failing workflows + dates + the quality_gate teaching); (b) **`audit security`** (template + persona) now reads run RESULTS where the host CLI works — a red latest run on the default branch is a finding with its age. **Boundaries, deliberate:** NOT in `status` (must stay fast/offline); NOT in `validate --ci` (probing CI from inside CI is circular latency); NOT in `validate --json` (Hub polls it in cycles — a network call per cycle is the documented battery-leak class, and no consumer asked for the field — the field waits for a consumer per protocol); ADVISORY always (a red run does not mechanically mean a broken build — the evidenced T-Ledger case was a scheduled dependency-audit job; the blocking path IS a declared quality_gate). **Rejected:** a daemon/watcher (WATCH-0718 tombstone stands — reaffirmed by Hub's battery-leak incident); a mechanical deploy-gate block on red CI (semantics too ambiguous for a gate; would also make `validate` network-dependent).

**Trade-off:** plain `validate` gains up to ~8s of network probing when `gh` exists (bounded by timeout; zero when absent); GitHub-only until another host CLI shows up in the field (conditional phrasing, principle 4). **Impact:** Low-Medium (one module, one advisory surface, prompt clauses; no schema, no gate change). **Value to produced software:** 7 — the evidenced failure class is real vulnerabilities rotting unattended; visibility at the "am I ready?" moment closes the attention gap the passive CLI can honestly close. **Severity:** Low (advisory only).

---

## ADR-088 — 2026-09-03 — child output is captured to a file, not a pipe: Aitri stops manufacturing kills of its own; `detached` process groups REJECTED (DISPATCH-PIPE-0903 + COST-VISIBILITY-0903)

**Status:** ACCEPTED — shipped 2.2.0-rc.10. Owner-reported field case: a consumer project (T-Ledger) whose machine kept dying under verification.

**Context.** Nine kernel OOM events in the seven days the macOS record covers, one forcing a restart mid-session. The diagnosis separated three causes, only two of them Aitri's: (a) a test the project itself had written that shelled out to run its own whole suite, which re-entered itself ~54,000 processes deep in 11 minutes (not Aitri's, but its templates never forbade it); (b) Docker Desktop's default half-the-RAM ceiling on a 16 GB machine (not Aitri's); (c) **the dispatch mechanics below, which are entirely Aitri's.** Two independently reproduced defects, both consequences of capturing child output through a PIPE:

1. **The pipe holds the run open.** `spawnSync` with piped stdio returns when the LAST holder of the pipe closes it, not when the child exits — measured here: a child that exits immediately but leaves a background survivor returns in 6.0 s (the survivor's lifetime), versus 0.0 s when the survivor's stdio is redirected. An e2e suite that boots an app server is exactly that shape, so a FINISHED 2-minute run held the CLI to the 15-minute timeout and was then recorded as a killed dispatch with every e2e TC `skipped` — a green verify-run accrediting nothing, at 15 minutes of peak memory per occurrence. The consumer measured the same thing independently (2:07 with output to a file vs not finishing in 30 min through the pipe) and had to work around it inside their own test harness — a project should not have to know Aitri's capture mechanics to get a truthful verdict.
2. **The pipe imposes a ceiling.** Past it Node kills the child mid-run and truncates. rc.129 raised the runner to 64 MiB for exactly this reason and documented why 1 MiB is a defect — but `runQualityGates` never got the fix and stayed on Node's 1 MiB default, so a gate that merely TALKS a lot (a full e2e suite, a verbose scanner) was killed and recorded `error` for a check that never failed. A required `error` blocks `verify-complete`: Aitri was blocking the pipeline on its own capture buffer.

**Decision.** One helper, `spawnCaptured()`, backs all three dispatch sites (test runner, Playwright auto-run, quality gates): the child writes to two temp files (stdout/stderr kept separate, so every existing parser sees exactly what it saw before), which are read back after it exits and unlinked. No pipe for a survivor to hold; no ceiling for Node to kill over. A read past the cap is flagged `truncated` and every caller treats it as "this run was not fully seen": the main runner REFUSES exactly as the old ENOBUFS path did (no partial credit, no phantom regression — ADR-068 doctrine unchanged, minus the murdered process), the auto-run warns and leaves under-credited TCs as `skipped` (which still blocks at `verify-complete` — failing safe), a gate keeps its real exit code and records that only its tail was captured. No writable tmpdir → falls back to the historical pipe capture, so a degraded environment loses nothing it had.

**What this does NOT change, deliberately and pinned by `test/dispatch-capture.test.js`:** a real test failure still fails; a timeout kill is still a kill and never a pass; a killed gate is still `error`; a gate's real exit code is judged in both directions however loud it is; an over-cap read still refuses. The net effect on verification is MORE accreditation, not less — results that used to disappear into `skipped` because of Aitri's own pipe are now recorded. This was the owner's stated concern when authorizing the work ("me preocupa degradar las pruebas que hace aitri"), and it is the reason half the new tests are rigor pins rather than fix tests.

**Rejected — `detached: true` + process-group sweep.** The obvious companion fix for orphaned grandchildren (a killed dispatch leaves them alive: reproduced — the direct child takes the SIGTERM, the grandchild survives) is to give the child its own process group and `kill(-pid)` it. `spawnSync` does honor `detached` (verified: distinct pgid) and the sweep works (verified: the grandchild dies). It was still rejected: a detached child leaves the terminal's foreground process group, so Ctrl-C no longer reaches the suite, and a SIGINT handler cannot run while `spawnSync` blocks (verified: the handler never fired during an 8-second blocking call). The trade would be "no orphans, but Ctrl-C stops stopping your test run" — strictly worse for the operator. Orphan sweeping needs a design that does not cost interactive control; it stays open, unbundled from this fix.

**Also shipped (COST-VISIBILITY-0903).** Additive `04_TEST_RESULTS.json#timings` (`runner_ms`/`e2e_ms`/`gates_ms`/`total_ms`) + `quality_gates[].duration_ms`, and a `## Cost` block in the run summary naming the slowest gates. Motivation is the same field case: this project's verify-run executes the whole e2e suite TWICE (once as Aitri's auto-run, once as its declared `e2e` gate) and builds the app three times, and none of that was visible in a pass/fail summary. Nothing gates on the numbers — the cheapest honest step is to make the cost legible before deciding what to cut.

**Still open, named here so it is not lost:** the Playwright auto-run cannot be scoped or suppressed even when a declared gate already runs the same suite, because the auto-run is the only path that credits e2e TC ids (`quality_gates` output is judged by exit code and never parsed) — the sound fix is to parse TC ids from gate output first, then let a project opt out of the auto-run; and the double-execution nudge that exists misses a gate like `{name:"e2e", command:"./e2e.sh"}` because `hasPlaywrightGate` greps for the literal word "playwright". Both are structural (schema + gate behavior) and get their own decision.

**Hardened by the adversarial pass before ship (findings verified against the code, not taken on trust).** (1) The capture is now **unlinked at open time** on POSIX and read back through the open fd: a signal that terminates Aitri — the OOM kill and the Ctrl-C at the centre of this very ADR — runs no `finally`, so a `finally`-only cleanup leaked exactly on the event the change is about, and a surviving orphan kept growing the leaked file. Nothing is reachable on disk now; the inode is reclaimed when the last fd closes. Windows cannot unlink an open file, so there the paths are removed on the way out and a grandchild holding the handle can still defeat it — a documented gap, not a solved one. (2) `readCappedFd` distinguishes **`failed` from empty**: a read that could not happen (ENOMEM on a large log, a lock, a tmpdir cleaner) used to return an empty string, which the runner would have credited as a run that printed nothing — every TC skipped, `verifyPassed` false, a phantom regression with no diagnostic. It now refuses, and a gate keeps its real exit-code verdict while recording that its log was lost. (3) `Buffer.alloc` + an honoured `readSync` return, so a short read cannot leak uninitialised process memory into the artifact. (4) The runner no longer reads the tail of an over-cap log it is about to refuse (~130 MB of RSS, measured, on a machine already short of memory). (5) Additive `e2e_output_incomplete` preserves the provenance that an over-cap e2e log used to carry as an `e2e_run` kill record. (6) The raw-Playwright block in the summary is capped like the runner block — pre-rc.10 an oversized e2e log could not reach it because the pipe killed the dispatch first.

**Trade-off:** the pipe ceiling was a real (if brutal) resource bound, and it is gone — a runaway child now writes into the temp filesystem until it exits or hits `test_runner_timeout_ms`, so the worst case moved from RAM to disk and is bounded only by that timeout. Judged acceptable — the old bound was enforced by killing healthy runs, which is what this ADR exists to stop — but it is a bound Aitri no longer has. The pipe FALLBACK (no writable tmpdir) is close to, not identical with, pre-rc.10 behaviour: gates get `readCap` there instead of inheriting Node's 1 MiB default, i.e. more tolerant of loud gates, never less. Two temp files per dispatch; a pathological log now costs the run's full wall time before Aitri refuses, where the pipe used to abort it early — the run is honest instead of murdered, and the refusal message says how to shrink the output. **Impact:** Medium (one shared helper under three dispatch sites; no schema change to `.aitri`, no artifact-chain change, two additive result fields). **Value to produced software:** 8 — it removes a class of green-that-accredits-nothing (the worst failure mode a verification harness can have), unblocks pipelines that were failing on Aitri's buffer rather than on their code, and stops the harness from holding a machine hostage for 15 minutes per verbose run. **Severity:** Moderate (the defect degraded verification truthfulness, not just cost).
## ADR-089 — 2026-09-09 — creation order is rendered into the file tree as a derived, gitignored `features/INDEX.md`; numbered feature directories stay rejected (FEATURE-INDEX-0909)

**Status:** ACCEPTED — shipped 2.2.0-rc.12.

**Context.** The owner asked twice, from real use, to see the order features were created in "in the project". rc.2 (FEATURE-ORDER-0729) answered in `aitri status` (`#N · created · verified`) and tombstoned the literal proposal — an ordinal in the directory name. The second report located the gap precisely: the operator looks at the IDE's folder tree, which sorts alphabetically and where no Aitri surface existed. Tier-2 (operator usability), real, twice-reported, and the information already existed — the failure was placement, the FB-SCOPE-BLIND family again.

**Decision.** A generated markdown view at `<features>/INDEX.md`: one row per feature in creation order with created date, phase count, verify state and last-verify date. Derived from `createdAt` through `proposalOrder()` — the single rule the status render and the next-action ladder already share (`creationOrdinals()` now serves all three, replacing the status render's private copy) — so the number in the tree, in the terminal and in the ladder cannot disagree. Regenerated by `status` (text only), `resume` and `feature init`; idempotent; never read back by Aitri; removed when the last feature goes; never creates `features/` on its own. **Gitignored**, with the layout-aware entry added by `init`/`adopt --upgrade`, or once on the index's first creation for projects that re-ran neither.

**Rejected, with grounds (do not re-propose without new evidence).** *Ordinal in the directory name* (`001.grid-ux`) — re-affirmed: a frozen number collides when two people create features on parallel branches (team target), lies about implementation order (creation ≠ verified; in the owner's own project `backend` is #5 by creation and verified after #20), breaks the `aitri feature <verb> <name>` grammar, and renames every consumer's directories. Ids never encode order (ADR-074/081). *Committing the index* — a derived file that conflicts on every parallel feature creation is the mild form of the same cost and carries no state; a fresh clone regenerates it on its first `status`/`resume`, which AGENTS.md makes the session-opening move. *Writing it from `buildProjectSnapshot`* — the snapshot is the read-only SSoT behind `validate --ci` and Hub's `status --json` polling; a write there, even idempotent, would break that contract. *Leaving it to Hub* — Hub renders `features[].createdAt` (rc.2) for its own view; the reported need is the tree, which only a file can serve.

**Adversarial corrections folded pre-commit.** Ownership marker (never delete/overwrite a file Aitri did not write — a flat project's own `features/` is a real collision), ignore-before-write and re-ensured whenever missing (an un-ignored derived file is the exact cost this ADR rejects), `layoutRoot` confinement (the write side must not trust hand-edited committed JSON), atomic write, and in-block `.gitignore` placement. Residual named, not fixed: `state.featuresDir` itself still resolves a garbage `layoutRoot` verbatim on the READ side (pre-existing; reads find nothing there).

**Trade-off.** A file that exists only to be looked at, which can lag the tree until the next `status`/`resume`/`feature init` (verify dates change without those running) — accepted: staleness is bounded by the session-opening command and the header says what refreshes it. One bounded `.gitignore` edit from a read-ish command on first creation — accepted over leaving an untracked derived file the operator might commit; surfaced on stderr. **Impact:** Low (one new module + three hooks; no schema, no artifact, no `.aitri` change; feature discovery already filters `isDirectory()`). **Value to produced software:** tier-2 — 3 direct; justified as placement of an existing signal, not new machinery. **Severity if wrong:** Low (a derived file; delete it and it regenerates).

## ADR-090 — 2026-09-09 — a backlog item is mutable after creation through two verbs — `note` (append-only journal on the item) and `update` (fields in place); the item does not become a phase input (FB-BACKLOG-AMEND-0909)

**Status:** ACCEPTED — shipped 2.2.0-rc.13.

**Context.** `aitri backlog` had one mutator, `done`. The owner reported from real use that an analysis growing across sessions had nowhere to accumulate. Triage (protocol + adversarial pass, claims verified against the owner's Ultron project) reclassified it from preference to verified defect: the agent had hand-edited `BACKLOG.json` — appending to `problem` with hand-written dates, replacing `acceptance`, bumping priority, and closing five items with `status: "done"` + `resolvedAt`, which every reader (`list`, `aggregateBacklog`, Hub) counts as open. The shipped Entry Standard (`templates/BACKLOG.md`) demands items be "expanded before scheduling"; the CLI could not do it, so the file was edited by hand and drifted.

**Decision.** Two verbs on the existing command, feature scope through the existing dispatch. `note <id> --text` appends `{ at, text }` to an optional `log[]` — append-only; entries are never edited and are rendered in file order (a merge that reorders stays visible). `update <id> --<field>` sets `title/priority/problem/fr_id/files/behavior/decisions/acceptance` in place, no per-field history; `--status` is refused because the lifecycle stays in `done`. Both stamp `updatedAt`, both allowed on closed items. In the same change, the drift path is closed at its root: B8 parity (malformed file marked — syntax AND shape, mirroring `snapshot.readJsonList`; every mutator refuses — `add` previously overwrote a corrupt file, and the first cut of this change coerced a non-array `items` to `[]` before the adversarial pass caught it), an unrecognized-status warning on the `backlog` sub-commands, and case-folded `done`. Field named `log`, not `notes` (`notes` is a string on test results).

**Where the growing analysis lives — decided, do not re-litigate.** *On the item*, not as a pointer to a document: the evidence shows the owner accumulates inline on the item, and a `--ref` to `idea_context/` would make the backlog a second index of loose files. But the item is **not** promoted to a phase input: `BACKLOG.json` is read by no phase or audit briefing (the audit prompt only suggests `backlog add`), and injecting the backlog into discovery/requirements would put un-triaged "maybe later" text in front of the persona as if it were intent. The log retains intent; the agent carries `backlog show <id>` into the feature seed when the item becomes a feature. This limit is stated in AGENTS.md and ARTIFACTS.md rather than implied away.

**Rejected / evidence-gated.** `--file`/stdin for long text (agents pass multi-paragraph strings through `execFile` without difficulty; a human rarely types them); `reopen`; `done --note`; `feature init --from BL-xxx` (the seed is a hand-carried copy today — build the bridge when a project shows the copy being lost); per-field edit history (the backlog is neither a gate nor an audit record).

**Trade-off.** Two more verbs and one array field on a public artifact — carried forever under the additive rule. `update` overwrites without history, so a wrong edit is recoverable only from git; accepted, because the alternative (an edit log on every field) is bookkeeping no consumer reads, and the file is committed.


## ADR-091 — 2026-09-09 — a `fixed` bug is reported as "awaiting verification", never as open; the bug record is correctable through `note`/`update` with severity changes journaled; the session narrative renders as unverified text, last, and is flagged stale by content (BUG-STATE-VISIBLE-0909)

**Status:** ACCEPTED — shipped 2.2.0-rc.14.

**Context.** Owner field report (T-Ledger): "things already fixed keep being treated as open; it is always something different." Triage against the project's files found the report true and plural. `resume` rendered `open|in_progress|fixed` bugs under one heading, "Open Bugs", with no per-line status — 9 of 11 were `fixed`; `status` said "(open/in-fix)"; `validate` said "open". The fixed→verified transition is mechanical only through `tc_reference`, which none of the project's 55 bugs carried (the only writer of that field is the TTY prompt in `verify-run`, which never fired for agent-registered bugs), so `fixed` was a de-facto terminal state that displayed as open. Separately, the agent's `checkpoint --context` narrative — 4 KB, printed verbatim above bugs and Next Action — carried dated bug counts, an unverified hypothesis parked deliberately outside bug/backlog, and "the title of BG-036 is wrong, the CLI cannot fix it, do not trust it"; its stale flag was clock-based and the commands that invalidate such text (`bug fix|verify|close`, `backlog done`) never wrote `lastSession`. Backlog got `note`/`update` in rc.13; bug had not.

**Decision.**
1. *Display tells the states apart.* Active (open|in_progress) and fixed are separate groups in `resume` (each line carries its status; the fixed group names its exits and says "not open defects"); `status`/`validate` break the count down; `status --json` adds `bugs.active` + `bugs.fixed` and leaves `bugs.open` (active + fixed) untouched — Hub reads it from `--json` with that meaning, while Hub's direct BUGS.json reader counts `open` only; neither definition moves. `resume` also prints the verify ref state on root and feature lines — `verify ✅ (stale)` — because a stale-but-passed verify rendered as "verified; now verify".
2. *`fixed` is not auto-verified.* Steelmanned and rejected: a later green verify-run whose FR tests predate the bug does not prove the bug fixed; auto-promotion would be a mechanical false-pass on the deploy gate's own input. The exit stays `bug verify` (human) or a linked TC (`bug update --tc`) that `verify-run` checks.
3. *The record is correctable.* `bug note` (append-only `log[]`, same shape as backlog's) and `bug update` (fields in place). Divergence from ADR-090, justified: BUGS.json is a gate input, so every severity change is journaled and lowering a BLOCKING bug out of critical/high requires `--reason` — the evidence contract of `fix`/`close` (B8), closing ADR-086's "severity gaming" residual. `--status`/`--resolution` stay with their verbs. `bug fix` on an already-fixed bug refuses: it re-stamped `fix_commit_sha`/`fix_at` to the current HEAD (adversarial finding — the first draft's hint pointed exactly there).
4. *The narrative is narrative.* Rendered last among state sections, framed "agent-written, not verified by Aitri; the sections above are current"; stamped at checkpoint with `stateAtSave` (active/fixed bugs, open backlog) and diffed against the live snapshot on `resume` — stale by content, mechanically, with no text parsing; bug/backlog mutators now write `lastSession` so the clock flag fires too. No length cap: length is a proxy, not the defect. AGENTS.md states what does not belong in the narrative and why.
5. *Feature seal advisory, no auto-close.* One line at `feature verify-complete` when the root backlog has open items. The field item (BL-040) was delivered by two features — the link is N:M — so an auto-close at the first seal would be a false close. `feature init --from BL-NNN` stays evidence-gated (ADR-090).

**Objections recorded.** (a) "Reordering and framing text is theater — the narrative already said 'nine are fixed, not open' and the next session still got it wrong." Accepted in part: the text-on-text pieces are the weakest; the mechanical pieces (state split in the counters, content-stale diff, mutators stamping `lastSession`, the record being correctable) are what carry the fix. (b) "Bug filing on unreproduced premises is the agent's discipline, not Aitri's." Agreed — Aitri cannot verify claims; what it can do is make a false record correctable and keep hypotheses out of the channel that never closes.

**Not built, own items.** `package.json` is treated as non-behavioral though gate commands resolve through its scripts (the owner's `test:e2e → ./e2e.sh` rewire changed what `verify-run` executes for the root and seven features without marking drift — a false-fresh hole in ADR-085's allowlist). The security-audit nudge in `resume` is mtime-based (a fresh clone re-triggers it after a covering audit).

**Trade-off.** Two more verbs and one array field on a public artifact, carried forever under the additive rule; one per-machine stamp on the narrative; `resume` output reorders (no known text-parsing consumer). The display split makes `fixed` bugs visible for what they are — which also makes the manual `bug verify` debt visible; that is the point.

## ADR-092 — 2026-09-11 — TC ids are unique across the pipelines of a project: Phase 3 refuses an id another pipeline already plans; verify-run warns on older plans (TC-ID-COLLISION-0911)

**Status:** ACCEPTED — shipped 2.2.0-rc.16. Design confirmed by the owner before build (a new blocking content gate).

**Context.** Found while analyzing field feedback FB-VERIFY-ONCE-0911. `verify-run` credits a plan TC by its id alone — an exact lookup in the ids parsed from runner output, plus a case-insensitive fallback — with no binding to a test file or pipeline. A feature's runner often executes the whole repository: in T-Ledger, 13 features share one `unit.sh` that changes to the repo root. Phase 3 rejected duplicate ids only inside one `03_TEST_CASES.json`; the briefing's canonical examples are bare `TC-001h`, and its namespace rule described a format, not an obligation. So a feature planning `TC-001h` while the root also plans `TC-001h` receives the root's result: a pass while the feature's own test is missing or renamed (false pass), or — fail-wins — a failure it never owned. **Live, verified:** a read-only scan of the four consumer projects on the maintainer's machine found collisions in two. In one (18 pipelines, 77 colliding ids) two features run the same test file; one of them records `TC-009h`/`TC-010h`/`TC-011h` as `pass` (not manually verified) for "tryRenderBlocks removed from chat.js", "lang-toggle CSS and JS references removed" and "all 8 chip translation keys removed", credited by the sibling's single same-id tests, which assert glass tokens, a STATE object and `buildDetailView` — and no test anywhere references what the three TCs describe. The other (16 pipelines, 270 colliding ids, several features running the whole suite) was not traced test by test. T-Ledger and a fourth project have none. Any cross-pipeline result sharing (FB-VERIFY-ONCE P1) would make the class systematic.

**Decision.** One source, `lib/tc-collisions.js`: it enumerates the root and every feature through the layout (`featuresDir`, `.aitri` required), reads each plan tolerantly (a conflicted or malformed sibling is skipped — its own gates refuse it) and compares ids case-insensitively, matching verify-run's fallback. (1) `phase3.validate` refuses an id another pipeline plans whenever it is given the project on disk (`ctx.dir` — `complete 3` and `--check`); the refusal names the other pipeline and, in a feature, suggests a namespace derived from the feature name. Content-only validate calls skip it. (2) `verify-run` warns, without blocking, when automated plan ids still collide — a plan approved before this gate is not walled; renaming routes through Phase 3, where the gate then applies. (3) `adopt --upgrade`'s read-only validator pass over approved phases surfaces root-side collisions like any other validator finding (a feature dir validated without `featureRoot` sees no siblings). The briefing (`tests.md`) and `AGENTS.md` state the rule.

**Rejected, with grounds.** Blocking `verify-complete` on legacy collisions — with the evidence above it would wall roughly 17 pipelines across two projects at their next re-verification, each exit being a Phase-3 rename that cascades to build and verify; the owner-approved design keeps it advisory, and the warning names every collision on the next `verify-run`. Revisit if a warned collision ships a false pass that a block would have stopped. **Owner decision (2026-09-11, after seeing the live evidence): stay advisory.** Binding results to test files or pipeline paths — needs per-runner knowledge of where file paths appear in output (stack-specific, principle 4). Auto-namespacing ids — renaming a plan silently desyncs it from test titles.

**Trade-off:** re-completing Phase 3 in a project that already has a collision now requires a rename on one side (plan ids + test titles), even when that project's runners are scoped and would never cross-credit — accepted: uniqueness is a cheap invariant, and "this runner is scoped" is a property Aitri cannot verify. `complete 3` and `verify-run` read every sibling plan (22 small JSON files in the largest known consumer). **Impact:** Medium (new blocking content gate in Phase 3; no schema change). **Value to produced software:** 7 — closes a silent false-pass class in the verification spine. **Severity:** Critical class (silent false pass), live in a consumer project.

**Adversarial pass (folded before commit).** (1) A feature collided with its OWN plan when its name was typed in a different case on a case-insensitive filesystem: `feature <name>` builds the dir from the typed name, and the self-exclusion compared paths (`fs.realpathSync` keeps the typed case) — reproduced; identity is now device + inode. (2) The refusal was pinned only through `validate()` with a hand-built ctx; if `complete.js` stopped passing `dir`/`featureRoot`, the gate would switch off silently — pins now drive the real `complete 3` in both scopes and with `--check`. Named, not acted on: running bare `aitri complete 3` from INSIDE a feature dir treats the feature as a root with no siblings (a pre-existing scoping hazard of the project-dir walk; the gate inherits it); symlinked feature dirs are not enumerated, consistent with the snapshot's pipeline discovery; the Phase-3 check counts manual TCs too (a simple rule, though a manual-vs-manual pair cannot cross-credit); `adopt verify-spec --complete` stubs TCs into the root plan without Phase-3 validation (the verify-run warning still catches a collision); the upgrade validator notice points at the root's Phase 3 even when the feature is the side to rename; the briefing's schema example still shows a bare `TC-001h` (the rule line and the refusal's namespace suggestion carry the fix).
