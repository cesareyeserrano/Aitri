...
- [x] CI workflow for PR + push `main` with deterministic install and smoke test gates
- [x] CI workflow executes machine-readable Aitri checks (`status --json`, `validate --json`)
- [x] Release workflow for semantic tags (`vX.Y.Z`) added
- [x] Release workflow enforces package version/tag parity before publish
- [x] Release workflow publishes npm package with secret-based auth (`NPM_TOKEN`)
- [x] Release workflow creates GitHub Release notes
- [x] CI/CD runbook documented for contributors and maintainers
- [ ] Project-level CI/CD scaffolding command in Aitri CLI (currently documented as manual baseline)
