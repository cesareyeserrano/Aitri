...
> Last updated: 2026-07-22 (v2.1.0-rc.7).
...
| 2.2.0-rc.10 | **Child output is captured to a FILE, not a pipe (DISPATCH-PIPE-0903)** — one helper, `verify.js#spawnCaptured()`, backs all three dispatch sites (test runner, Playwright auto-run, `runQualityGates`). Two reproduced defects were consequences of the pipe itself, and both turned into verification results: (1) `spawnSync` returns when the LAST holder of the pipe closes it, not when the child exits — a suite that leaves an app server running held a FINISHED run open to the 15-min timeout, which was then recorded as a killed dispatch with its e2e TCs `skipped` (green that accredits nothing); (2) the pipe ceiling killed loud-but-healthy children — the gates were still on Node's 1 MiB default and were recorded `error` for checks that never failed. Judging is unchanged and pinned in both directions (`test/dispatch-capture.test.js`): failures still fail, kills are still kills, an over-cap read still REFUSES rather than crediting a tail. **`detached` process groups REJECTED** — they would let Aitri sweep orphaned grandchildren, but a detached child leaves the terminal's foreground group and a SIGINT handler cannot run during a blocking `spawnSync`, so Ctrl-C would stop stopping the suite (both measured); orphan sweeping stays open, unbundled. Additive with it: `04_TEST_RESULTS.json#timings` + `quality_gates[].duration_ms` + a `## Cost` block in the run summary (COST-VISIBILITY-0903), because a verify-run that executes the same e2e suite twice was invisible in a pass/fail view | ADR-088 / DISPATCH-PIPE-0903 (2026-09-03) |
| 2.2.0-rc.15–16 | **The verify verdict binds to what actually ran, and a TC id belongs to one pipeline.** (rc.15) `verify-run` reads the binding ref + dirty state BEFORE any dispatch and again at stamp time — a commit made mid-run was being stamped as certified; the git path readers in `lib/git-frame.js` carry a 256 MiB ceiling (the 1 MiB default silently dropped the binding on large untracked trees). (rc.16) New cross-pipeline invariant: **TC ids are unique across the root and every feature** — `verify-run` credits results by id alone and a shared runner prints every pipeline's ids, so a shared id cross-credits. One source, `lib/tc-collisions.js`; `complete 3` refuses a new collision, `verify-run` warns on legacy ones (advisory — renames route through Phase 3) | ADR-085 Add.2 / VERIFY-REF-PRERUN-0911 · ADR-092 / TC-ID-COLLISION-0911 (2026-09-11) |
| 2.1.0-rc.7 | **Security threading enforced (SEC-THREADING-0722)** — the phase-threaded security model (no security phase — kept deliberately; industry shift-left shape) leaves the honor system at its four weak links: `complete 1` gates the security applicability decision (≥1 Security-category NFR — active promise or the terminator-bound exclusion idiom "Not applicable: …"; features exempt unless they declare a security-typed FR); `complete 2` requires a non-empty fence-aware `## Security Design` body when active security NFRs exist; `verify-run` gains the fifth advisory nudge (active security NFRs + no security-looking gate + no `technical_debt` record → stderr; reserved debt id `SEC-GATE`); classification SSoT in `lib/requirements.js` (see Shared predicate SSoTs). A mechanical per-FR-type TC gate (unit+integration per security FR) was KILLED as theater — phase3's own history shows type-forcing gates make agents relabel TC types | ADR-082 / SEC-THREADING-0722 (2026-07-22) |
| 2.1.0-rc.2 | **Stable epic IDs + advisory epic progress (PLAN-ARTIFACT-0715)** — the Phase-4 plan skeleton mandates stable `EP-NN` ids (assigned once, never renumbered, per plan generation); `lib/build-plan.js` reads BUILD_PLAN.md TOLERANTLY (degrade-to-null — nothing gates on it) to feed an epic-progress line in `status` and the additive display-only `buildPlan` field in `status --json`. BUILD_PLAN.json-as-contract and `verify-run --epic` REJECTED by panel (wrong layer: verify-run refuses pre-approve-4; no stack-agnostic run-time filtering; partial results break ADR-069 run-binding; agent-authored JSON contract = third artifact class). Mechanical mid-build epic verification deferred with blueprint (`epicVerifications` in `.aitri`, Aitri-written) | ADR-081 / PLAN-ARTIFACT-0715 (2026-07-17) |
| 2.1.0-rc.1 | **Feature increments inherit the parent product's standards (GOVERNANCE-0717)** — feature `ux` briefings inject the root `01_UX_SPEC.md`'s `## Design Tokens` + `## Component Inventory` (anchored section extraction, `lib/phases/context.js#extractSections`; per-block cap → loud OPEN-this pointer, never silent truncation; staleness warning when root ux is drifted); feature phase-2 briefings inject the root `02_SYSTEM_DESIGN.md`'s `## System Architecture` + `## Implementation Approach` ("conform or justify" — never `head()`, which carries decisions not standards). Adopted-case fallback: no root ux spec → the adoption audit's new `#### Conventions Observed` section (adopt scan template; conventions-to-follow, explicitly excluding Priority-Action defects) injects instead. The UX authority ladder gains a slot IN BOTH carriers (persona + template): feature-provided design → feature visual FRs → **parent standard** → archetype → inferred context (a mandate block outside the ladder = ADR-065's inversion mirrored). Adjacent verified defect fixed: the completed-pipeline re-run guard now covers every phase whose cascade hits approved core phases (`state.js#cascadeDownstreamOf`) — previously a non-TTY re-run of a drifted approved `ux` wiped 2–5 + verify state unconfirmed. Automated standards-amendment routing deliberately NOT shipped (cascade economics punish it); CONVENTIONS artifact deferred with an observable trigger | ADR-080 / GOVERNANCE-0717 (2026-07-17) |
...
| Security posture declared / promised / excluded? | `lib/requirements.js` `securityNfrs()` / `activeSecurityNfrs()` / `isSecurityExclusion()` / `hasSecurityFr()` (terminator-bound exclusion idiom `SECURITY_EXCLUSION_RE`) | phase1 (decision gate), phase2 (Security Design body gate), verify (security-gap nudge), audit `buildSecurityNfrSummary` (→ resume nudge + audit-security briefing) (ADR-082) |
