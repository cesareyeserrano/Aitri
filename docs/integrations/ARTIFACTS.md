...
```json
{
  ...
  "coverage_map": "array<{need, disposition}> (v2.0.0-rc.124+, ADR-060) — the agent's idea→requirement decomposition made explicit: one entry per distinct need found in the seed, with `disposition` = the FR/NFR id that covers it OR \"out_of_scope\" (reason in no_go_zone). Required by the gate on a fresh seed (skipped once Phase 1 is approved — historical thereafter). Additive for readers (old readers ignore it). Consumed by `aitri audit requirements`, whose prompt directs the agent to diff its own independent re-derivation of the needs against this map to surface a silently-dropped need (the diff is agent-performed, not a mechanical Aitri operation).",
...
- **Security decision gate (v2.1.0-rc.7+, SEC-THREADING-0722/ADR-082):** at least one `non_functional_requirements` entry with `category` matching `/security/i` is required — root pipelines always; feature sub-pipelines only when a functional requirement has `type` matching `/security/i`. The entry is EITHER an active security promise OR an explicit exclusion: a `requirement` field that opens with the exclusion idiom — `"Not applicable: …"`, `"N/A"`, `"Does not apply."`, `"No aplica: …"` — where the phrase is immediately followed by `:` `.` `;`, a whitespace-preceded dash (`"Not applicable — reason"`), or end-of-text. A comma or an unspaced hyphen does NOT terminate: `"Not applicable, except …"` (exclusion-with-exception = a promise) and `"NA-region access control must…"` classify ACTIVE. **The idiom is contract** (SSoT: `lib/requirements.js#SECURITY_EXCLUSION_RE`): readers distinguishing "security promised" from "security excluded" must use it. Consumers: the `complete 2` Security Design body gate, the verify-run security-gap nudge, and the `resume` security-audit nudge all key on active (non-excluded) security NFRs.
...
- **Every MUST FR must have ≥1 linked user story, and every user story linked to a MUST FR must have ≥1 `acceptance_criteria` entry** (v2.0.0-rc.173+, [ADR-077](../DECISIONS.md), REQ-RICHNESS-0711). A proportional depth floor: SHOULD/NICE FRs are exempt; feature scope obeys the same rule. Presence is enforced, not form — a plain string, `{ id, text }`, `{ id, description }` (legacy), or `{ id, given, when, then }` all satisfy the AC requirement (Given/When/Then is the target the template pushes, not a gate). Before rc.173 `user_stories: []` and a MUST FR with no story passed silently, producing thin/underspecified downstream tests. The JSON *shape* is unchanged (no new field) — this is a required *population* of existing fields, so readers are unaffected; only producers (and `adopt --upgrade`'s advisory gate re-check on pre-rc.173 approved requirements) see it
- **FR `priority` is required and must be exactly `"MUST"`, `"SHOULD"`, or `"NICE"`; NFR `priority` is optional but must be canonical when present; an `acceptance_criteria` entry must carry actual content** (v2.0.0-rc.174+, ADR-077 addendum, REQ-RICHNESS-0711). Priority is the join key every MUST gate filters on (exact-match `=== "MUST"` in phases 1/3/5) — before rc.174 a miscased `"must"` or a priority-less FR silently exempted the requirement from every MUST gate, and a content-free AC element (`""`, `{}`) satisfied the rc.173 presence floor. The JSON shape is unchanged — a required population/vocabulary of existing fields; readers are unaffected; producers and `adopt --upgrade`'s advisory gate re-check see it
...
**Security Design body (v2.1.0-rc.7+, SEC-THREADING-0722):** when `01_REQUIREMENTS.json` declares **active** security NFRs (security-category entries not phrased as the exclusion idiom — see the 01 section above), `## Security Design` must have a non-empty body, not just the header (same rule Technical Risk Flags has always had). The check is fence-aware (a `## Security Design` line inside a code fence neither satisfies nor terminates the section) and fails open when requirements are unreadable. Projects with no security NFRs or only exclusions are unaffected.
...
- **Reserved `fr_id` value `"SEC-GATE"` (v2.1.0-rc.7+, SEC-THREADING-0722):** a `technical_debt` entry with `fr_id: "SEC-GATE"` records the deliberate omission of a security quality_gate on a project with active security NFRs (its `substitution`/`reason` must be real sentences — the generic-value rejection applies). It is NOT a functional-requirement id: readers joining `technical_debt.fr_id` against `01_REQUIREMENTS.json` ids must treat `SEC-GATE` (and, defensively, any non-`FR-`/`NFR-`-prefixed value) as a project-level record, not a broken reference. `verify-run` suppresses its security-gap nudge when any debt entry mentions security (loose match — the canonical shape is the `SEC-GATE` id).
- `test_runner` is required (e.g. `"npm test"`, `"node --test tests/"`) — **except** in manual-verification mode (see below)
- **`test_runner_timeout_ms`** (optional, v2.0.0-rc.146+) — per-project ceiling in milliseconds for how long the automated test run (the `test_runner` command, and the auto-detected Playwright e2e run) may take before `verify-run` kills it as hung. Default `900000` (15 min). It is a **hang-catcher, not a speed target**: the failure is asymmetric — too-low kills a healthy-but-slow suite, too-high just delays killing a genuinely hung process — so the default is generous and a slow suite raises it. Since v2.2.0-rc.10 the runner is captured to a temp **file** rather than a pipe, which removes two kill causes that were Aitri's own: output volume (a pipe ceiling Node enforced by killing the child) and a background process the suite leaves running (with a pipe, `spawnSync` returns only when the LAST holder closes it, so a finished run was held open until this timeout and reported as killed, with its e2e TCs recorded `skipped`). A runner **killed** for any remaining reason (timeout, OS signal) is reported as "did not finish" and does **NOT** mutate `verifyPassed` or write `04_TEST_RESULTS.json` — a killed run is not a failing suite (distinct from a clean non-zero exit, which is a real test failure and IS persisted). If present, must be a positive number.
...
```json
{
  ...
  "e2e_output_incomplete": "true (optional, v2.2.0-rc.10+) — the e2e auto-run FINISHED but its output was not fully seen (past the read cap, or unreadable). Not a kill: e2e TCs whose result lines fell outside the captured window record as `skip`, never as `pass`, and still block verify-complete. Absent when the output was fully captured",
  "timings": "object (optional, v2.2.0-rc.10+) — wall time per dispatch stage in ms: { runner_ms?, e2e_ms?, gates_ms?, total_ms }. `e2e_ms` present only when the Playwright auto-run dispatched; per-gate cost lives in quality_gates[].duration_ms. Read-only cost visibility — nothing gates on it",
  "results": [
    {
      "tc_id": "TC-001h",
...
| `BUILD_PLAN.md` | the build agent (Phase 4 plan-first protocol, v2.0.0-rc.151+) | Present on fresh builds. A **working file, not a pipeline artifact**: epics (user-story groups, with FR/TC ids as derived references; "clusters" before v2.0.0-rc.170) + order + per-epic status the agent maintains during Phase 4 (the AUDIT_REPORT.md precedent — agent-authored markdown, no schema, nothing validates it). Readers may render it as build progress; do NOT treat it as a contract |
| `UX_PREVIEW.html` | the UX agent (advisory output of `run-phase ux`, v2.0.0-rc.169+) | Present when the UX phase ran AND the product has a graphical surface (skipped for CLI/TUI/library/service). A **working file, not a pipeline artifact** (the BUILD_PLAN.md class): one self-contained HTML page rendering the spec's Design Tokens (swatches, type scale, contrast badges) for the human at the approve gate. Agent-authored, no schema, nothing validates it, NOT hashed — may be stale after hand-edits to the spec. Readers may render or link it as a design preview; the design contract is `01_UX_SPEC.md`, never this file |
