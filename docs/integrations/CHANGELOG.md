...
- **New contract document [VALIDATE_JSON.md](./VALIDATE_JSON.md)** — the shape of `aitri validate --json` (per-artifact `exists/approved/drift/required` entries incl. the additive `absorbed`, `verifyPassed`, `resultsBinding` fields; `deployFiles`; `setupCommands`; the snapshot-derived `deployable`/`deployableReasons`/`openBugs`/`blockingBugs`) plus the `--ci` exit-code gate (v2.0.0-rc.117+). The emitter has always promised this shape to consumers ("legacy schema preserved") but its contract had no home — it was only recoverable from scattered CHANGELOG entries. No shape change; documentation of an existing surface. Enforced in `test/release-sync.test.js` (now five documents). README.md gains the surface rows for `validate --json` and `aitri export traceability`.
- **Code fix: `status --json` `nextActions[].severity` for the `artifact_missing` action is now `"critical"`** — rc.149 shipped `"blocker"`, a value outside the documented enum (`info | warn | critical`) that a consumer switching on the contract would drop or misrender. All emitters are now pinned to the enum by test. (Consumers that special-cased `"blocker"` in the two days it existed: match `"critical"`.)
- **SCHEMA.md corrections (doc-only, code unchanged):** the artifact-map key for the review phase is **`"review"`, not `"4r"`** (the doc's own `hasDrift` sample silently missed review-phase drift with the wrong key); `driftPhases` is `array<number|string>` on disk (core phases canonicalised to numbers — compare with `String(key)` as always); `verifyPassed` reset triggers completed (required quality-gate failure; reconcile entering pending; cascade) and the six-field cascade wipe documented; event payloads completed (`verify-run` carries `skipped`/`manual`; `reconcile-resolved`, `verify-spec-complete`, `layout_migrated`, `approve_preflight_autofix` payloads); `lastSession` writer list completed; `updatedAt` semantics, `.aitri.bak` branch nuance, transient `.aitri.lock` note.
- **ARTIFACTS.md corrections (doc-only, code unchanged):** the Phase-3 critical-flow minimum is **stack-aware since rc.62** (backend-only surfaces satisfy it with `integration` TCs — the doc still said "minimum 2 e2e"); per-requirement TC minimums bind **targeted NFR ids too**; `05_TRACEABILITY.json#requirement_compliance[].title` has **no Aitri consumer** (`export traceability` titles come from `01_REQUIREMENTS.json` — the doc claimed otherwise); Phase-2 "minimum 3 risks" is template-taught, not gated; the `complete 5` run-binding tamper refusal (B3) and the string-path guard on `files_created`/`files_modified` documented; BUGS.json gains the B8 evidence contract (blocking-bug `fix`/`close` require `--resolution`, `verify` refuses never-fixed, malformed file refuses all gates) — shipped rc.149, previously only in this changelog; `detected_by` documented as an open set (only `manual`/`verify-run`/`playwright` are written today); Playwright evidence falls back to `*.webm`; killed-at-timeout gates are `status: "error"`; `manual_verified` counts every `verified_manually` result (incl. `--evidence`-recorded automated TCs); `tc verify` recomputes `ac_coverage` too.
- **STATUS_JSON.md corrections (doc-only):** priority-2 row includes the `artifact_missing` trigger; priority-4 reconcile actions are suppressed while blocking bugs are open; non-zero exit also fires on an unreadable root `.aitri`; optional phases appear in `phases[]` with string keys; `verifySummary` provenance precise (`.aitri#verifySummary`, not re-read from disk).
- Also restored the `## v2.0.0-rc.151` heading below (the rc.154 commit accidentally replaced it instead of inserting above; the BUILD_PLAN.md entry had been left without its own heading).

Nothing previously documented changed shape; readers that implemented against the previous text should re-check the SCHEMA.md artifact-map key (`"4r"` → `"review"`) and the `driftPhases` element type — both were doc errors, not code changes.
