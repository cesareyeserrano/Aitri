...
| `verifyRanRef` | `string\|absent` | absent | **v2.2.0-rc.7+ (additive, ADR-085).** The commit SHA `verify-run` executed at (git projects only; absent on non-git projects and on stamps made by older CLIs). Binds the verify verdict to the code tree it certified — the snapshot derives `verify.refState` from it (`fresh` when no behavioral file changed between this ref and HEAD; `stale`/`unreachable` block the deploy gate and `verify-complete`). Shared field: the binding travels with every clone/worktree |
| `verifyRanDirty` | `boolean\|absent` | absent | **v2.2.0-rc.7+ (additive, ADR-085).** `true` when `verify-run` executed over uncommitted behavioral changes — the verdict is machine-local/non-portable (`refState: "dirty"`, warn surface, not deploy-blocking). Cleared/re-stamped on every run |
