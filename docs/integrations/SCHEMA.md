...
| `verifyRanRef` | `string\|absent` | absent | **v2.2.0-rc.7+ (additive, ADR-085).** The commit SHA HEAD pointed at when `verify-run` STARTED — read before any dispatch (v2.2.0-rc.15+, ADR-085 Addendum 2; older CLIs read it after the runner/e2e/gates, so a commit made mid-run could be stamped as certified) (git projects only; absent on non-git projects and on stamps made by older CLIs). Binds the verify verdict to the code tree it certified — the snapshot derives `verify.refState` from it (`fresh` when no behavioral file changed between this ref and HEAD; `stale`/`unreachable` block the deploy gate and `verify-complete`). Shared field: the binding travels with every clone/worktree |
| `verifyRanDirty` | `boolean\|absent` | absent | **v2.2.0-rc.7+ (additive, ADR-085).** `true` when `verify-run` executed over uncommitted behavioral changes — present at the START or at the END of the run, or git unreadable at one of them (v2.2.0-rc.15+; older CLIs checked the end only) — the verdict is machine-local/non-portable (`refState: "dirty"`, warn surface, not deploy-blocking). Cleared/re-stamped on every run |
...
| `coverageAuditReqHash` | `string` (sha256 hex) | `null` | Content hash of `01_REQUIREMENTS.json` at the last `aitri audit requirements` invocation. Freshness signal for the requirements coverage audit that is keyed to the requirements *version*, not a timestamp — survives git clone (mtime resets) and is not faked by the other audit types that share `AUDIT_REPORT.md`. "Fresh" = this hash equals the current requirements hash AND a report file exists. Read-only for subproducts (v2.0.0-rc.154+, UPLAN-0703 D5) |
...
| `sessionContext` | `object\|null` | `null` | Durable narrative thread (the "what/why/next"). Set by `aitri checkpoint --context`; SURVIVES later state transitions (unlike `lastSession.context`). Schema: `{ text: string, at: "ISO", stateAtSave?: { activeBugs: N, fixedBugs: N, openBacklog: N } }`. Per-machine — the personal thread; cross-dev action traceability is in shared `events[]`. `aitri resume` shows it and flags staleness/absence (v2.0.0-rc.60+). `stateAtSave` (v2.2.0-rc.14+, optional): the snapshot counts the narrative was written against; `resume` diffs them against the live snapshot and flags the text as stale by content. Display-only; the narrative is agent-written and never a gate input |
| `reconcileState` | `object\|null` | `null` | Off-pipeline change baseline. Set on `approve 4`, by `aitri reconcile`, and by `aitri reconcile --resolve`. Schema: `{ status: "pending" \| "resolved", baseRef: "<git-sha>" \| "<ISO>", method: "git" \| "mtime", lastRun: "ISO", pendingFiles?: string[] }` (v0.1.80+; `--resolve` flag added v0.1.84). `pendingFiles` (additive, v2.0.0-rc.158+) is the sorted file set recorded when reconcile entered/refreshed pending — NEW files on a later re-run are treated as a fresh transition (they re-clear `verifyPassed`, closing the growing-drift stale-proof window at the SET level; new content inside an already-pending file is not detected — recorded residual); absent on pre-rc.158 pending states (backfilled without clearing on the next run) and dropped when the state returns to `resolved`. Since v2.2.0-rc.11 entries are git pathspecs FROM THE PROJECT DIR: project-relative inside the subtree, `:/`-anchored repo-relative outside it; the additive sibling `pendingFilesFrame: "pipeline"` marks a set written in that frame (an unmarked set predates rc.11 and is trusted for growth detection only at the repo root, where the frames coincide) |
...
Read `.aitri` (shared) exactly as before — the path and the shared fields are unchanged. **Never depend on `.aitri.local` as shared truth** — it is per-machine and gitignored, and its fields (`lastSession`, `sessionContext`, `reconcileState`) are not in `.aitri`; a clone or another machine will not have it. Scoped exception (2026-07-05, Hub catch-up): a consumer running ON the same machine MAY read that machine's own `.aitri.local` for **display-only** enrichment (e.g. last session), if and only if absence and malformed content degrade silently — the file is optional context, never required input. A project may still legitimately gitignore the whole `.aitri` (the owner's choice); if `.aitri` is absent or its `updatedAt` is older than the project's last git commit, treat that as "state is per-machine for this project," not a corruption signal.
