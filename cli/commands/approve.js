...
export async function runApproveCommand({
  ...
  // Semantic gate: output arch-consistency prompt for agent review (if artifact exists)
  const archDecisionPath = path.join(process.cwd(), ".aitri/architecture-decision.md");
  if (fs.existsSync(archDecisionPath) && !options.nonInteractive) {
    ...
    const archContext = personaResult.ok ? `## Persona\n${personaResult.systemPrompt}\n\n` : "";
    console.log("\n--- AGENT TASK: arch-consistency check ---");
    console.log(archContext + `Review this feature spec for consistency with the architecture decision.
...
${archSnippet}
--- END TASK ---`);
    console.log("\nReview any concerns above before confirming approval.");
  }
