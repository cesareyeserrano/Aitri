...
const VALID_REQUIREMENTS = JSON.stringify({
  ...
  coverage_map: [
    ...
    { need: 'export data to CSV', disposition: 'FR-003' },
    { need: 'persist data across restarts', disposition: 'FR-004' },
...
describe('Aitri CLI — run-phase smoke', () => {
  ...
  const DESIGN_MD = [
    ...
    'FR-001: Create invoice. Method: validated insert + status transition table. I/O: {client_id,amount,due_date} → {id,status:"draft"}. Failure: 422 on invalid amount.',
    '',
    '## Security Design',
...
describe('Aitri CLI — review smoke', () => {
  ...
  const REVIEW_REQUIREMENTS = JSON.stringify({
    ...
    coverage_map: [
      { need: 'log in', disposition: 'FR-001' },
      { need: 'see a dashboard', disposition: 'FR-002' },
      { need: 'export CSV', disposition: 'FR-003' },
