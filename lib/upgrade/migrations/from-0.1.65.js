...
// rewrite the manifest — the operator changes the runner to an absolute path
// or a PATH-resolved binary. Auto-resolving would violate ADR-027 §2 (shape
// transforms only) and is fragile across venv layouts (.venv, venv, ~/venvs,
// poetry, pipenv). A `--cwd` flag was rejected: it puts the burden on every
// verify-run invocation and would undo the alpha.9 scoping fix.
//
...
function buildVenvFinding(target, runner) {
  return {
    ...
    autoMigratable: false,
    reason:         'Pre-alpha.9 manifests stored runner paths relative to the project root. Since alpha.9, `aitri feature verify-run` runs from the feature directory, so the binary is no longer reachable and fails with "Command not found" before any test executes. Edit `04_IMPLEMENTATION_MANIFEST.json::test_runner` to use an absolute path (e.g. `/abs/.venv/bin/pytest`) or a PATH-resolved binary (e.g. `pytest`).',
  };
}
...
// ── BLOCKING: orphan IDEA.md absorption (alpha.17) ───────────────────────────
//
...
// subsequent `adopt --upgrade` because no migrator touches it. Operator-
// visible symptom: a stale brief lingering at the project root after
// multiple alpha bumps.
//
// Auto-migratable when (Phase 1 approved + IDEA.md exists + original_brief
// absent in the artifact). Shape-only: same content, file → field; nothing
// semantic. Re-stamps artifactHashes[1] to match the post-archive artifact
// so `status` does not flag drift on the very next call.
//
// Flagged (autoMigratable: false) when original_brief is already populated
// — overwriting it would clobber a non-empty field. Operator decides whether
// the on-disk IDEA.md or the archived brief is authoritative; Aitri does
// not guess.
//
// Cost shape: the function short-circuits on `fs.existsSync('IDEA.md')`
// before any JSON parse. After the first successful migration the file is
// gone, so subsequent upgrades pay one stat() and return immediately.

// Pre-flight scan helper for the orphan IDEA migration. Walks the root
// pipeline artifacts plus every features/<x>/spec/* and reports any file
// whose content matches /\bIDEA\.md\b/. 01_REQUIREMENTS.json is skipped at
// every level: post-absorption it legitimately holds IDEA.md content inside
// `original_brief`, and a substring match there is the absorbed text — not
// a stale path reference.
//
// Read-only across feature directories: scanning does not violate A2 (root
// upgrade does not cascade); only mutations to features/*/.aitri would.
function findIdeaPathReferences(dir, config) {
  const PATTERN = /\bIDEA\.md\b/;
  const SKIP_BASENAMES = new Set(['01_REQUIREMENTS.json']);
  const refs = [];

  function scanFile(absPath) {
    if (!absPath || !fs.existsSync(absPath)) return;
    if (SKIP_BASENAMES.has(path.basename(absPath))) return;
    let content;
    try { content = fs.readFileSync(absPath, 'utf8'); }
    catch { return; }
    if (PATTERN.test(content)) refs.push(path.relative(dir, absPath));
  }
  ...
  for (const n of rootNames) scanFile(artifactPath(dir, config, n));

  const featuresDir = path.join(dir, 'features');
  let featureEntries = [];
  try {
    if (fs.existsSync(featuresDir) && fs.statSync(featuresDir).isDirectory()) {
      featureEntries = fs.readdirSync(featuresDir, { withFileTypes: true });
    }
  } catch { /* unreadable features/ — leave empty */ }
  for (const e of featureEntries) {
    if (!e.isDirectory()) continue;
    const featSpecDir = path.join(featuresDir, e.name, 'spec');
    if (!fs.existsSync(featSpecDir)) continue;
    let featFiles = [];
    try { featFiles = fs.readdirSync(featSpecDir); }
    catch { continue; }
    for (const fn of featFiles) {
      if (!/\.(md|json)$/.test(fn)) continue;
      scanFile(path.join(featSpecDir, fn));
    }
  }
...
function diagnoseOrphanIdea(dir, config) {
  ...
  // STALE-REF DETECTION (post-absorption breakage). Triggers when IDEA.md
  // is gone (already absorbed by a prior alpha.17+ upgrade) but downstream
  // artifacts still grep IDEA.md as a literal path. Surfaces invisibly-
  // broken projects (e.g. tests written when IDEA.md was still on disk).
  if (!ideaExists) {
    const refs = findIdeaPathReferences(dir, config);
    if (refs.length === 0) return [];
    return [{
      category:       'validatorGap',
      target:         'IDEA.md (absorbed)',
      transform:      `${refs.length} downstream artifact(s) still reference IDEA.md as a path`,
      autoMigratable: false,
      reason:         `IDEA.md was absorbed into 01_REQUIREMENTS.json#original_brief in a prior upgrade and no longer exists at the project root, but ${refs.length} artifact(s) still reference 'IDEA.md' as a literal path: ${refs.join(', ')}. Tests, manifest paths, or verification commands grepping IDEA.md will fail. Update those references to read from 01_REQUIREMENTS.json#original_brief, or rewrite them to a different documentation file the project owns.`,
    }];
  }
  ...
  // PRE-FLIGHT REFERENCE SCAN. If any artifact references IDEA.md as a path,
  // unlinking the file would break those references silently (tests grepping
  // IDEA.md, manifest deliverables registered at "path": "IDEA.md", etc.).
  // Block auto-absorption and let the operator update the references first.
  const refs = findIdeaPathReferences(dir, config);
  if (refs.length > 0) {
    return [{
      category:       'validatorGap',
      target:         'IDEA.md',
      transform:      `absorption blocked: ${refs.length} artifact(s) reference IDEA.md as a path`,
      autoMigratable: false,
      reason:         `IDEA.md is referenced as a literal path in ${refs.length} downstream artifact(s): ${refs.join(', ')}. Unlinking IDEA.md would break those references (tests grepping the file, manifest deliverables, verification commands). Update or remove those references first, then re-run aitri adopt --upgrade. To accept the breakage and proceed, delete IDEA.md manually before upgrading.`,
    }];
  }
  ...
  return [{
    ...
    afterHash,
    apply: (cfg) => {
      fs.writeFileSync(reqFull, afterContent, 'utf8');
      fs.unlinkSync(ideaPath);
      updatePhaseHashIfApproved(cfg, '1', afterHash);
    },
  }];
}
