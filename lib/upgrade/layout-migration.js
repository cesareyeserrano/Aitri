...
export function planLayoutMigration(dir) {
  ...
  // Archived seeds (rc.80): file-level moves — the bare `archive/` DIR is not
  // moved wholesale because the name is generic enough to be user-owned.
  // Includes the timestamp-suffixed variants re-seeded projects produce
  // (IDEA-<stamp>.md — adversarial review, rc.82).
  try {
    for (const f of fs.readdirSync(path.join(dir, 'archive'))) {
      if (/^IDEA(-.*)?\.md$/.test(f)) addIfExists(`archive/${f}`, `aitri/product/archive/${f}`);
    }
  } catch { /* no archive/ dir — nothing to carry */ }
  addIfExists('BACKLOG.md', 'aitri/BACKLOG.md');
  // features/ is claimed by OWNERSHIP MARKER, not by name (rc.82): a repo can
  // have its own features/ (e.g. Cucumber specs). Aitri feature units carry a
  // .aitri inside — move the collection only when at least one is present.
  const featuresAbs = path.join(dir, 'features');
  if (fs.existsSync(featuresAbs)) {
    let hasAitriFeature = false;
    try {
      hasAitriFeature = fs.readdirSync(featuresAbs, { withFileTypes: true })
        .some(e => e.isDirectory() && fs.existsSync(path.join(featuresAbs, e.name, '.aitri')));
    } catch { /* unreadable — leave it alone */ }
    if (hasAitriFeature) {
      addIfExists('features', 'aitri/features');
    } else {
      notes.push('A features/ directory exists but contains NO Aitri feature units (no features/<name>/.aitri) — it is NOT moved; it looks user-owned (e.g. BDD specs).');
    }
  }
  ...
  // Gitignored files under the moved paths travel with the move but git CANNOT
  // restore them (`git checkout` only restores tracked files) — the recovery
  // command would delete the only copy. Surface them so the operator knows.
  try {
    const ignored = execSync('git ls-files --others --ignored --exclude-standard', {
      cwd: dir, stdio: ['ignore', 'pipe', 'ignore'],
    }).toString().trim().split('\n').filter(Boolean);
    const sources = moves.map(m => m.from.replace(/\\/g, '/'));
    const affected = ignored.filter(f =>
      sources.some(s => f === s || f.startsWith(s + '/')));
    if (affected.length) {
      notes.push(
        `${affected.length} gitignored file(s) live under the moved paths (e.g. ${affected.slice(0, 3).join(', ')}` +
        `${affected.length > 3 ? ', …' : ''}). They move along fine, but git CANNOT restore them — ` +
        `if you ever revert with \`rm -rf aitri/\`, move them back out first.`
      );
    }
  } catch { /* git hiccup — the clean-tree gate already passed; skip the note */ }
  ...
  notes.push('Aitri-GENERATED agent instruction files (those carrying the "Aitri Pipeline Rules" header) will be regenerated with layout-correct paths. User-authored agent files (e.g. your own pre-Aitri CLAUDE.md) are KEPT untouched and reported — reconcile them yourself.');
...
export function executeLayoutMigration(dir, rootDir) {
  ...
  const { regenerated, kept } = regenerateAgentFiles(dir, rootDir, config);

  return { moved, agentFiles: regenerated, keptAgentFiles: kept };
}
