...
function printReport({ config, prevVer, VERSION, inferred, skipped, dir, migrationResult, dryRun = false }) {
  ...
  //                        as-is (rejected, in-progress). Must be surfaced even
  //                        on otherwise no-op runs so the operator can act.
  const PRESERVED_REASONS = new Set(['rejected, not auto-completed', 'in progress, not auto-completed']);
  ...
  if (preservedSkipped.length) {
    ...
    console.log(`\n  These phases were NOT auto-completed. Operator intent (rejection or`);
    console.log(`  in-progress work) takes precedence — re-run the phase or run`);
    console.log(`  aitri complete <phase>  /  aitri approve <phase>  deliberately.`);
  }
