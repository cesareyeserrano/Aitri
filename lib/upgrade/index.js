...
import { loadConfig, saveConfig, artifactPath, ideaContextSubdir } from '../state.js';
...
import { featuresIndexIgnoreEntry } from '../features-index.js';
 ...
 */
// Compare Aitri version strings X.Y.Z[-(alpha|rc).N]. Returns -1 (a<b), 0 (equal/unparseable),
// 1 (a>b). Precedence: stable (no pre-release) > rc > alpha; within a pre-release, higher N is
// newer. A single anchored regex (not a fragile parser) — used to refuse a downgrade (ADV-0622-13).
export function compareVersions(a, b) {
  const parse = (v) => {
    const m = /^(\d+)\.(\d+)\.(\d+)(?:-(alpha|rc)\.(\d+))?$/.exec(String(v ?? '').trim());
    if (!m) return null;
    return [Number(m[1]), Number(m[2]), Number(m[3]), m[4] ? (m[4] === 'alpha' ? 0 : 1) : 2, m[5] ? Number(m[5]) : 0];
  };
  const pa = parse(a), pb = parse(b);
  if (!pa || !pb) return 0;
  for (let i = 0; i < pa.length; i++) if (pa[i] !== pb[i]) return pa[i] < pb[i] ? -1 : 1;
  return 0;
}
...
export function runUpgrade({ dir, VERSION, rootDir, dryRun = false }) {
  ...
  // ADV-0622-13: never silently downgrade. If the installed CLI is OLDER than the version this
  // project was last touched with, running forward migrations + rewriting aitriVersion DOWN would
  // corrupt the recorded version and flip-flop a committed .aitri between teammates. Refuse.
  if (config.aitriVersion && compareVersions(VERSION, config.aitriVersion) < 0) {
    process.stderr.write(
      `⚠️  Aitri CLI v${VERSION} is OLDER than this project's recorded version (v${config.aitriVersion}).\n` +
      `   Refusing to downgrade — upgrade the CLI instead of rewriting the project version down.\n`
    );
    return;
  }
  ...
  if (!dryRun) {
    ...
    // ADR-045: saveConfig above auto-migrated the file (per-machine fields → .aitri.local).
    // Fix the project's .gitignore to match: stop ignoring the now-shared .aitri, ignore the
    // per-machine sibling. Surgical + idempotent — and surfaced below, since it touches the
    // user's file.
    gitignoreFix = ensureAitriGitignore(dir, { extra: [featuresIndexIgnoreEntry(config)] });
  }

  // REPORT.
  printReport({ config, prevVer, VERSION, inferred, skipped, dir, migrationResult, dryRun, gitignoreFix });

  // Gate re-check (TPA-6): a previously-approved artifact can become invalid when
  // gates evolve across versions (e.g. an rc.34 test plan rejected by rc.52's
  // stricter ac_id rules). adopt --upgrade migrates STATE but does not re-run the
  // artifact validators, so the breakage used to surface only later at the next
  // `complete --check`. Surface it now, at upgrade — advisory, approvals unchanged.
  const gateFailures = dryValidateApproved(dir, config);
  if (gateFailures.length) {
    process.stderr.write(
      `\n⚠ Gate re-check — ${gateFailures.length} approved artifact(s) would be REJECTED by the current gates:\n`
    );
    for (const f of gateFailures) {
      const msg = String(f.error).split('\n').map(l => `      ${l}`).join('\n');
      process.stderr.write(`  ✗ Phase ${f.key} (${f.artifact}):\n${msg}\n`);
    }
    process.stderr.write(
      `  These were valid under the previous version. Your approvals are UNCHANGED — the gate runs\n` +
      `  at the next \`complete\`. To fix now: aitri run-phase <phase> → edit → complete → approve.\n`
    );
  }

  // Legacy idea/ folder (TPA-3): renamed to idea_context/ and no longer scanned. A
  // project migrated from an older layout may still hold assets there that every
  // briefing now silently ignores — surface it at upgrade (the earliest point an
  // existing project re-enters Aitri), not only later at run-phase.
  const legacyIdea = listAssets(dir, 'idea');
  if (legacyIdea.length) {
    const ctxRel = ideaContextSubdir(config).replace(/\\/g, '/');
    process.stderr.write(
      `\n⚠ ${legacyIdea.length} asset(s) found in idea/ — that folder is no longer scanned ` +
      `(renamed to ${ctxRel}/), so they are absent from every briefing.\n` +
      `  Move them so they are ingested:  mv idea/* ${ctxRel}/\n`
    );
  }
...
/**
 * Dry-run the current artifact validators against already-APPROVED phases.
 * Read-only: never mutates config or approvals. Returns the phases whose sealed
 * artifact the current `PHASE_DEFS[*].validate()` would now reject — the §4.3
 * "gate evolution silently invalidated a valid artifact" case the adopter hit.
 * A missing artifact is skipped (STATE-MISSING handles that separately).
 *
 * @returns {{ phase: (number|string), key: string, artifact: string, error: string }[]}
 */
function dryValidateApproved(dir, config) {
  const failures = [];
  for (const phaseKey of (config.approvedPhases || [])) {
    const p = PHASE_DEFS[phaseKey];
    if (!p?.validate || !p.artifact) continue;
    const artPath = artifactPath(dir, config, p.artifact);
    if (!fs.existsSync(artPath)) continue;
    let content;
    try { content = fs.readFileSync(artPath, 'utf8'); } catch { continue; }
    try {
      p.validate(content, { dir, config });
    } catch (e) {
      failures.push({ phase: phaseKey, key: p.alias || phaseKey, artifact: p.artifact, error: e.message });
    }
  }
  return failures;
}
...
function isInProgress(events, phaseNum) {
  ...
  for (const e of events) {
    if (!e) continue;   // AUDIT-0629-D: tolerate a null/non-object element in a hand-edited events[]
...
function hasNoEventHistory(events, phaseNum) {
  if (!Array.isArray(events) || events.length === 0) return false;
  const target = String(phaseNum);
  return !events.some(e => e && String(e.phase) === target);   // AUDIT-0629-D: null-element tolerant
}
...
function printReport({ config, prevVer, VERSION, inferred, skipped, dir, migrationResult, dryRun = false, gitignoreFix = null }) {
  ...
  if (migrated.length) {
    ...
    // Z2 backfill caveat — surface the silent assumption that current on-disk
    // state IS the approved baseline. If the operator modified an artifact
    // post-approval but pre-upgrade, the backfill locks that change in as
    // approved, and `aitri status` will show no drift. There is no prior
    // baseline to compare against — the lock-in is correct given missing
    // information, but the operator should know.
    const hasArtifactHashesBackfill = migrated.some(f =>
      typeof f.target === 'string' && f.target.startsWith('.aitri#artifactHashes')
    );
    if (hasArtifactHashesBackfill) {
      const verb = dryRun ? 'would be stamped' : 'were stamped';
      console.log(`\n  ⚠️  Note: artifactHashes ${verb} from current on-disk state.`);
      console.log(`     If you modified an artifact after approval but before this upgrade, that change`);
      console.log(`     is now the approved baseline (no drift visible). Compare git history if unsure.`);
    }
  }
  ...
  if (gitignoreFix && (gitignoreFix.removedLegacy || gitignoreFix.added.length)) {
    console.log(`\n  ⚙️  .gitignore updated for the .aitri split (ADR-045):`);
    if (gitignoreFix.removedLegacy)
      console.log(`      • removed the legacy bare \`.aitri\` ignore — the shared .aitri is now COMMITTED`);
    if (gitignoreFix.added.length)
      console.log(`      • now ignoring per-machine state: ${gitignoreFix.added.join(', ')}`);
    console.log(`      Commit .aitri so teammates get approvals + the drift baseline; .aitri.local stays per-machine.`);
  }
...
function printAgentFilesGuidance(created) {
  ...
  if (created.includes('.claude/commands/aitri.md')) {
    console.log(`  .claude/commands/aitri.md is a Claude Code slash command: type /aitri to`);
    console.log(`  advance the pipeline one step.`);
  }
}
