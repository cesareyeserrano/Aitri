...
/** Build the root-frame context for judging out-of-subtree paths. */
...
function isBehavioralInFrame(repoRel, pipelinePrefix, config, rootCtx) {
...
/** Unquote a porcelain path (C-quoted when it contains spaces/special chars) — parity
 *  with reconcile's porcelain reader, the declared SSoT for this parsing. */
...
export function captureVerifyRef(dir, config, rootCtx = null) {
  try {
    ...
    // --untracked-files=all: an untracked DIRECTORY otherwise collapses to `?? dir/`,
    // hiding whether its contents are behavioral (reconcile parity).
    ...
    const dirty = porcelain.split('\n').some(line => {
      ...
      if (!(code === '??' || /[AMRC]/.test(code))) return false;  // pure deletions/unmerged: skip
      let p = line.slice(3);
      const arrow = p.indexOf(' -> ');
      if (arrow !== -1) p = p.slice(arrow + 4);                    // rename/copy → new path
      p = unquotePorcelainPath(p);
      return isBehavioralInFrame(p, prefix, config || {}, rootCtx);
    });
    return { ref, dirty };
  } catch {
...
export function verifyRefFreshness(pipelineDir, config, rootCtx = null) {
  ...
  try {
    ...
    const out = execFileSync(
      'git', ['diff', `${ref}..HEAD`, '--name-only', '--diff-filter=ACMRD'],
      { cwd: pipelineDir, stdio: ['ignore', 'pipe', 'ignore'] }
    ).toString().trim();
    const behavioral = out
