...
export function buildPipelineEntry(pipelineDir, scope) {
  ...
  return {
    ...
    phases,
    verify: {
      ran:     !!config.verifyPassed || !!config.verifySummary,
      passed:  !!config.verifyPassed,
      summary: config.verifySummary || null,
      // Fase 1 gap: verify timestamp is not persisted in .aitri. A future
      // phase may add `verifyRanAt`; until then `ranAt` stays null and
      // `tests.stalenessDays` is null in the aggregate.
      ranAt: null,
    },
...
function aggregateTests(pipelines) {
  const byPipeline = {};
  ...
  for (const pl of pipelines) {
    ...
    if (pl.verify.summary) {
      totalPassing += pl.verify.summary.passed || 0;
      totalFailing += pl.verify.summary.failed || 0;
    }
  }
  ...
  return {
    ...
    stalenessDays: null, // Fase 1 gap — see buildPipelineEntry note on verify.ranAt
  };
}
...
function computeAudit(rootPipeline, now) {
  ...
  let mtimeMs = null;
  try { mtimeMs = fs.statSync(auditPath).mtimeMs; } catch { /* unreadable */ }
  return {
    exists:        true,
    path:          auditPath,
    lastAt:        mtimeMs ? new Date(mtimeMs).toISOString() : null,
    stalenessDays: daysSince(mtimeMs, now),
  };
}
...
function computeHealth({ project, pipelines, bugs, audit }) {
  ...
  return {
    ...
    staleVerify:      [], // Fase 1 gap — requires verifyRanAt timestamp
    staleAudit,
    driftPresent,
...
function computeNextActions(snapshot) {
  const actions = [];
  const { project, pipelines, bugs, audit, health } = snapshot;
  const root = pipelines.find(p => p.scopeType === 'root');
...
export function buildProjectSnapshot(dir, opts = {}) {
  ...
  const tests        = aggregateTests(pipelines);
