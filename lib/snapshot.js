...
import { loadConfig, readArtifact, artifactPath, hasDrift, featuresDir as resolveFeaturesDir } from './state.js';
...
function artifactGitClean(pipelinePath, artifactsDir, artifactFile) {
  ...
  try {
    ...
    execSync(
      `git ls-files --error-unmatch -- ${JSON.stringify(rel)}`,
      { cwd: pipelinePath, stdio: ['ignore', 'ignore', 'ignore'] },
    );
    const out = execSync(
      `git diff HEAD -- ${JSON.stringify(rel)}`,
...
function nextPhaseAction(pl) {
  ...
  if (phase4 && phase4.status === 'approved' && !pl.verify.passed) {
    ...
    // Root only: `tc verify` is a root command; a feature-scoped manual-verify path
    // does not exist yet, so a feature pipeline still routes to verify-run below.
    if (pl.scopeType === 'root' && last && last.passed === 0 && last.failed === 0 && last.skipped === 0 && last.manual > 0) {
      return {
        command: 'aitri tc verify',
...
function computeNextActions(snapshot) {
  ...
  for (const pl of pipelines) {
    ...
    // ordinary phase work is priority 6
    const isVerify = next.command.includes('verify-run') || next.command.includes('verify-complete') || next.command.includes('tc verify');
    actions.push({
