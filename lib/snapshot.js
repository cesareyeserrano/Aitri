...
// Path-frame SSoT (rc.11): git speaks repo-relative, Aitri's filters speak pipeline-relative.
// The raw isBehavioralFile/isAitriStatePath predicates are reached THROUGH isBehavioralInFrame
// so no reader here can filter an unframed path again.
import { gitShowPrefix, isBehavioralInFrame, verifyRefRootCtx,
 ...
 */
export function buildPipelineEntry(pipelineDir, scope, { tolerateConflict = false, refRootCtx = null } = {}) {
  ...
  return {
    ...
    verify: {
      ...
      // M1 (ADR-085): the verdict's binding to the code tree it certified.
      // 'unbound' is free (no git call); the git probes run only when a stamp exists.
      refState:       verifyRefFreshness(pipelineDir, config, refRootCtx),
    },
...
function discoverFeaturePipelines(rootDir) {
  ...
  const featuresDir = resolveFeaturesDir(rootDir, rootConfig);
  if (!fs.existsSync(featuresDir)) return [];
  // Root-frame context for feature refState: out-of-subtree paths (root code seen from
  // a feature dir) are judged with the ROOT's artifactsDir/layoutRoot filters.
  const refRootCtx = verifyRefRootCtx(rootDir, rootConfig);
  ...
  for (const e of entries) {
    ...
    const entry = buildPipelineEntry(featureDir, `feature:${e.name}`, { tolerateConflict: true, refRootCtx });
...
function aggregateBugs(pipelines) {
  ...
  let open     = 0;
  let blocking = 0;
  // BUG-STATE-VISIBLE-0909: `open` keeps its historical meaning (open|in_progress|fixed —
  // the Hub contract). `active` (open|in_progress) and `fixed` split it so a reader can
  // tell "still broken" from "fixed, awaiting verification" — a project with 9 fixed bugs
  // and 2 open read as "11 open" in resume/status/validate.
  let active   = 0;
  let fixed    = 0;
  ...
  openIds.sort();
  return { total, open, active, fixed, blocking, bySeverity, openIds, byPipeline, openByPipeline, blockingByPipeline, list, parseErrors };
}
...
export function detectUncountedChanges(rootPipeline) {
  ...
  try {
    ...
    // Frame-normalized (rc.11): git emits repo-relative paths. When the project root is a
    // repo subdir, the raw filter counted Aitri's OWN `.aitri`/artifacts as project drift
    // and this counter never reached zero — the livelock ADR-085's M1 header describes.
    // gitDiffNames runs `-z` so a non-ASCII path arrives raw, not C-quoted.
    const prefix = gitShowPrefix(rootPipeline.path) || '';
    const files = gitDiffNames(rootPipeline.path, `${baseline.baseRef}..HEAD`, { diffFilter: 'ACMR' })
      .filter(f => isBehavioralInFrame(f, prefix, rootPipeline, null));
...
// ── Verify ref binding (M1, MULTI-TEAM-0722 / ADR-085) ───────────────────────
//
// The verify verdict is bound to the code state it certified: `verify-run` stamps
// `verifyRanRef` (HEAD) + `verifyRanDirty` (uncommitted behavioral files at run time)
// as SHARED fields, so the binding travels with every clone/worktree — unlike the
// per-machine reconcile baseline, which a fresh checkout auto-stamps at current HEAD,
// silently burying a merge window (reproduced: study doc R3). Doctrine = ADR-069
// run-binding extended from the results FILE to the code TREE: raises the cost of a
// stale pass; does not prevent a deliberate `.aitri` edit.

/**
 * Path-frame normalization (rc.7 adversarial BLOCKER): git emits paths relative to the
 * REPO root, but `isAitriStatePath` matches prefixes relative to the PIPELINE root
 * (artifactsDir, layoutRoot, features/). Whenever the two frames differ — a contained-
 * layout feature (`aitri/features/f1/…`, the DEFAULT layout) or a monorepo whose
 * project root is a repo subdir — the pipeline's own artifacts classified as
 * behavioral: every feature verify-run stamped dirty, and committing the artifacts
 * (the git-mediated handoff this milestone exists for) made the verdict permanently
 * 'stale' — a livelock against verify-complete's refusal. `pipelinePrefix` is the
 * pipeline dir's repo-relative prefix (`git rev-parse --show-prefix`); paths inside it
 * are re-framed before filtering. Paths OUTSIDE the subtree (root code seen from a
 * feature) are judged in the ROOT's frame when a rootCtx is provided, else by the
 * frame-independent filters only (conservative: real code still counts). We do NOT use
 * `git diff --relative`: it silently drops out-of-subtree paths, which would blind a
 * feature's refState to parent-code changes.
 */
// gitShowPrefix / verifyRefRootCtx / isBehavioralInFrame / unquotePorcelainPath now live in
// lib/git-frame.js — the SSoT shared with reconcile's three readers (rc.11).
export { verifyRefRootCtx };

/**
 * Capture the ref stamp at verify-run time. Returns { ref, dirty } for git projects,
 * or null (no stamp — non-git projects degrade to pre-M1 behavior, documented).
 * `dirty` = uncommitted changes in behavioral paths (same allowlist + porcelain
 * parsing as reconcile — docs/lockfile churn does not make a verdict non-portable).
 */
export function captureVerifyRef(dir, config, rootCtx = null) {
  try {
    ...
    // Shared `-z` porcelain reader (reconcile parity by construction — one fetch + parser).
    const dirty = gitPorcelainPaths(dir)
      .some(p => isBehavioralInFrame(p, prefix, config || {}, rootCtx));
    return { ref, dirty };
  } catch {
    return null;   // not a git repo / git unavailable — no stamp
  }
}

/**
 * Freshness of a verify verdict relative to the current HEAD.
 *   'unbound'     — no stamp (non-git, or verify predates M1). No gate effect: degrades
 *                   to pre-M1 behavior rather than blocking projects that cannot stamp.
 *   'dirty'       — stamped over uncommitted behavioral code: the verdict is machine-
 *                   local and non-portable (warn surface, not a deploy block).
 *   'unreachable' — the stamped commit is not in this history (rebase/squash/shallow):
 *                   refuse-not-pass (B4 doctrine) — never treat as fresh.
 *   'stale'       — behavioral files changed between the stamp and HEAD (merge, or any
 *                   post-verify commit): the verdict certifies a tree that is not this
 *                   one. Deploy-blocking.
 *   'fresh'       — no behavioral diff between the stamp and HEAD (docs-only commits
 *                   stay fresh — same allowlist as reconcile).
 */
export function verifyRefFreshness(pipelineDir, config, rootCtx = null) {
  const ref = config.verifyRanRef;
  if (typeof ref !== 'string' || !/^[0-9a-f]{40}$/.test(ref)) return 'unbound';
  try {
    execFileSync('git', ['cat-file', '-e', `${ref}^{commit}`],
      { cwd: pipelineDir, stdio: ['ignore', 'ignore', 'ignore'] });
  } catch {
    return 'unreachable';
  }
  try {
    const prefix = gitShowPrefix(pipelineDir) || '';
    const behavioral = gitDiffNames(pipelineDir, `${ref}..HEAD`, { diffFilter: 'ACMRD' })
      .filter(f => isBehavioralInFrame(f, prefix, config, rootCtx));
    // Order matters (adversarial find, pre-commit): a behavioral diff since the stamp
    // outranks the dirty flag — otherwise a dirty-stamped verdict would ride 'dirty'
    // (warn-only) forever while behavioral commits pile up, and deployable would read
    // Ready on never-verified code: the exact hole this field exists to close. 'dirty'
    // is only the answer while the certified commit is still HEAD-equivalent.
    if (behavioral.length) return 'stale';
    return config.verifyRanDirty ? 'dirty' : 'fresh';
  } catch {
    return 'unreachable';   // diff against the ref failed — refuse-not-pass
  }
}
...
function computeHealth({ project, pipelines, bugs, audit, tests }) {
  ...
  // pipeline that is all-core-approved, verify-passed, and carries NO drift is
  // exempt from CALENDAR staleness — re-running verify on the same tracked state
  // would reproduce the same result; the nudge was pure noise that kept finished
  // features (and the root) from ever reaching idle. Two harder signals own the
  // real "evidence is outdated" cases: drift (priority 2) for artifact changes,
  // and — since M1 (ADR-085) — verify.refState for CODE changes: a merged or
  // post-verify behavioral commit makes the verdict 'stale', which blocks
  // deployable and emits its own verify-run rung regardless of this calendar
  // exemption. The calendar check below applies only while a pipeline is still
  // in flux (incomplete, verify not passed, or drifted).
  const staleVerify = [];
  for (const pl of pipelines) {
  ...
  if (!root) {
    ...
    // M1 (ADR-085): a passed verdict whose stamped tree is not THIS tree cannot certify
    // deployability. 'stale' = behavioral code changed since the run (merge window, or
    // any post-verify commit); 'unreachable' = the certified commit is gone from this
    // history (rebase/squash) — refuse-not-pass, B4 doctrine. 'dirty' and 'unbound'
    // deliberately do NOT block (non-portable warn / pre-M1+non-git degradation).
    if (root.verify.passed && root.verify.refState === 'stale')
      reasons.push({ type: 'verify_stale_ref', message: `verify passed at ${String(root.verify?.ranAt || '').slice(0, 10) || 'an earlier commit'} but behavioral code changed since (merge or post-verify commit) — re-run verify-run` });
    else if (root.verify.passed && root.verify.refState === 'unreachable')
      reasons.push({ type: 'verify_ref_unreachable', message: 'the commit verify certified is not in this history (rebase/squash?) — re-run verify-run' });
  }
...
function computeNextActions(snapshot) {
  ...
  // Priority 5 — M1 (ADR-085): a passed verdict with a stale/unreachable code ref
  // blocks deployable while NO phase is pending, so without this rung the ladder
  // would offer no exit (the documented visible-deadlock class). One action per
  // affected pipeline, in proposal order; P7's calendar-stale loop skips these.
  const refStaleScopes = new Set();
  for (const pl of workOrder) {
    if (!pl.verify.passed) continue;
    if (pl.verify.refState !== 'stale' && pl.verify.refState !== 'unreachable') continue;
    refStaleScopes.add(pl.scope);
    actions.push({
      priority: 5,
      scope:    pl.scope,
      command:  pl.scopeType === 'root' ? 'aitri verify-run' : `aitri feature verify-run ${pl.scopeName}`,
      reason:   pl.verify.refState === 'stale'
        ? 'verify passed on an earlier code state — behavioral code changed since (merge or post-verify commit)'
        : 'the commit verify certified is not in this history (rebase/squash?) — the verdict cannot be trusted',
      severity: 'warn',
    });
  }
  ...
  if (health.deployable) {
    ...
    if (auditFresh) {
      ...
      for (const pl of workOrder) {
        if (refStaleScopes.has(pl.scope)) continue;   // M1: ref-stale rung already routed verify-run
