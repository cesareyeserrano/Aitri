...
/**
 * Build a complete scope-aware `aitri …` command string for a next-step HINT.
 * Encapsulates the one subtle rule for TWO-LEVEL commands (tc/bug/backlog): the
 * `<name>` goes AFTER the verb and BEFORE the sub-verb — `aitri feature bug <name>
 * add`, never `aitri feature bug add <name>`. (That exact misplacement shipped for
 * `tc` and was caught in review; this helper prevents repeating it.)
 *
 *   scopedCmd(fr, sn, 'verify-run')        → root: 'aitri verify-run'
 *                                            feat: 'aitri feature verify-run <name>'
 *   scopedCmd(fr, sn, 'bug', 'add')        → root: 'aitri bug add'
 *                                            feat: 'aitri feature bug <name> add'
 *   scopedCmd(fr, sn, 'tc', 'verify')      → feat: 'aitri feature tc <name> verify'
 *
 * @param {string|null|undefined} featureRoot
 * @param {string|null|undefined} scopeName
 * @param {string} verb        - the top-level command (verify-run, bug, backlog, tc…)
 * @param {string} [subVerb]   - optional sub-verb appended AFTER the name (add, verify…)
 * @returns {string}
 */
export function scopedCmd(featureRoot, scopeName, verb, subVerb = '') {
  const { verb: sv, arg: sa } = scopeTokens(featureRoot, scopeName);
  return `aitri ${sv}${verb}${sa}${subVerb ? ' ' + subVerb : ''}`;
}
