...
import { injectCoverageFlag } from '../verify-parsers.js';
...
export default {
  ...
  validate(content, { dir } = {}) {
    ...
    if ('quality_gates' in d && d.quality_gates != null) {
      ...
      // Coverage-gate dry-run (FB-COVERAGE-GATE-0715 part 3) — advisory, never throws.
      // A threshold gate's instrumentability is knowable HERE, from the declared
      // test_runner; until now a non-instrumentable runner was discovered only at
      // verify-run, one approve cycle later. Same detection verify-run uses
      // (injectCoverageFlag — single source), warn-only: both remedies (command-mode
      // gate, runner fix) are edits to this same manifest, so this gate is where the
      // operator can still act cheaply.
      const thresholdGate = d.quality_gates.find(g => g && typeof g.threshold === 'number');
      if (thresholdGate && typeof d.test_runner === 'string' && d.test_runner.trim()) {
        const gateName = (typeof thresholdGate.name === 'string' && thresholdGate.name.trim()) ? thresholdGate.name : 'coverage';
        const inj = injectCoverageFlag(d.test_runner);
        if (inj.warn) {
          process.stderr.write(`[aitri] ⚠ Coverage gate "${gateName}": ${inj.warn}\n`);
        } else if (!inj.tool) {
          process.stderr.write(
            `[aitri] ⚠ Coverage gate "${gateName}" declares threshold ${thresholdGate.threshold}%, but the runner ` +
            `"${d.test_runner.trim().split(/\s+/)[0]}" is not auto-instrumentable — unless the wrapped command itself ` +
            `emits a parseable coverage report, at verify-run coverage will read "not measured" and the gate will ` +
            `${thresholdGate.required === false ? 'warn' : 'error'}. Either call an ` +
            `instrumentable runner directly (node --test / go test / pytest / jest / vitest — not an npm-script wrapper ` +
            `or a && chain), or declare the gate in command mode: ` +
            `{ "name": "coverage", "command": "<your coverage command>", "required": true } — the command enforces its ` +
            `own threshold and Aitri gates on its exit code.\n`
          );
        }
      }
    } else {
