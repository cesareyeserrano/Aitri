...
export default {
  ...
  validate(content, { dir, config } = {}) {
    ...
    // RELEASE-VERSION-0722 (rc.9): version_source is OPTIONAL (additive — pre-rc.9
    // artifacts lack it) but when present must be canonical, because status/approve
    // key display and the "assumed" warning on exact values. The deploy template
    // mandates declaring it: "manifest" (read from the product's own package
    // descriptor), "team" (the team decided/confirmed it), or "assumed" (could not
    // confirm — surfaced loudly at approve 5).
    if (d.version_source != null) {
      const VALID_SOURCES = ['manifest', 'team', 'assumed'];
      if (!VALID_SOURCES.includes(d.version_source))
        throw new Error(`version_source must be "manifest" | "team" | "assumed" (or omitted) — got "${d.version_source}". It declares where "version" came from; never invent a version without marking it "assumed".`);
    }
