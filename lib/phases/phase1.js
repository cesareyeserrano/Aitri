...
import { VAGUE, BROAD_VAGUE, HAS_METRIC, TITLE_STOP, normalizeAC, IDEA_PROVENANCE_FIELDS, PROVENANCE_VALUES, PRIORITIES, acContent, hasContentfulAC } from './phase1-checks.js';
import { securityNfrs, hasSecurityFr } from '../requirements.js';

// Scope-dependent validator floors — SSoT for BOTH validate() (the gate) and buildBriefing()
// (the {{MIN_*}} placeholders in requirements.md). One definition so the prompt can never
// promise a different number than the gate enforces (UPLAN-0703 C4). These are lower bounds
// that reject an empty/hollow artifact — the template explicitly frames them as floors, not
// sizing targets (the right count is derived from the seed via the coverage_map).
export function scopeFloors(isFeature) {
  return isFeature
    ? { minFR: 2, minNFR: 1, minNoGo: 1 }   // a feature is an INCREMENT (finance-dashboard canary)
    : { minFR: 5, minNFR: 3, minNoGo: 3 };  // greenfield root floor
}
...
export default {
  ...
  validate(content, ctx = {}) {
    ...
    const { minFR, minNFR, minNoGo } = scopeFloors(isFeature);
    ...
    // SEC-THREADING-0722: the security applicability decision must be recorded, not
    // omitted. The template already mandates it ("decide explicitly — does security
    // apply? … Do NOT leave security unaddressed by omission") but nothing enforced it,
    // so a project that FORGOT security was indistinguishable from one that decided it
    // away — and that silence switched off the entire downstream chain (the phase-2
    // Security Design body gate, the verify security-gate nudge, the resume audit nudge
    // all key on security NFRs existing). Same class as the no_go_zone gate above:
    // make omission visible and vetoable at human review, not impossible.
    // Feature sub-pipelines are exempt UNLESS the increment itself declares a
    // security-typed FR (new security surface forces the decision at feature scope;
    // otherwise the root project's recorded posture governs and a filler line here
    // would be pure ceremony — mirror of the feature floors above).
    if (securityNfrs(d).length === 0 && (!isFeature || hasSecurityFr(d))) {
      throw new Error(
        `No security decision recorded — non_functional_requirements has no category:"Security" entry${isFeature ? ' (this feature declares a security-typed FR, so the decision is required at feature scope)' : ''}.\n` +
        `  Decide explicitly and record ONE of:\n` +
        `    applies    → a real security NFR with acceptance criteria, e.g.\n` +
        `                 {"id":"NFR-00X","category":"Security","requirement":"All API endpoints reject unauthenticated requests","acceptance_criteria":"returns 401 without a valid token"}\n` +
        `    does not   → an explicit exclusion in the "requirement" field, e.g.\n` +
        `                 {"id":"NFR-00X","category":"Security","requirement":"Not applicable: offline single-user tool — no network, no secrets, no PII"}\n` +
        `  Security left unaddressed by omission is indistinguishable from security forgotten.\n` +
        `  (Probe without side effects: aitri complete 1 --check)`
      );
    }
    ...
    // REQ-RICHNESS-0711 addendum (rc.174): priority is the OTHER join key — the one the
    // whole MUST-gate family filters on (type/metric/vagueness/US-AC floor here, the 3×
    // TC floor at Phase 3, the compliance proof at Phase 5 — all exact-match `=== 'MUST'`).
    // It was never validated: a miscased "must" or a priority-less FR silently exempted
    // the requirement from EVERY MUST gate while looking prioritized — the same
    // silent-pass class ADR-077 shipped to close (post-ship adversarial finding).
    // FRs require one; NFR priority stays optional (a category:"Regression" NFR is MUST
    // by category alone — lib/requirements.js), but when present it must be canonical.
    const badPriority = [];
    for (const fr of d.functional_requirements) {
      if (!PRIORITIES.includes(fr.priority))
        badPriority.push(`${fr.id}: ${JSON.stringify(fr.priority ?? null)}`);
    }
    for (const nfr of d.non_functional_requirements) {
      if (nfr.priority != null && !PRIORITIES.includes(nfr.priority))
        badPriority.push(`${nfr.id}: ${JSON.stringify(nfr.priority)}`);
    }
    if (badPriority.length)
      throw new Error(
        `Invalid priority value(s): ${badPriority.join(', ')} — use exactly "MUST", "SHOULD", or "NICE" ` +
        `(case-sensitive; FRs require one, NFR priority may be omitted — a category:"Regression" NFR is MUST by category). ` +
        `Priority is the key every MUST gate filters on: an off-vocabulary value silently exempts the requirement ` +
        `from the type/AC/user-story gates here, the Phase-3 test floor, and the Phase-5 compliance proof.`
      );
    ...
    // rc.174: contentful, not length-only — `['']` / `[{}]` satisfied the old `.length`
    // check while carrying nothing a Phase-3 test could assert (same silent-pass class).
    const missingCriteria = mustFRs.filter(fr => !hasContentfulAC(fr.acceptance_criteria));
    if (missingCriteria.length)
      throw new Error(`MUST FRs missing acceptance_criteria: ${missingCriteria.map(f => f.id).join(', ')} — each needs ≥1 criterion with actual content (a content-free entry like "" does not count).`);
    ...
    // rc.174: read AC text via acContent — a raw `.includes` crashed with a TypeError on an
    // object-form AC, and hasContentfulAC now formally admits that shape at the FR level.
    const assumptions = d.functional_requirements.filter(fr =>
      fr.title?.includes('[ASSUMPTION') ||
      (fr.acceptance_criteria || []).some(ac => acContent(ac).includes('[ASSUMPTION'))
    );
    ...
    // REQ-RICHNESS-0711 A1/A2 (ADR-077): the proportional US/AC floor. Runs AFTER the
    // FR-level content checks (metric / vagueness / title / duplicate-AC) so a malformed FR
    // surfaces its specific error before the story-layer complaint — an FR must be well-formed
    // at the FR level before we require its story. MUST-only (SHOULD/NICE exempt — proportional,
    // no filler stories on nice-to-haves); feature scope obeys the same rule (a feature's MUST
    // FRs are still core behaviors). Presence is gated, richness is pushed by briefing + persona.
    //
    // A1: every MUST FR must have ≥1 linked user story. Was a non-blocking warning — thin
    // requirements (a MUST FR with no story) passed `complete 1` silently, and the story is
    // where the acceptance criteria a Phase-3 test derives from live.
    ...
    if (mustFRsWithoutStory.length)
      throw new Error(
        `MUST FR(s) with no linked user story: ${mustFRsWithoutStory.map(f => f.id).join(', ')} — ` +
        `every MUST FR needs ≥1 user_story whose requirement_id references it. A MUST behavior ` +
        `with no story is under-decomposed; its Phase-3 test cases derive from the story's ` +
        `acceptance criteria, so no story means no testable acceptance surface.`
      );

    // A2: a user story linked to a MUST FR must carry ≥1 acceptance criterion. Closes the
    // empty-story hole — `user_stories: []` and a story with zero acceptance_criteria both used
    // to pass. PRESENCE is gated, not FORM: a plain string, a { id, text }, or a
    // { id, given, when, then } all satisfy it; the Given/When/Then shape is the target the
    // briefing + persona push, never gated here (form-gating would reject a legitimate one-line
    // metric AC and tip into the presence-theater ADR-061 was withdrawn for). The acceptance
    // criteria ARE what Phase 3 turns into test cases — a MUST-linked story with none yields a
    // test with nothing to assert. rc.174: presence = ≥1 element with actual CONTENT
    // (hasContentfulAC) — `['']` / `[{}]` / `[null]` satisfied the old length check while
    // carrying nothing to assert (post-ship adversarial finding on ADR-077).
    const mustLinkedStoriesNoAC = (d.user_stories || []).filter(us =>
      us.requirement_id && mustFRs.some(fr => fr.id === us.requirement_id) &&
      !hasContentfulAC(us.acceptance_criteria)
    );
    if (mustLinkedStoriesNoAC.length)
      throw new Error(
        `User story(ies) linked to a MUST FR with no acceptance_criteria: ` +
        `${mustLinkedStoriesNoAC.map(us => us.id || us.requirement_id).join(', ')} — ` +
        `give each ≥1 acceptance criterion with actual content (a content-free entry like "" or {} ` +
        `does not count; Given/When/Then with concrete values is the target). ` +
        `The acceptance criteria are what Phase 3 turns into the story's test cases.`
      );
  ...
  buildBriefing({ dir, inputs, feedback, artifactsBase, config = {}, featureRoot = null, scopeVerb = '', scopeArg = '', contextAssets = '' }) {
    ...
    if (!isRerun) {
      if (!idea) {
        // Name the layout-resolved location — "Create IDEA.md" alone reads as project root,
        // but contained projects (LAYOUT-1) keep the seed at <layoutRoot>/product/IDEA.md.
        const seedRel = featureRoot
          ? 'FEATURE_IDEA.md'
          : [productSubdir(config), 'IDEA.md'].filter(Boolean).join('/').replace(/\\/g, '/');
        throw new Error(
          `Missing required file: ${featureRoot ? 'FEATURE_IDEA.md' : 'IDEA.md'}\n` +
          `  Phase 1 needs a seed brief on first run. Create ${seedRel}${featureRoot ? '' : ' (or run aitri wizard)'}.`
        );
      }
    ...
    return render('phases/requirements', {
      ...
      // Scope-correct validator floors (UPLAN-0703 C4), from the scopeFloors SSoT shared with
      // validate() — the template previously hardcoded the root values, so a feature agent
      // read "Min 5 FRs" against a gate that requires 2 and padded with filler (the exact
      // defect the feature floor was added to remove, reintroduced at the prompt layer).
      // The template frames these as validator lower bounds, NEVER as sizing targets.
      MIN_FR:  String(scopeFloors(!!featureRoot).minFR),
      MIN_NFR: String(scopeFloors(!!featureRoot).minNFR),
      MIN_NGZ: String(scopeFloors(!!featureRoot).minNoGo),
