...
import { readArtifact, productSubdir } from '../state.js';
...
export default {
  ...
  validate(content, ctx = {}) {
    ...
    // Structured acceptance-criterion ids (ADR-041 option A): when Phase 1 declares ACs
    // as `{ id, text }` in user_stories[].acceptance_criteria, that id is the finer join
  ...
  buildBriefing({ dir, inputs, feedback, artifactsBase, config = {}, featureRoot = null, scopeVerb = '', scopeArg = '' }) {
    ...
    const idea = isRerun ? '' : (readArtifact(dir, 'IDEA.md', productSubdir(config)) || inputs['IDEA.md'] || '');
