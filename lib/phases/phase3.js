...
export default {
  ...
  validate(content, ctx = {}) {
    ...
    for (const [reqId, cases] of Object.entries(byReq)) {
      ...
      if (cases.length < 3) {
        coverageIssues.push(`${reqId} has ${cases.length} test case(s) — min 3 required (happy path, edge case, negative)`);
        continue; // happy/edge/negative cannot exist below the minimum — fixing the count comes first
      }
