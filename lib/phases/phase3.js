...
import { crossPipelineTCCollisions, describeCollision, suggestTCNamespace } from '../tc-collisions.js';
...
export default {
  ...
  validate(content, ctx = {}) {
    ...
    // Cross-pipeline TC-id uniqueness (TC-ID-COLLISION-0911, ADR-092). verify-run credits a
    // result by TC id alone, and a runner often executes more than its own pipeline's tests
    // (a whole-repo script), so an id planned by two pipelines cross-credits between them.
    // Needs the project on disk (ctx.dir); a content-only validate call skips it.
    if (ctx.dir) {
      const collisions = crossPipelineTCCollisions({
        dir: ctx.dir,
        featureRoot: ctx.featureRoot,
        ids: d.test_cases.filter(tc => tc && typeof tc.id === 'string').map(tc => tc.id),
      });
      if (collisions.length) {
        const shown = collisions.slice(0, 20).map(c => `  ${describeCollision(c)}`);
        if (collisions.length > 20) shown.push(`  … and ${collisions.length - 20} more`);
        const ns = ctx.featureRoot ? suggestTCNamespace(path.basename(ctx.dir)) : null;
        throw new Error(
          `TC id(s) already planned by another pipeline of this project:\n${shown.join('\n')}\n` +
          `TC ids must be unique across the root and every feature. verify-run credits a result by ` +
          `TC id alone, and a test runner often executes more than this pipeline's own tests (a ` +
          `whole-repo test script, a shared suite) — so the other pipeline's test would be credited ` +
          `to this plan's TC: a pass while this TC's own test is missing, or a failure it never owned. ` +
          (ns
            ? `Give this feature's ids their own namespace (e.g. ${ns}001h), `
            : `Rename the colliding ids on one side (a feature namespace, e.g. TC-<FEATURE>-001h), `) +
          `rename the matching test titles, then complete phase 3 again.`
        );
      }
    }
          ...
          if (knownAcIds.size > 0) {
            ...
            if (tcsWithAc.length > 0)
              process.stderr.write(
                ...
                `  For real AC-level traceability, add { "id": "AC-…", "given": "…", "when": "…", "then": "…" } entries to user_stories[].acceptance_criteria.\n`
              );
          }
  ...
  buildBriefing({ dir, inputs, feedback, artifactsBase, bestPractices, scopeVerb = '', scopeArg = '', contextAssets = '' }) {
