 ...
 * five generated files and re-render them, so the emitted paths match the new
 ...
 *
 * @returns {string[]} The regenerated file list.
 */
export function regenerateAgentFiles(dir, rootDir, config) {
  for (const relPath of AGENT_FILES) {
    try { fs.unlinkSync(path.join(dir, relPath)); } catch { /* absent — fine */ }
  }
  return writeAgentFiles(dir, rootDir, config);
}
