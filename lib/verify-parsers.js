 ...
 *   - istanbul table ("All files | XX.XX |"): node built-in --experimental-test-coverage, jest, vitest
...
export function parseCoverageOutput(output) {
  ...
  // Line-anchored to block a mid-line spoof; the ONLY tolerated prefixes are the test
  // reporters' own line markers — node:test spec prints `ℹ all files | …` and its TAP
  // reporter `# all files | …` (FB-COVERAGE-GATE-0715: the bare `^\s*` anchor rejected
  // node's real output, so built-in-runner coverage was never measured). A prose line
  // ("Copying All files | 100 done") still cannot match: nothing but whitespace and
  // reporter markers may precede the row.
  let m = output.match(/^\s*(?:[#ℹ]\s*)?all files\s*\|\s*([\d.]+)/im);
 ...
 * @returns {{cmd: string, tool: string|null, warn?: string}} - tool is the instrumentation label,
 *   or null if unrecognized; warn is a diagnosis the caller should surface (cmd is never rewritten)
 */
/**
 * Shell chain/pipe operator detection — ONE grammar, shared with
 * gatesWithShellOperators (verify.js). Space-delimited operators only, matching the
 * shell-noise class it was written for; `a&&b` glued is out of scope on both sites.
 * @param {string} cmd
 * @returns {boolean}
 */
export function hasShellChain(cmd) {
  return /(^|\s)(&&|\|\||;|\|)(\s|$)/.test(cmd || '');
}

export function injectCoverageFlag(testCmd) {
  const cmd = (testCmd || '').trim();
  // A chained command cannot be threshold-instrumented: every per-runner branch edits the
  // STRING (prefix after `node `, suffix for pytest/jest/vitest), so on a chain the flag
  // lands on the WRONG command — verified: 'vitest run && playwright test' handed
  // playwright the --coverage flag, turning a coverage request into a broken-looking test
  // run (whole-canary adversarial, FB-COVERAGE-GATE-0715). build.md/AGENTS.md already
  // declare chains non-instrumentable (command-mode gate is the remedy); the detector now
  // agrees with its own documentation. tool:null → verify-run says not-auto-instrumentable
  // (and still parses output, in case the chain itself emits a parseable coverage table)
  // and the complete-4 dry-run warns BEFORE the approve cycle.
  if (hasShellChain(cmd)) return { cmd, tool: null };
  if (cmd.startsWith('node ')) {
    if (/--experimental-test-coverage\b/.test(cmd)) return { cmd, tool: 'node' };
    if (/--coverage(?![\w-])/.test(cmd)) {
      // Bare `--coverage` is NOT a Node CLI option on ANY version. If node itself parses
      // it (it appears before the entry script), the run aborts with `node: bad option`
      // (exit 9) before a single test executes; if it sits after the entry script it is
      // an argument to that script, which may or may not understand it. Aitri never
      // rewrites a user-declared command — pass it through and surface the diagnosis,
      // so an exit-9 abort is attributed to the flag, not to the toolchain
      // (FB-COVERAGE-GATE-0715). `--coverage-<x>` args do not trip this: they are not
      // the bare flag and must not suppress injection (the (?![\w-]) guard).
      return {
        cmd, tool: 'node',
        warn: 'test_runner passes `--coverage` to node — not a Node CLI option. If node parses it ' +
              '(it appears before the entry script), the run aborts with `node: bad option` and zero ' +
              'tests execute. Use --experimental-test-coverage, or declare the coverage gate in ' +
              'command mode ({ "name": "coverage", "command": "...", "required": true }).',
      };
    }
    // Node's built-in runner has exactly one coverage flag (present since 18.15/19.7,
    // unchanged through 24): --experimental-test-coverage. `--coverage` is NOT a Node
    // CLI option on ANY version — injecting it aborts the whole run with
    // `node: bad option` (exit 9), so zero tests execute (FB-COVERAGE-GATE-0715).
