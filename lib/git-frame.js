...
// Output ceiling for the path-listing readers below. Node's execFileSync default is 1 MiB:
// a working tree with a few thousand untracked, non-ignored files (a leftover coverage or
// report tree) overflowed it, the reader threw ENOBUFS, and every caller silently degraded
// — the verify binding lost its stamp (reproduced, VERIFY-REF-PRERUN-0911 adversarial
// pass). Node allocates as output arrives, so a high ceiling costs nothing on a small tree.
...
export function gitDiffNames(dir, range, { diffFilter = null } = {}) {
  ...
  const out = execFileSync('git', args, { cwd: dir, stdio: ['ignore', 'pipe', 'ignore'], maxBuffer: GIT_MAX_BUFFER }).toString();
...
export function gitPorcelainPaths(dir) {
  const out = execFileSync('git', ['status', '--porcelain', '-z', '--untracked-files=all'],
    { cwd: dir, stdio: ['ignore', 'pipe', 'ignore'], maxBuffer: GIT_MAX_BUFFER }).toString();
