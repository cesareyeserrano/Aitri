...
function adoptApply({ dir, VERSION, err, fromPhase, rootDir }) {
  ...
  // Relocate adoption inputs the operator/agent placed at the project root (pre-config —
  // bare `aitri adopt` writes there, and the scan writes ADOPTION_AUDIT.md there) into the
  // unit's idea_context/, so Phases 1-3 list them (run-phase CONTEXT_PHASES) and ground
  // requirements + design in them (ADR-056). IDEA.md stays the operator's pure intent; the
  // audit + supporting docs are the consumed context. Never clobber an existing unit file.
  ...
  // The scan writes ADOPTION_AUDIT.md at the project root.
  moveIntoCtx(path.join(dir, 'ADOPTION_AUDIT.md'), 'ADOPTION_AUDIT.md (Phases 1-3 read it)');
  // A root idea_context/ (from `aitri adopt`) — move its files in so they are not orphaned.
  ...
  if (fs.existsSync(rootCtxDir) && path.resolve(rootCtxDir) !== path.resolve(unitCtxDir)) {
    for (const f of fs.readdirSync(rootCtxDir)) {
      // The unit already carries its own README — drop the root scaffold copy.
    ...
    try { fs.rmdirSync(rootCtxDir); } catch { /* not empty (a clobber-skipped file remains) — leave it */ }
  }
