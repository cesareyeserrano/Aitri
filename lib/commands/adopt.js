...
function adoptInit({ dir, err }) {
  ...
  if (!fs.existsSync(ctxReadme)) {
    fs.writeFileSync(ctxReadme,
      '# idea_context/ — provided definitions for the adoption\n\n' +
      'Drop the objective\'s supporting material here: a migration guide, a prior PRD, API docs, ' +
      'specs, screenshots, reference docs. At the requirements/design phases Aitri injects the full text of ' +
      'the readable files here into the briefing (and flags what it cannot read, e.g. images, to be opened); ' +
      'the adoption audit (ADOPTION_AUDIT.md) lands here too — so the requirements and plan are grounded in them.\n');
  }
