...
function adoptScan({ dir, VERSION, err }) {
  ...
  process.stderr.write(
    ...
    ` Your agent will create: ADOPTION_AUDIT.md and IDEA.md\n\n` +
    ` What happens next:\n` +
...
function adoptApply({ dir, VERSION, err, fromPhase, rootDir }) {
  ...
  // The scan writes ADOPTION_AUDIT.md at the project root (it runs pre-config, before the
  // layout exists). Move it into the unit's idea_context/ so Phases 1-3 list it
  // (run-phase CONTEXT_PHASES) and the agent grounds requirements + design in its findings
  // (ADR-056). IDEA.md stays the operator's pure intent; the audit is the consumed diagnostic.
  const rootAuditPath = path.join(dir, 'ADOPTION_AUDIT.md');
  if (fs.existsSync(rootAuditPath)) {
    const ctxDir = ideaContextDir(dir, config);
    const unitAuditPath = path.join(ctxDir, 'ADOPTION_AUDIT.md');
    if (rootAuditPath !== unitAuditPath) {
      if (fs.existsSync(unitAuditPath)) {
        process.stderr.write(
          `[aitri] Warning: ${path.relative(dir, unitAuditPath)} already exists — the root ADOPTION_AUDIT.md was NOT moved.\n` +
          `  Compare the two and keep the current one, then delete the root copy.\n`
        );
      } else {
        fs.mkdirSync(ctxDir, { recursive: true });
        fs.renameSync(rootAuditPath, unitAuditPath);
        console.log(`\n  📋 ADOPTION_AUDIT.md → ${path.relative(dir, ctxDir)}/ — Phases 1-3 will read it`);
      }
    }
  }
...
function adoptVerifySpecComplete({ dir, err }) {
  ...
  if (verified) console.log(`   ${verified} already verified (existing tests cover those ACs)`);
