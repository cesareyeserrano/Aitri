...
import { sgr, divider } from '../format.js';
...
export function cmdHelp({ VERSION, args = [] }) {
  ...
  // must not carry ANSI escape codes. sgr() returns '' when color is off, so the banner
  // and section headers render as plain text.
  const steel = sgr('38;5;75');
  const fire  = sgr('38;5;208');
  const ember = sgr('38;5;166');
  const dim   = sgr('2');
  const bold  = sgr('1');
  const reset = sgr('0');
...
`;
  ...
  aitri verify-run --coverage-threshold N   Also measure line coverage; warn below N% (node/go/pytest/jest/vitest)
                                       (npm-script wrapper or chained test_runner can't be instrumented —
                                        declare a command-mode coverage gate in quality_gates instead)
  ...
  aitri bug list                       Unresolved bugs (open, in-progress, fixed-unverified)
  aitri bug add --title "..." [--severity critical|high|medium|low] [--fr FR-XXX] [--tc TC-NNN]
  aitri bug fix <BG-NNN>               Mark fixed (still counts as unresolved until verified)
  aitri bug verify <BG-NNN>            Confirm fix verified
  aitri bug close <BG-NNN>             Archive
  aitri bug note <BG-NNN> --text "..." Append a dated entry to the bug's log
  aitri bug update <BG-NNN> --<field>  Correct the record (title, severity, tc, ...) — never status
  ...
  aitri backlog update <id> --priority|--problem|--files|... "..."
  ...
  aitri feature bug       <name> add|fix|verify|close|note|update|list ...
  aitri feature backlog   <name> add|list|show|note|update|done ...
  ...
  // `aitri help <command>` → just the section that documents it. `help help` gets the
  // short overview itself (its "More:" block IS help's own usage), not the unknown note.
  if (topic && topic !== 'help' && !wantAll) {
