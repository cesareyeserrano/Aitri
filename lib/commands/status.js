...
function emitText(snapshot) {
  const { project, pipelines, bugs, backlog, nextActions, health } = snapshot;
...
function emitJson(snapshot) {
  const { project, pipelines, bugs, audit, health, nextActions, snapshotVersion, backlog } = snapshot;
  const root = pipelines.find(p => p.scopeType === 'root');
