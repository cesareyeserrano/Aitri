...
import { creationOrdinals, writeFeaturesIndex } from '../features-index.js';
...
export function cmdStatus({ dir, VERSION, args = [] }) {
  ...
  // FEATURE-INDEX-0909: refresh the file-tree view of creation order. Text mode only —
  // `--json` is Hub's polling surface and must stay read-only. Idempotent (no-op when
  // unchanged), so the constant `status` cadence costs nothing.
  try { writeFeaturesIndex(dir, snapshot); } catch { /* derived view — never fail status over it */ }
...
function emitText(snapshot) {
  ...
  if (features.length) {
    ...
    // One rule (proposalOrder via creationOrdinals) — the `#N` here, in features/INDEX.md
    // and in the ladder's work order are the same number by construction (rc.12).
    const ordinals = creationOrdinals(pipelines);
  ...
  // Suppress the "no unresolved bugs" label when a parse error is being reported below —
  // "no unresolved bugs" + "unreadable" side by side reads as a contradiction (the zero is
  // an artifact of the corruption, not a fact).
  if ((bugs.total > 0 || bugsFilesExist(snapshot)) && !(bugs.open === 0 && bugs.parseErrors.length > 0)) {
    // BUG-STATE-VISIBLE-0909: say what the count is made of. "(open/in-fix)" read as
    // open|in_progress while the number included `fixed` — a project with 2 open + 9
    // fixed showed "11 active bugs", and the fixed ones were re-triaged as open.
    const n = bugs.open;
    const parts = [];
    if (bugs.active > 0) parts.push(`${bugs.active} open/in-progress`);
    if (bugs.fixed  > 0) parts.push(`${bugs.fixed} fixed awaiting verify`);
    // "unresolved", not "active": `status --json` bugs.active and isActiveBug mean open|in_progress
    // ONLY — the text count includes fixed-awaiting-verify, so it must not borrow that word.
    const label = n === 0 ? 'no unresolved bugs' : `⚠️  ${n} unresolved bug${n > 1 ? 's' : ''} (${parts.join(' · ')})`;
...
function emitJson(snapshot) {
  ...
  const payload = {
    ...
    bugs:           {
      total:      bugs.total,
      open:       bugs.open,
      // Additive (2.2.0-rc.14, BUG-STATE-VISIBLE-0909): `open` = open|in_progress|fixed
      // (unchanged, compatibility). `active` = open|in_progress (what actually blocks);
      // `fixed` = fixed-awaiting-verification. active + fixed === open.
      active:     bugs.active,
      fixed:      bugs.fixed,
