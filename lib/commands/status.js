...
function emitText(snapshot) {
  ...
  if (features.length) {
    ...
    const dated = features.filter(f => f.createdAt).sort((a, b) => a.createdAt.localeCompare(b.createdAt));
    ...
    const createdRank = f => (f.createdAt ? `0${f.createdAt}` : `1${f.scopeName}`);
    ...
    for (const f of sorted) {
      ...
      const created = f.createdAt ? ` · created ${shortDate(f.createdAt)}` : '';
...
function emitJson(snapshot) {
  ...
  const featureSummaries = pipelines.filter(p => p.scopeType === 'feature').map(f => {
    ...
    return {
      ...
      // a real feature timeline. null on pre-createdAt projects.
