...
// "complete 4" → fails → a 4-hop backwards chain to discover run-phase). buildProjectSnapshot
// already owns next-action logic (SSoT invariant), so read its top action instead of
// duplicating the ladder here. Best-effort — any failure falls back to a scope-safe pointer.
function snapshotNextAction(dir) {
  try {
    const snap = buildProjectSnapshot(dir);
    return (snap.nextActions || [])[0] || null;
  } catch { return null; }
}
...
import {
  ...
  parseCoverageOutput, injectCoverageFlag, findMultiTCIdLines
...
export function cmdVerifyRun({ dir, args, flagValue, err, featureRoot, scopeName }) {
  ...
  if (!(config.approvedPhases || []).includes(4)) {
    ...
    const next = featureRoot ? null : snapshotNextAction(dir);
    const hint = next
