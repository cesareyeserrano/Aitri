...
import { loadConfig, saveConfig, readArtifact, readArtifactFile, artifactPath, appendEvent, writeLastSession, atomicWrite, hashArtifact } from '../state.js';
...
export function runQualityGates(gates, dir) {
  ...
  for (const g of (Array.isArray(gates) ? gates : [])) {
    ...
    } else if (r.signal || r.error) {
      // killed (typically timed out at timeoutMs) — surface as `error`, not `fail`, so a
      // too-short timeout reads as "raise timeout_ms", not "the code failed the check".
...
export function gatesWithShellOperators(gates) {
  return (Array.isArray(gates) ? gates : [])
    .filter(g => g && typeof g.command === 'string' && /(^|\s)(&&|\|\||;|\|)(\s|$)/.test(g.command))
...
export function cmdVerifyComplete({ dir, err, featureRoot, scopeName, VERSION }) {
  ...
  // Run-binding (Finding 1): the results file must be the one verify-run produced.
  // verify-run / tc verify stamp `config.verifyResultsHash` with the hash of the file they
  // write; if it is present and the current file no longer matches, the file was edited or
  // replaced after the run — block, because the deploy gate must not trust a hand-edited
  // results file. Honest ceiling: an absent hash (a pre-stamp or externally-produced file)
  // is NOT blocked (backward-compatible), and a fully-adversarial agent that also rewrites
  // the stamp in `.aitri` is not stopped (the file is writable) — this closes the realistic
  // "ran verify-run, saw failures, edited the results to pass" shortcut.
  if (config.verifyResultsHash && hashArtifact(resultsContent) !== config.verifyResultsHash)
    err(
  ...
  if (failedStubs.length) {
    ...
    config.verifyResultsHash = hashArtifact(updatedResults);
