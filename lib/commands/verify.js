...
// Depth-bounded scan for TRX / JUnit-XML result files written by the test run.
// `sinceMs > 0` keeps only files modified at/after the run started — essential
// because `dotnet test` writes a NEW guid-named .trx per run and never deletes
// the old ones, so without the mtime guard a stale PASS file from a prior run
// could mask a current FAIL. node_modules/.git/build dirs are skipped.
function collectXmlResultFiles(root, sinceMs, depth = 4) {
  const out = [];
...
/**
 * Resolve which TRX/JUnit-XML files to read, given the `--results` flag value
 * (or null for auto-discovery). Exported for direct testing.
 *  - explicit FILE  → just that file (no mtime guard; the operator chose it)
 *  - explicit DIR   → the single newest result file in it (deterministic; avoids
 *                     merging stale guid-named .trx siblings)
 *  - null           → auto-discover under `dir`, modified since `runStartMs`
 * @returns {string[]} absolute-or-dir-relative file paths to parse
 */
export function resolveResultFiles(dir, resultsFlag, runStartMs) {
  if (resultsFlag) {
    ...
    if (st.isDirectory()) {
      const files = collectXmlResultFiles(p, 0)
  ...
  return collectXmlResultFiles(dir, runStartMs - 1000);
}
...
export function cmdVerifyRun({ dir, args, flagValue, err, featureRoot, scopeName }) {
  ...
  // files written by a previous run (see collectXmlResultFiles).
  ...
  // `--results <file|dir>` is explicit; otherwise auto-discover fresh files under dir.
  ...
  if (resultsFlag && xmlFiles.length === 0) {
    process.stderr.write(`[aitri] --results: no TRX/JUnit-XML file found at "${resultsFlag}"\n`);
  }
  if (xmlFiles.length > 0) {
    ...
    if (xmlCount > 0)
      process.stderr.write(`[aitri] TRX/JUnit-XML parser: ${xmlCount} TC(s) detected from ${xmlFiles.length} file(s)\n`);
  }
  ...
  const SKIP_NOTE = (tc) => {
    const base =
      ...
      `For runners that emit results to a file (dotnet test --logger trx, Maven/Gradle JUnit, ` +
      `pytest --junitxml), point aitri at it: verify-run --results <file|dir>.`;
    return hasPwConfig
