...
export function cmdVerifyRun({ dir, args, flagValue, err, featureRoot, scopeName }) {
  ...
  console.log([
    ...
    `  Manual:  ${(summary.manual + (summary.manual_verified || 0)) === 0 ? '0' : `${summary.manual_verified || 0}/${summary.manual + (summary.manual_verified || 0)} verified${summary.manual > 0 ? ` (${summary.manual} pending — run: aitri tc verify <TC-ID> --result pass|fail --notes "...")` : ''}`}`,
    ...coverageLines,
    ...zeroTCsLines,
    ...manifestWarnLines,
    ...assertionWarnLines,
