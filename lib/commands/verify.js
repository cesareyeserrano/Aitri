...
export function parseJUnitXmlResults(xml) {
  ...
  while ((m = openRe.exec(xml)) !== null) {
    ...
    if (!selfClosed) {
      ...
      const nextOpen  = xml.indexOf('<testcase', bodyStart);
      // Bound the body to THIS testcase only. On well-formed XML that's its own
      // </testcase>. On malformed XML (a missing close tag, FB-MULTI-0619 #5) the
      // close is absent or sits past the next sibling's opening — without a bound the
      // scan would slurp to EOF and absorb a SIBLING's <failure>, flipping this PASS
      // to FAIL (a false regression). Stop at whichever comes first: own close, or
      // the next <testcase. If neither exists (the genuinely-last, unclosed case),
      // its body is the remainder — a real child still counts, an absent one stays pass.
...
export function cmdVerifyRun({ dir, args, flagValue, err, featureRoot, scopeName }) {
  ...
  // Root scope only. Feature scope has no `feature tc verify` to follow the seed
  // (the scope grammar is `feature <verb> <name>`, which `tc verify <id>` doesn't
  // fit), so seeding + the verify-complete zero-verification guard would wall a
  // feature with no escape. Feature manual-verification is a known gap left to the
  // deferred C1 design — feature all-manual keeps its prior behavior here.
  ...
  if (allManual) {
    process.stderr.write(
      ...
      `[aitri] Seeding 04_TEST_RESULTS.json with manual results. Record each: aitri ${sv}tc verify${sa} <TC-ID> --result pass|fail --notes "..."\n` +
      `[aitri] (a human at a terminal can run bare aitri ${sv}tc verify${sa} for a guided checklist).\n`
    );
  } else {
  ...
  console.log([
    ...
    ...(allManual
      ? [`Verify each manual TC by hand, then gate:`,
         `  aitri tc verify                                   (guided checklist, at a terminal)`,
         `  aitri tc verify <TC-ID> --result pass|fail --notes "..."   (one at a time / scripted)`,
...
export function cmdVerifyComplete({ dir, err, featureRoot, scopeName, VERSION }) {
  ...
  // status pass, so this clears once the human verifies. Root scope only — `tc verify`
  // is a root command (no feature-scoped form), so a feature pipeline is left on its
  // prior behavior rather than walled by a guard it cannot clear (see cmdVerifyRun).
  if (!featureRoot && manualTCs.length > 0 && failed.length === 0 && passedTCs.length === 0)
    err(
      ...
      `  Verify each by hand:  aitri tc verify   (guided checklist, at a terminal)\n` +
      `  or one at a time:      aitri tc verify <TC-ID> --result pass|fail --notes "..."`
    );
