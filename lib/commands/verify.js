...
import { buildProjectSnapshot, topNextAction, captureVerifyRef, verifyRefFreshness, verifyRefRootCtx } from '../snapshot.js';
import { crossPipelineTCCollisions, describeCollision } from '../tc-collisions.js';
...
import { autoVerifyBugs, getBlockingBugs, getBlockingBugsAllScopes, promptAndRegisterBugs, assertBugsReadable } from './bug.js';
import { countOpenBacklog } from './backlog.js';
...
// ── Dispatch capture (DISPATCH-PIPE-0903) ───────────────────────────────────────
// Every child Aitri runs — test runner, Playwright auto-run, quality gates — used to
// be captured through PIPES. Two defects follow from the pipe itself, both measured in
// a real project, and BOTH manufacture kills that Aitri then reports as verification
// outcomes:
//
//  1. spawnSync does not return when the child EXITS — it returns when the LAST holder
//     of the pipe closes it. An e2e suite that starts an app server leaves that server
//     holding the inherited pipe, so a run that finished in 2 minutes held the CLI until
//     the 15-minute timeout, was then reported as a KILLED dispatch, and every e2e TC
//     recorded as `skipped`: a green verify-run that accredited nothing. (T-Ledger,
//     measured 2026-08-28: 2:07 with output to a file vs not finishing in 30 min through
//     the pipe.) The suite was healthy; the pipe was the defect.
//  2. A pipe capture has a fixed ceiling. Past it Node KILLS the child mid-run (ENOBUFS)
//     and truncates the output — a loud-but-healthy suite or gate read as an `error` it
//     never was. The runner's ceiling was raised to 64 MiB for exactly this reason; the
//     gates were left on Node's 1 MiB default and kept dying of volume.
//
// Capturing into a temp FILE removes both causes: there is no pipe for a survivor to
// hold open, and no ceiling for Node to kill over. What Aitri JUDGES is unchanged — the
// same real exit code, the same real output, the same kill detection (timeout/signal) —
// it only stops manufacturing kills of its own, which is strictly MORE verification, not
// less: results that used to vanish into `skipped` are now accredited.
//
// The read back is capped (memory, not the child): a log past the cap is read from its
// TAIL and flagged `truncated`, and every caller treats a truncated read as "this run was
// not fully seen" — never as a complete run with fewer results.

/**
 * Read a captured stream back from its OPEN fd (never by path — the path is unlinked
 * immediately on POSIX so a crash or a SIGKILL of Aitri itself cannot leave the capture
 * behind, and a surviving orphan that keeps writing writes into an inode nobody can see).
 *
 * Returns `{ text, truncated, failed }`. `failed` is NOT the same as "the child printed
 * nothing": a read that could not happen (ENOMEM on a huge log, a Windows lock, a tmpdir
 * cleaner) must never be indistinguishable from an empty run — every caller refuses or
 * reports on `failed`, because silently reading a run as output-less credits nothing and
 * would surface as a phantom regression with no diagnostic.
 *
 * @param {number} fd
 * @param {number} cap - max bytes to read back (memory bound, not a limit on the child)
 * @param {boolean} skipBodyOverCap - when true, an over-cap stream is reported as
 *   truncated WITHOUT being read at all (the caller refuses on truncation anyway, so
 *   allocating the tail would cost ~2x the cap in RSS for output nobody consumes)
 */
export function readCappedFd(fd, cap, skipBodyOverCap = false) {
  let size;
  try { size = fs.fstatSync(fd).size; } catch { return { text: '', truncated: false, failed: true }; }
  const over = size > cap;
  if (over && skipBodyOverCap) return { text: '', truncated: true, failed: false };
  const want = over ? cap : size;
  if (want === 0) return { text: '', truncated: false, failed: false };
  // Buffer.alloc (never allocUnsafe): a short read must not leave uninitialised process
  // memory in the tail, which would flow straight into the artifact and the summary.
  let buf, read = 0;
  try {
    buf = Buffer.alloc(want);
    let pos = over ? size - cap : 0;
    while (read < want) {
      const n = fs.readSync(fd, buf, read, want - read, pos);
      if (n <= 0) break;               // EOF / short read — keep exactly what we got
      read += n; pos += n;
    }
  } catch {
    return { text: '', truncated: over, failed: true };
  }
  let text = buf.subarray(0, read).toString('utf8');
  // Reading from the tail can start mid-codepoint; drop the resulting replacement char.
  if (over && text.charCodeAt(0) === 0xFFFD) text = text.slice(1);
  return { text, truncated: over, failed: false };
}

/**
 * spawnSync with the output captured through temp FILES instead of pipes.
 *
 * Drop-in for the three dispatch sites: same arguments, same returned shape
 * (`status` / `signal` / `error` / `pid` / `stdout` / `stderr`), plus:
 *   - `truncated`   — the child's output exceeded `readCap` and only its tail was read
 *   - `captureMode` — 'file' normally, 'pipe' when no temp file could be created
 *
 * Never uses a shell (injection surface) and never detaches the child: `detached`
 * would give Aitri its own process group to sweep, but it also removes the child from
 * the terminal's foreground group, and a SIGINT handler cannot run while spawnSync
 * blocks — Ctrl-C would stop stopping the suite. Group-sweeping is deliberately left
 * to a separate change.
 *
 * @param {string} bin
 * @param {string[]} args
 * @param {{cwd?: string, timeout?: number, env?: object, readCap?: number}} opts
 */
export function spawnCaptured(bin, args, opts = {}) {
  const { cwd, timeout, env, readCap = RUNNER_MAX_BUFFER, skipBodyOverCap = false } = opts;
  const base = { cwd, timeout, shell: false, ...(env ? { env } : {}) };
  const isWin = process.platform === 'win32';
  let outPath = null, errPath = null, outFd = null, errFd = null, unlinked = false;
  try {
    const stem = path.join(os.tmpdir(),
      `aitri-capture-${process.pid}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`);
    outPath = `${stem}.out`; errPath = `${stem}.err`;
    // 'w+' (not 'w'): the same fd is written by the child and read back here, so it must
    // be readable — a write-only fd fails every read with EBADF.
    outFd = fs.openSync(outPath, 'w+');
    errFd = fs.openSync(errPath, 'w+');
    if (!isWin) {
      // Unlink NOW, while the fds stay valid (POSIX). Nothing can leak afterwards: not a
      // crash, not the OOM kill or the Ctrl-C that terminates Aitri without running any
      // `finally`, and not an orphan that keeps writing — it writes into an inode with no
      // name, reclaimed when the last fd closes. Windows cannot unlink an open file, so
      // there the paths are removed in the finally below (best effort: a surviving
      // grandchild holding the handle can defeat it — a known, documented Windows gap).
      fs.unlinkSync(outPath); fs.unlinkSync(errPath); unlinked = true;
    }
  } catch {
    // No writable tmpdir (read-only /tmp, sandbox) — capture through pipes instead of
    // refusing to run. NOTE this is close to, but not identical to, the pre-rc.10
    // behaviour: the gates used to inherit Node's 1 MiB default and now get `readCap`
    // here too, so the fallback is MORE tolerant of loud gates than the old pipe path,
    // never less. Everything else (kill detection, exit codes) is unchanged.
    for (const fd of [outFd, errFd]) { if (fd !== null) { try { fs.closeSync(fd); } catch { /* noop */ } } }
    for (const f of [outPath, errPath]) { if (f) { try { fs.unlinkSync(f); } catch { /* never created */ } } }
    const r = spawnSync(bin, args, { ...base, encoding: 'utf8', maxBuffer: readCap });
    return { ...r, truncated: false, readFailed: false, captureMode: 'pipe' };
  }
  try {
    const r = spawnSync(bin, args, { ...base, stdio: ['ignore', outFd, errFd] });
    const out = readCappedFd(outFd, readCap, skipBodyOverCap);
    const errOut = readCappedFd(errFd, readCap, skipBodyOverCap);
    return {
      ...r,
      stdout: out.text,
      stderr: errOut.text,
      truncated: out.truncated || errOut.truncated,
      readFailed: out.failed || errOut.failed,
      captureMode: 'file',
    };
  } finally {
    for (const fd of [outFd, errFd]) { if (fd !== null) { try { fs.closeSync(fd); } catch { /* noop */ } } }
    if (!unlinked) {
      for (const f of [outPath, errPath]) { if (f) { try { fs.unlinkSync(f); } catch { /* held open / already gone */ } } }
    }
  }
}
...
export function runQualityGates(gates, dir) {
  ...
  for (const g of (Array.isArray(gates) ? gates : [])) {
    ...
    // Captured to a file, not a pipe (DISPATCH-PIPE-0903): the gates were the last
    // dispatch still on Node's 1 MiB pipe default, so a gate that merely TALKS a lot —
    // a full e2e suite, a verbose scanner — was killed mid-run and recorded `error`,
    // blocking verify-complete on a check that never actually failed. Judging is
    // unchanged: still the real exit code of a gate that ran to the end.
    const startedAt = Date.now();
    const r = spawnCaptured(resolveWinBin(parts[0]), parts.slice(1), { cwd: dir, timeout: timeoutMs });
    ...
    if (r.readFailed && status !== 'error') {
      // The gate RAN and its exit code is the verdict — only the log could not be read
      // back. Never downgrade a real pass/fail to `error` over lost evidence; do not let
      // the record imply the output was captured either.
      note = `${note}${note ? '\n' : ''}[aitri] gate output could not be read back (temp capture unreadable) — the exit code above is the gate's real verdict, but its log was lost.`;
    }
    if (r.truncated && status !== 'error') {
      // The gate FINISHED and its exit code is real — only Aitri's read of the log was
      // capped. Say so rather than letting a silent truncation look like full output.
      note = `${note}${note ? '\n' : ''}[aitri] gate output exceeded the ${RUNNER_MAX_BUFFER}-byte read cap — only its tail was captured (the exit code above is the gate's real verdict).`;
    }
    const text = [r.stdout || '', r.stderr || '', note].filter(Boolean).join('\n').trim();
    out.push({ name, command: g.command, required, status, exit_code, duration_ms, ...(text ? { output: text.slice(-600) } : {}) });
  }
  return out;
}
...
export function cmdVerifyRun({ dir, args, flagValue, err, featureRoot, scopeName }) {
  ...
  // Verify ref binding (ADR-085 Addendum 2, VERIFY-REF-PRERUN-0911): read HEAD + dirty
  // BEFORE any dispatch. The runner, the e2e auto-run and the gates can take many minutes;
  // a HEAD read only after them bound the verdict to whatever was committed meanwhile, so a
  // commit no test ever ran against read `fresh`. The pre-run ref is the tree the dispatches
  // started from; the post-run read (at stamp time) only widens `dirty` and detects a move.
  const refRootCtx = featureRoot ? verifyRefRootCtx(featureRoot, loadConfig(featureRoot)) : null;
  const refAtStart = captureVerifyRef(dir, config, refRootCtx);
  ...
  // Per-stage wall time (COST-VISIBILITY-0903) — a verify-run that quietly runs the
  // same e2e suite twice is invisible in a pass/fail summary and obvious in a timing one.
  let runnerMs = null, e2eMs = null, gatesMs = null;
  ...
  if (allManual) {
    ...
    const result = spawnCaptured(resolveWinBin(cmdParts[0]), cmdParts.slice(1), {
      cwd: dir,
      timeout: runnerTimeoutMs,
      readCap: RUNNER_MAX_BUFFER,
      // An over-cap runner log is refused below, so reading its tail back would cost ~2x
      // the cap in RSS for output nothing consumes — on a machine already short of memory.
      skipBodyOverCap: true,
    });
    ...
    // The suite now RUNS TO THE END even when it is loud (no pipe ceiling to be killed
    // over) — but if its log outgrew the read cap, Aitri saw only the tail and TC markers
    // before the cut are invisible. Parsing that would credit fewer TCs than really
    // passed and flip verifyPassed on a healthy suite: the phantom regression the old
    // ENOBUFS refusal existed to prevent. Same doctrine, same refusal — the difference is
    // that the run was not murdered halfway to get here.
    // A capture that could not be READ is not an empty run. Crediting it would mark every
    // TC skipped and flip verifyPassed on a suite that may have been perfectly green —
    // the phantom-regression class, with no diagnostic. Refuse, like every other
    // did-not-finish path, and name the cause.
    if (result.readFailed) {
      err(
        `Test runner ran, but Aitri could not read back its captured output.\n` +
        `04_TEST_RESULTS.json was NOT written and verifyPassed was left unchanged — an unreadable ` +
        `capture is not an empty run, and crediting it would report a phantom regression.\n` +
        `Check that the temp directory (${os.tmpdir()}) is writable and has free space, then re-run.`
      );
    }

    if (result.truncated) {
      err(
        `Test runner produced more output than the ${RUNNER_MAX_BUFFER}-byte read cap, so only its tail was captured.\n` +
        `04_TEST_RESULTS.json was NOT written and verifyPassed was left unchanged — crediting a partially ` +
        `read run would under-count passing TCs and surface a phantom regression.\n` +
        `Reduce the runner's output volume (quiet/summary reporter flags, or write per-test results to a ` +
        `file and pass it with --results).`
      );
    }
  ...
  // Provenance for the artifact: the e2e output was not fully seen (over the read cap, or
  // unreadable). Not a kill — the dispatch finished — but a consumer must be able to tell
  // that unseen TCs are unseen, not absent. Pre-rc.10 an over-cap e2e log went down the
  // ENOBUFS kill branch and stamped `e2e_run`; that provenance must not silently vanish.
  let e2eOutputIncomplete = false;
  let playwrightExitCode = null;
  let e2eRunKilled = null;
  ...
  if (!allManual && hasPwConfig) {
    {
      ...
      const pwResult = spawnCaptured(resolveWinBin('npx'), ['playwright', 'test'], {
        cwd: pwRoot,
        timeout: runnerTimeoutMs,
        readCap: RUNNER_MAX_BUFFER,
      });
      e2eMs = Date.now() - e2eStartedAt;
      // A KILL (timeout, buffer overflow, signal) is "did not finish", not failing e2e
      // tests — but unlike the MAIN runner's kill (which refuses without writing,
      // ADR-068: its output is truncated mid-run and parsing it would fabricate
      // results), the unit run here already FINISHED and parsed cleanly before this
      // dispatch. Pre-rc.8 this branch err()'d and discarded those finished unit
      // results, deadlocking verify → verify-complete → reconcile --resolve on any
      // project whose e2e cannot run in the current environment (FB-E2E-KILL-0722,
      // T-Ledger: 211/211 unit greens unrecordable). Boundary refinement of ADR-068
      // (Addendum): the killed AUXILIARY dispatch contributes nothing — none of its
      // partial output is parsed (zero false-pass surface), every e2e TC records as
      // skipped — while the finished unit results persist. verify-complete's e2e
      // coverage gate still hard-blocks until e2e evidence exists (runner pass or
      // mark-manual + `tc verify --evidence`), so nothing here weakens the spine.
      if (pwResult.signal || pwResult.error) {
        const timedOut   = pwResult.error && pwResult.error.code === 'ETIMEDOUT';
        const overBuffer = pwResult.error && pwResult.error.code === 'ENOBUFS';
        const neverRan   = pwResult.error && pwResult.error.code === 'ENOENT';
        const reason = timedOut  ? `exceeded its ${runnerTimeoutMs}ms timeout`
                     : neverRan  ? `could not start — 'npx' was not found on PATH`
                     : `was killed before finishing (${pwResult.error?.code || `signal ${pwResult.signal}`})`;
        e2eRunKilled = {
          runner: 'playwright',
          status: 'killed',
          reason: timedOut ? 'timeout' : (pwResult.error?.code || `signal ${pwResult.signal}`),
        };
        // First exit varies by kill cause — "raise the timeout" cannot fix a buffer
        // overflow, and a dispatch that never started needs npx, not patience.
        const firstExit = timedOut
          ? `    - Raise "test_runner_timeout_ms" (ms) in 04_BUILD_REPORT.json if the e2e suite\n` +
            `      legitimately needs longer — default is ${RUNNER_TIMEOUT_DEFAULT_MS} (15 min).\n`
          : overBuffer
          ? `    - Reduce the e2e runner's output volume (quiet/summary reporter flags, log to a file).\n`
          : neverRan
          ? `    - Ensure Node/npx is on PATH in this environment (the dispatch runs 'npx playwright test').\n`
          : `    - Investigate why the process was terminated (OS signal/OOM); reduce output verbosity if it recurs.\n`;
        // Sanctioned downgrade route (audit-stamped), NOT a hand-edit of 03_TEST_CASES.json:
        // mark-manual records --reason + downgraded_from; tc verify demands evidence + notes.
        const tcMarkManualCmd = featureRoot ? `aitri feature tc ${scopeName} mark-manual` : `aitri tc mark-manual`;
        process.stderr.write(
          `[aitri] ⚠ Playwright E2E run ${reason} and did not finish.\n` +
          `  This is a killed run, not failing e2e tests. The finished unit-runner results ARE\n` +
          `  preserved and written; every e2e TC is recorded as skipped (no partial e2e output\n` +
          `  is trusted). verify-complete will still require e2e evidence. Your exits:\n` +
          firstExit +
          `    - If e2e cannot run in THIS environment: downgrade each TC with\n` +
          `      '${tcMarkManualCmd} <id> --reason "<why e2e cannot run here>"', then record it with\n` +
          `      '${tcVerifyCmd} <id> --result pass --evidence <path> --notes "<what the evidence shows>"'\n` +
          `      using output from an environment where the e2e DOES run (CI, another machine).\n`
        );
      } else {
        if (pwResult.truncated || pwResult.readFailed) e2eOutputIncomplete = true;
        if (pwResult.readFailed) {
          process.stderr.write(
            `[aitri] ⚠ The Playwright dispatch ran, but Aitri could not read back its captured output — ` +
            `its e2e TCs record as SKIPPED (never as passed), so verify-complete still blocks on them. ` +
            `Check that ${os.tmpdir()} is writable and has free space.\n`
          );
        }
        if (pwResult.truncated) {
          process.stderr.write(
            `[aitri] ⚠ The Playwright dispatch produced more output than the ${RUNNER_MAX_BUFFER}-byte read cap — ` +
            `only its tail was captured. e2e TCs whose result lines fell before the cut record as SKIPPED (never as ` +
            `passed), so verify-complete still blocks on them. Reduce the e2e reporter's verbosity to credit them.\n`
          );
        }
  ...
  // Cross-pipeline TC-id collisions (TC-ID-COLLISION-0911, ADR-092). Phase 3 refuses NEW
  // collisions; a plan approved before that gate may still carry them, and a result parsed
  // for such an id may be the other pipeline's test. Advisory, so existing projects are not
  // walled — renaming routes through Phase 3, where the gate then applies.
  {
    const collisions = crossPipelineTCCollisions({
      dir, featureRoot,
      ids: testCaseList
        .filter(tc => tc && typeof tc.id === 'string' && tc.automation !== 'manual')
        .map(tc => tc.id),
    });
    if (collisions.length) {
      const shown = collisions.slice(0, 10).map(describeCollision).join('; ');
      process.stderr.write(
        `[aitri] Warning: ${collisions.length} planned TC id(s) are also planned by another pipeline of this project: ` +
        `${shown}${collisions.length > 10 ? `; … and ${collisions.length - 10} more` : ''}. ` +
        `Results are credited by TC id alone — if this run's runner also executes that pipeline's tests, its ` +
        `result can be credited to this plan (a false pass, or a failure this TC never owned). Rename one side to ` +
        `a pipeline namespace (e.g. TC-<FEATURE>-001h); Phase 3 refuses new collisions.\n`
      );
    }
  }
  ...
  gatesMs = Date.now() - gatesStartedAt;
  ...
  const testResults = {
    ...
    // Additive (FB-E2E-KILL-0722, rc.8): present only when the auto-run e2e dispatch was
    // KILLED (timeout/signal/buffer). Informational — records that the skipped e2e TCs in
    // this run come from a killed dispatch, not from missing tests. Cleared naturally by
    // the next verify-run (this object is rebuilt from scratch).
    ...(e2eRunKilled ? { e2e_run: e2eRunKilled } : {}),
    results,
    ...
    // Additive (DISPATCH-PIPE-0903): the e2e dispatch FINISHED but its output was not
    // fully seen (past the read cap, or unreadable) — TCs whose lines fell outside record
    // as skipped, never as passed. Absent when the output was fully captured.
    ...(e2eOutputIncomplete ? { e2e_output_incomplete: true } : {}),
    // Additive (COST-VISIBILITY-0903): wall time per dispatch stage, in ms. A verify-run
    // that runs the same e2e suite twice — once as the auto-run, once as a declared gate —
    // is invisible in a pass/fail summary and unmissable here. `e2e_ms` is present only
    // when the Playwright auto-run dispatched; per-gate cost is `quality_gates[].duration_ms`.
    timings: {
      ...(runnerMs !== null ? { runner_ms: runnerMs } : {}),
      ...(e2eMs !== null ? { e2e_ms: e2eMs } : {}),
      ...(gatesMs !== null ? { gates_ms: gatesMs } : {}),
      total_ms: Date.now() - verifyStartedAt,
    },
  };
  ...
  // M1 (ADR-085): bind this verdict to the code state it certified. SHARED fields —
  // the binding travels with every clone/worktree, unlike the per-machine reconcile
  // baseline. Non-git projects get no stamp (refState 'unbound' → pre-M1 behavior).
  // A dirty stamp (uncommitted behavioral files at run time) marks the verdict
  // non-portable and prints the commit-then-verify teaching below.
  // The ref is the one read BEFORE the dispatches (refAtStart, above) — the tree the run
  // actually tested. `dirty` is true if the tree was dirty at either end: code that was
  // uncommitted when the run started, or that changed while it ran, was possibly what the
  // tests saw. A post-run read that fails counts as dirty (unknown is not clean). A moved
  // HEAD needs no special stamp: freshness diffs ref..HEAD, so a behavioral commit made
  // mid-run reads `stale` and a docs-only one stays `fresh`.
  // If ONE of the two reads fails (git broken or unreadable at that moment) the stamp still
  // binds — to whichever ref was read — but as dirty: an unknown tree is not a clean one, at
  // either end. Only when both fail (not a git project) is there no stamp.
  const refAtEnd = captureVerifyRef(dir, config, refRootCtx);
  const refUnreadableAt = (refAtStart && !refAtEnd) ? 'end' : ((!refAtStart && refAtEnd) ? 'start' : null);
  const refTreeDirty = !!(refAtStart?.dirty || refAtEnd?.dirty);
  const refBase = refAtStart || refAtEnd;
  const refStamp = refBase ? { ref: refBase.ref, dirty: refTreeDirty || refUnreadableAt !== null } : null;
  const headMovedTo = (refAtStart && refAtEnd && refAtEnd.ref !== refAtStart.ref) ? refAtEnd.ref : null;
  if (refStamp) {
    config.verifyRanRef   = refStamp.ref;
    config.verifyRanDirty = refStamp.dirty;
  } else {
    delete config.verifyRanRef;
    delete config.verifyRanDirty;
  }
  ...
  // FB-E2E-KILL-0722 (rc.8): a units-green + e2e-killed run must ALSO reset a stale
  // verifyPassed — otherwise reconcile --resolve could ride an earlier full-green
  // verify-complete while the current run's e2e evidence is a killed dispatch.
  const wouldNotPassComplete =
    (summary.passed === 0 && summary.skipped > 0) || summary.failed > 0 || requiredGateFailed || !!e2eRunKilled;
  ...
  // M1 (ADR-085): a dirty-stamped verdict is machine-local — say so at the moment it
  // is created, with the discipline that makes the next one portable.
  if (refStamp && refTreeDirty) {
    process.stderr.write(
      `[aitri] ⚠ verify ran over UNCOMMITTED behavioral changes — this verdict is bound to this\n` +
      `        working tree only (non-portable: other checkouts cannot trust it). For a portable\n` +
      `        verdict: commit first, then verify-run — the same clean-tree discipline reconcile\n` +
      `        --resolve already requires.\n`
    );
  }
  if (refUnreadableAt) {
    process.stderr.write(
      `[aitri] ⚠ Could not read the git state at the ${refUnreadableAt} of this run — the verdict is bound to\n` +
      `        ${refStamp.ref.slice(0, 7)} but marked non-portable (dirty): an unknown working tree is not a clean one.\n` +
      `        Common causes: git failing in this checkout, or no commit existing yet when the run started.\n` +
      `        Re-run verify-run for a portable verdict.\n`
    );
  }
  if (headMovedTo) {
    process.stderr.write(
      `[aitri] ⚠ HEAD moved while verify-run was running (${refStamp.ref.slice(0, 7)} → ${headMovedTo.slice(0, 7)}).\n` +
      `        This verdict is bound to ${refStamp.ref.slice(0, 7)}, the commit the run started on — not to the\n` +
      `        new HEAD. If the new commit(s) changed behavioral code, status shows the verdict stale:\n` +
      `        re-run verify-run on the current HEAD. Do not commit while a verify-run is in flight.\n`
    );
  }
  ...
  const displayOutput = isTruncated
    ...
    : (output || '(no output)');
  const pwLines = (playwrightOutput || '').split('\n');
  const pwDisplayOutput = pwLines.length > OUTPUT_MAX_LINES
    ? pwLines.slice(0, OUTPUT_MAX_LINES).join('\n') +
      `\n... (${pwLines.length - OUTPUT_MAX_LINES} more lines truncated — full output in your terminal)`
    : playwrightOutput;
  ...
  const fmtMs = (ms) => (ms >= 60000 ? `${Math.floor(ms / 60000)}m${String(Math.round((ms % 60000) / 1000)).padStart(2, '0')}s` : `${(ms / 1000).toFixed(1)}s`);
  const slowestGates = [...qualityGates]
    .filter(g => typeof g.duration_ms === 'number')
    .sort((a, b) => b.duration_ms - a.duration_ms)
    .slice(0, 3);
  const costLines = (testResults.timings && testResults.timings.total_ms > 0) ? [
    `## Cost — ${fmtMs(testResults.timings.total_ms)} total`,
    ...(typeof testResults.timings.runner_ms === 'number' ? [`  runner:       ${fmtMs(testResults.timings.runner_ms)}  (${testCmd})`] : []),
    ...(typeof testResults.timings.e2e_ms === 'number' ? [`  e2e auto-run: ${fmtMs(testResults.timings.e2e_ms)}  (npx playwright test, from the project root)`] : []),
    ...(typeof testResults.timings.gates_ms === 'number' ? [`  gates:        ${fmtMs(testResults.timings.gates_ms)}${slowestGates.length ? `  (slowest: ${slowestGates.map(g => `${g.name} ${fmtMs(g.duration_ms)}`).join(', ')})` : ''}`] : []),
  ] : [];
  ...
  console.log([
    ...
    // FB-E2E-KILL-0722: the kill must be visible in the briefing itself, not only on
    // stderr — an agent consuming this markdown would otherwise read an ordinary skip.
    ...(e2eRunKilled ? [
      ``,
      `## ⚠ E2E dispatch killed (${e2eRunKilled.reason})`,
      `  The Playwright auto-run did not finish — the e2e TCs above are skipped because the`,
      `  dispatch was killed, not because tests are missing. verify-complete will still require`,
      `  e2e evidence: raise test_runner_timeout_ms, or mark-manual + evidence-verify those TCs`,
      `  (exact commands printed on stderr by this run).`,
    ] : []),
    ...
    // COST-VISIBILITY-0903: what the run COST, per stage. A battery that grew one gate
    // at a time — and an e2e suite the auto-run and a declared gate both execute — is
    // invisible in a pass/fail summary and plain here. Read-only, never a gate.
    ...
    ...(playwrightOutput ? [
      ...
      `\`\`\``,
      // Capped like the runner block above. Pre-rc.10 an oversized e2e log could not reach
      // here (the pipe ceiling killed the dispatch first); with file capture it can, and
      // dumping a 64 MiB tail into a summary an agent reads helps nobody.
      pwDisplayOutput,
      `\`\`\``,
    ] : []),
    ``,
...
export function cmdVerifyComplete({ dir, err, featureRoot, scopeName, VERSION }) {
  ...
  //
  // GATE-SCOPE-BLIND-0805 (ADR-086): at ROOT scope the input set is ALL scopes — the
  // whole tree ships together, and Phase 5's sealing path gates on verifyPassed alone,
  // so a critical FEATURE bug must stop the root gate (the contract AGENTS.md and the
  // deploy spec always stated; only the input was scope-local). A FEATURE's own gate
  // stays scope-local: root bugs never block a feature's verification.
  if (featureRoot) {
    ...
    if (blockingBugs.length) {
      ...
      err(`Open critical/high bug(s) must be resolved before Phase 5:\n${list}\n\nRun: aitri feature bug ${scopeName} fix <id> [--tc TC-NNN]  then  aitri feature bug ${scopeName} verify <id>`);
    }
  } else {
    const blockers = getBlockingBugsAllScopes(dir, config, err);
    if (blockers.length) {
      const list = blockers.map(({ scope, bug: b }) =>
        `  ✗ ${b.id} [${b.severity}] (${scope}): ${b.title}${b.fr ? ` (${b.fr})` : ''}`).join('\n');
      const fixes = [...new Set(blockers.map(({ scopeName: n }) =>
        n ? `aitri feature bug ${n} fix <id>` : 'aitri bug fix <id>'))].join('   |   ');
      err(`Open critical/high bug(s) must be resolved before Phase 5 — the whole tree ships together:\n${list}\n\nRun: ${fixes}  then verify each fix with the same scope's bug verify <id>`);
    }
  }

  // M1 (ADR-085): refuse to bless a run whose certified tree is not THIS tree.
  // 'stale' = behavioral code changed since verify-run (the results no longer describe
  // HEAD); 'unreachable' = the certified commit is gone (rebase/squash) — B4 doctrine,
  // refuse-not-pass. 'dirty' passes with the non-portable warning already printed at
  // run time; 'unbound' (non-git / pre-M1 stamp) degrades to pre-M1 behavior.
  {
    const refState = verifyRefFreshness(dir, config,
      featureRoot ? verifyRefRootCtx(featureRoot, loadConfig(featureRoot)) : null);
    if (refState === 'stale') {
      err(
        `Behavioral code changed since this verify-run (merge or post-verify commit) —\n` +
        `the results describe an earlier tree, not HEAD.\n\n` +
        `Run: aitri verify-run   then   aitri verify-complete`
      );
    } else if (refState === 'unreachable') {
      err(
        `The commit this verify-run certified is not in the current history (rebase/squash?).\n` +
        `The verdict cannot be bound to this tree.\n\n` +
        `Run: aitri verify-run   then   aitri verify-complete`
      );
    }
  }
  ...
  // BUG-STATE-VISIBLE-0909: a feature seal is the moment a root backlog item it
  // delivered should close — nothing links the two records, and a field project carried
  // a delivered P1 item as open for a week because the closure is manual and happens
  // weeks after the item was read. One advisory line, only when there is something open.
  // No auto-close: one item can be delivered across several features (N:M), so a
  // mechanical close at the first seal would be a false close — worse than a stale open.
  if (scopeName && featureRoot) {
    const openRootBacklog = countOpenBacklog(featureRoot);
    if (openRootBacklog > 0) {
      console.log(
        `📋 ${openRootBacklog} open backlog item(s) in the root project — if this feature delivered any, close them now:\n` +
        `   aitri backlog          (list)      aitri backlog done <BL-NNN>      (close)`
      );
    }
  }
  console.log(`\n${bar}`);
