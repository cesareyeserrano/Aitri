...
export function runQualityGates(gates, dir) {
  ...
  for (const g of (Array.isArray(gates) ? gates : [])) {
    ...
    } else if (r.signal || r.error?.code === 'ETIMEDOUT' || r.error?.code === 'ENOBUFS') {
      // killed (timed out at timeoutMs, or overflowed the capture buffer) — surface as
      // `error`, not `fail`, so a too-short timeout reads as "raise timeout_ms", not
  ...
  if (Array.isArray(d.quality_gates)) {
    ...
    if (failedGates.length) {
      ...
      const reasonFor = (g) => {
        ...
        const gateNote = outStr.split('\n').find(l => l.includes('gate did not finish within') || l.includes('gate could not start'));
