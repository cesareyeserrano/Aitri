...
import { loadConfig, saveConfig, readArtifact, readArtifactFile, artifactPath, appendEvent, writeLastSession, atomicWrite } from '../state.js';
...
export function cmdVerifyRun({ dir, args, flagValue, err, featureRoot, scopeName }) {
  ...
  atomicWrite(resultsPath, JSON.stringify(testResults, null, 2));
...
export function cmdVerifyComplete({ dir, err, featureRoot, scopeName, VERSION }) {
  ...
  if (failedStubs.length) {
    ...
    atomicWrite(resultsPath, JSON.stringify(d, null, 2));
