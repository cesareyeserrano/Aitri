...
import { loadConfig, atomicWrite, saveConfig, writeLastSession, configExists } from '../state.js';
...
/**
 * Stamp `lastSession` after a backlog mutation (BUG-STATE-VISIBLE-0909). `resume` flags the
 * saved narrative as stale when a later action post-dates it — but only approve/complete/
 * verify/reconcile wrote lastSession, so `backlog done` (the very command that invalidates a
 * narrative's "BL-040 pending") never tripped the flag. Per-machine field (.aitri.local);
 * the shared .aitri is untouched. Never fatal — the backlog write already succeeded.
 */
function touchSession(dir, config, event) {
  // Never CREATE state (saveConfig writes a fresh .aitri when none exists) — see bug.js.
  if (!configExists(dir)) return;
  try { writeLastSession(config, dir, event); saveConfig(dir, config); } catch { /* advisory stamp only */ }
}

/**
 * Open backlog items in `dir` (root or feature), tolerant: 0 when the file is absent,
 * malformed, or the project unreadable. Used by the feature-seal advisory in verify-complete.
 * @param {string} dir
 * @returns {number}
 */
export function countOpenBacklog(dir) {
  try {
    const config = loadConfig(dir);
    if (!config) return 0;
    const data = readBacklog(backlogPath(dir, config));
    if (isMalformedBacklog(data)) return 0;
    return data.items.filter(i => !isClosed(i)).length;
  } catch { return 0; }
}
...
function note(dir, config, id, flagValue, err, backlogCmd = 'aitri backlog') {
  ...
  item.updatedAt = at;
  writeBacklog(fp, data);
  touchSession(dir, config, `backlog note ${id}`);
...
function update(dir, config, id, args, flagValue, err, backlogCmd = 'aitri backlog') {
  ...
  writeBacklog(fp, data);
  touchSession(dir, config, `backlog update ${id}`);
...
function done(dir, config, id, err, backlogCmd = 'aitri backlog') {
  ...
  writeBacklog(fp, data);
  touchSession(dir, config, `backlog done ${id}`);
