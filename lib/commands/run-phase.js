...
export function cmdRunPhase({ dir, args, flagValue, err, rootDir, featureRoot, scopeName, _readLine }) {
  ...
  // Scan the context folder — list any assets so the agent can reference them.
  ...
  if (fs.existsSync(ctxDir)) {
    ...
    if (assets.length) {
      // Staleness framing: these are supporting reference, NOT the source of truth.
      // The approved artifacts win; obsolete material as the project evolves is skipped.
  ...
  // Loud migration hint (rc.37+ rename): pre-rename projects keep assets in idea/,
  // which is no longer scanned. Tell the operator rather than silently dropping context.
  ...
  console.log(p.buildBriefing({ dir, inputs, feedback, failingTests, artifactsBase, bestPractices, interviewContext, config, scopeVerb: sv, scopeArg: sa }));
