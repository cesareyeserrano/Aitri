...
export function cmdRunPhase({ dir, args, flagValue, err, rootDir, featureRoot, scopeName, _readLine }) {
  ...
  if (phase === 1) {
    ...
    if (!hasCurrentReqs) {
      const idea = readArtifact(dir, 'IDEA.md', productSubdir(config)) || '';
  ...
  for (const filename of p.inputs) {
    // IDEA.md lives at the unit root ('' flat / 'aitri/product' contained) —
    // never in the artifacts subdirectory
    const adir = filename === 'IDEA.md' ? productSubdir(config) : artifactsDir;
    ...
    if (fs.existsSync(ctxDir)) {
      ...
      if (assets.length) {
        assetsNote = `\n── Additional context (${ctxRel}/ folder) ──────────────\n` +
          `Supporting reference material is available in ${ctxRel}/. The approved\n` +
