...
export function cmdRunPhase({ dir, args, flagValue, err, rootDir, featureRoot, scopeName, _readLine }) {
  ...
  // AUDIT-0630-B: the visual-fidelity exception to "no raw context at build". The deliberate
  // exclusion above bet the agent would "read the mockup on request" (see the CONTEXT_PHASES comment)
  // — the Ledger pilot falsified it: a UI built from tokens, ignoring the designated mockups, passed
  // every gate. So when building a UI surface (a UX spec exists), surface the VISUAL assets WITH their
  // paths — only the images Aitri can't read, not the whole raw context (respecting the anti-staleness
  // intent) — so the build briefing's "open the referenced mockups" instruction (rc.137) is actionable.
  ...
  if (phase === 4 && readArtifact(dir, '01_UX_SPEC.md', artifactsDir)) {
    const visSub  = featureRoot ? 'feature_context' : ideaContextSubdir(config);
    const all     = listAssets(dir, visSub);
    // Images (visualAssets) AND .html/.htm prototypes — both are the designated visual reference the
    // agent must open for fidelity. The Ledger pilot's reference included an offline .html demo, which
    // visualAssets() excludes as "readable text"; surface it here too. Build-local widening only — the
    // global visualAssets (used by the requirements asset advisory) stays "binary Aitri can't read".
    const visuals = [...visualAssets(all), ...all.filter(f => /\.html?$/i.test(f))];
    if (visuals.length) {
      assetsNote += `\n── Visual reference (open these — structural tests do NOT verify fidelity) ──\n` +
        `The approved UX/design spec was built against these visual assets. Open each and match its\n` +
        `fidelity (color, layout, affordances) — tokens alone are not the design:\n\n` +
        visuals.map(f => `  ${visSub}/${f}`).join('\n') + '\n';
    }
  }
