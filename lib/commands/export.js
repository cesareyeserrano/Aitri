...
// Reuse the requirement/compliance predicate SSoTs instead of reimplementing them inline — a private
// copy drifted from the Phase-5 deploy gate and made this document contradict it (AUDIT-0629-E/F).
...
export function buildTraceabilityMarkdown(dir, config) {
  ...
  const addReq = (r, kind) => {
    ...
    const must = isMustRequirement(r);
    ...
    // H1: cross-check the compliance level against the SAME evidence rule the Phase-5 deploy gate
    // enforces (HIGH_COMPLIANCE_LEVELS / EVIDENCE_OK_STATUSES SSoT) — a "complete" over a non-covered
    // status (incl. a pending "manual") is flagged here exactly as `complete 5` rejects it, never
    // printed as authoritative QA truth.
    let level = comp?.level || '—';
    if (comp && HIGH_COMPLIANCE_LEVELS.has(level)) {
      const backed = cov && EVIDENCE_OK_STATUSES.has(cov.status);
      if (!backed) level = `⚠ ${level} (vs tests)`;
    }

    rows.push([
      r.id, r.title || r.requirement || '',
      r.priority || (isMustRequirement(r) ? 'MUST (reg)' : '—'),
