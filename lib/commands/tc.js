...
import { loadConfig, saveConfig, artifactPath, readArtifactFile, atomicWrite, hashResultsFile } from '../state.js';
...
function applyResult(d, entry, result, notes, evidence) {
  // Audit trail (AUDIT-0629-A, parity with mark-manual's ADV-0622-05 guard): overriding a TC
  // the runner recorded as fail/skip to a DIFFERENT result is the laundering move (flip a failing
  // automated test, then self-attest it passed). `tc verify` already forces `--notes` (mandatory
  // justification), but it left no machine-readable trace and overwrote the prior verdict's notes —
  // so a reviewer could not tell a justified manual pass from a laundered failure. Stamp the prior
  // verdict (mirrors the `downgraded_from` mark-manual stamps on the TC) so the override is never
  // silent. Computed from entry.status BEFORE it is overwritten, here in the shared recorder so both
  // the explicit and the guided checklist paths leave the trail. The harness promise is "nothing
  // silently dropped"; this keeps the dropped failure visible.
  const prior = entry.status;
  if ((prior === 'fail' || prior === 'skip') && prior !== result) {
    entry.downgraded_from = prior;
  } else if ('downgraded_from' in entry) {
    // A re-override whose prior verdict was NOT a fail/skip means the current status was not
    // reached by laundering a failing run — clear any stale stamp so the trail never contradicts
    // the live verdict (e.g. a later pass→fail must not keep an old downgraded_from:'fail').
    delete entry.downgraded_from;
  }
  entry.status            = result;
...
function tcVerify({ dir, args, flagValue, err, featureRoot, scopeName }) {
  ...
  // Run-binding (UPLAN-0703 B2): tc verify is an authorized writer of the results file — it
  // applies one entry and re-stamps `verifyResultsHash` over the WHOLE file. If it re-stamped
  // blindly, that re-stamp would bless content no sanctioned run produced, in two ways:
  //   - laundering: hand-edit every fail→pass in a bound file, then one legitimate tc verify
  //     re-stamps the whole tampered file;
  //   - fabrication: hand-write an entire green results file (never run verify-run), include
  //     one manual entry, and tc verify stamps it — bypassing the B1 stamp requirement at
  //     verify-complete with a single extra command.
  // So tc verify holds the same line as verify-complete: NO stamp → refuse (verify-run is the
  // only entry point that creates a bound results file — since rc.129 it seeds AND stamps the
  // all-manual project too, so a sanctioned stamp-less file does not exist on rc.129+);
  // stamp present but file edited since → refuse. Only a bound, unmodified file may receive a
  // new entry and be re-stamped.
  if (!config.verifyResultsHash)
    err(
      `No verify-run is recorded for this project — recording a result would stamp a results\n` +
      `  file no run produced. Run: ${c.verifyRun} first (it seeds and binds 04_TEST_RESULTS.json,\n` +
      `  including for all-manual projects), then verify each TC.`
    );
  if (hashResultsFile(resultsContent) !== config.verifyResultsHash)
    err(
      ...
      `  It was edited or replaced after the tests ran — recording a result would re-stamp the\n` +
      `  tampered file. Re-run: ${c.verifyRun} to regenerate it from a real execution, then verify.`
    );
  ...
  // AUDIT-0629-A: make the override of a prior failing/skipped verdict visible, not just traced in
  // the JSON. A reviewer should see at the point of action that a recorded fail/skip was overridden.
  if (entry.downgraded_from)
    process.stderr.write(
      `   ⚠ overrode a prior "${entry.downgraded_from}" result — stamped downgraded_from for the reviewer.\n`
    );
...
function tcVerifyGuided({ dir, err, featureRoot, scopeName }) {
  ...
  // Run-binding (UPLAN-0703 B2): the guided loop also re-stamps after applying results, so it
  // holds the same line as the scriptable path above — no stamp → refuse (a stamp-less file
  // was not produced by any run; stamping it would launder a fabrication), edited-since-stamp
  // → refuse (would re-bless the edit).
  if (!config.verifyResultsHash)
    err(
      `No verify-run is recorded for this project — recording results would stamp a results\n` +
      `  file no run produced. Run: ${c.verifyRun} first (it seeds and binds 04_TEST_RESULTS.json,\n` +
      `  including for all-manual projects), then verify each TC.`
    );
  if (hashResultsFile(resultsContent) !== config.verifyResultsHash)
    err(
      `04_TEST_RESULTS.json does not match the last verify-run (results-hash mismatch).\n` +
      `  It was edited or replaced after the tests ran — recording results would re-stamp the\n` +
      `  tampered file. Re-run: ${c.verifyRun} to regenerate it from a real execution, then verify.`
    );
  ...
  if (done > 0) {
    ...
    atomicWrite(resultsPath, resultsJson);
    config.verifySummary = d.summary;
    config.verifyResultsHash = hashResultsFile(resultsJson); // re-stamp run-binding (Finding 1)
    saveConfig(dir, config);
  }
