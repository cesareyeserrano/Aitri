...
import { isAitriStatePath } from '../reconcile-patterns.js';
...
function gitDiffFiles(dir, fromSha, toSha) {
  ...
  try {
    ...
    return out.split('\n').filter(f => f && !isAitriStatePath(f, config));
  } catch { return []; }
}
...
export function getBlockingBugs(dir, config) {
  ...
  try {
    const data = loadBugsFile(p);
    // AUDIT-0629-B: case-fold at the blocking-decision read — this is the feature deploy gate
    // (verify-complete → getBlockingBugs). A hand-written "Critical"/"Open" in BUGS.json must not
    // slip a real blocking bug past the gate. Mirrors the same normalization in snapshot.aggregateBugs.
    return (data.bugs || []).filter(b => {
      const status   = String(b.status   || '').toLowerCase();
      const severity = String(b.severity || '').toLowerCase();
      return (status === 'open' || status === 'in_progress') && (severity === 'critical' || severity === 'high');
    });
  } catch { return []; }
}
...
export function cmdBug({ dir, args = [], err, featureRoot, scopeName }) {
  ...
  if (sub === 'fix') {
    const id  = args[1];
    const tc  = flagVal(args, '--tc');
    if (!id) err(`Usage: ${bugCmd} fix <BG-NNN> [--tc TC-NNN]`);
    const data = loadBugsFile(fp);
    ...
    console.log(`✅ ${id} → fixed${tc ? `  (TC: ${tc})` : ''}${shaNote}`);
  ...
  if (sub === 'verify') {
    ...
    if (!bug) err(`Bug ${id} not found`);
    if (bug.status !== 'fixed') {
      process.stderr.write(`  ℹ  ${id} is ${bug.status} — run ${bugCmd} fix ${id} first, then verify.\n`);
    }
    bug.status     = 'verified';
    bug.updated_at = new Date().toISOString();
    saveBugsFile(fp, data);
  ...
  if (sub === 'close') {
    const id   = args[1];
    if (!id) err(`Usage: ${bugCmd} close <BG-NNN>`);
    const data = loadBugsFile(fp);
    const bug  = (data.bugs || []).find(b => b.id === id);
    if (!bug) err(`Bug ${id} not found`);
    bug.status     = 'closed';
