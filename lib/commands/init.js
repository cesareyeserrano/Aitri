...
import {
  loadConfig, saveConfig, configExists, readArtifact,
...
export function cmdInit({ dir, rootDir, err, VERSION }) {
  ...
  // 3.2 (UX-PRO-0707): on re-init of a project whose seed was already absorbed into
  // 01_REQUIREMENTS.json (Phase 1 approved → IDEA.md archived), do NOT recreate a fresh
  // template seed — it would sit next to a pipeline that will never read it and read as
  // "fresh install". Requirements existing ⇒ the seed's job is done.
  const seedAbsorbed = !isNew && !!readArtifact(dir, '01_REQUIREMENTS.json', config.artifactsDir || '');
  let created = !fs.existsSync(ideaFile) && !seedAbsorbed;
  ...
  ensureAitriGitignore(dir, { extra: [featuresIndexIgnoreEntry(config)] });
  ...
  // dump new-user onboarding — the pipeline is mid-flight. Report the truth (scaffolding /
  // agent files refreshed, state untouched) and point at resume. New projects keep the full
  // onboarding below.
  if (!isNew) {
    console.log(`ℹ️  Already an Aitri project: ${dir}`);
    console.log(`   Agent instruction files and scaffolding refreshed — pipeline state untouched.`);
    if (created)      console.log(`📝 ${ideaFileRel} created (seed brief).`);
    // 3.2 (UX-PRO-0707 follow-up): the root-IDEA absorption (rename into the container,
    // line ~83) also happens on re-init — reporting it only on fresh installs made this
    // branch move a user's file while printing "state untouched".
    else if (absorbedRootIdea) console.log(`📝 ${ideaFileRel} moved from project root (your brief, kept verbatim).`);
    else if (seedAbsorbed) console.log(`   Seed already absorbed into 01_REQUIREMENTS.json — not recreated.`);
    const legacyIdeaReinit = listAssets(dir, 'idea');
    if (legacyIdeaReinit.length) {
      console.log(
        `\n⚠ ${legacyIdeaReinit.length} asset(s) in idea/ are NOT scanned (renamed to idea_context/) — ` +
        `move them:  mv idea/* ${ideaCtxRel}/`
      );
    }
    console.log(`\n  Resume where you left off:  aitri resume   ·   aitri status`);
    return;
  }
...
What to do now:
  ...
  Claude Code users also get /aitri (.claude/commands/aitri.md) — one command to advance the pipeline one step.`);
}
