...
function gitFileIsClean(dir, artifactRelPath) {
  try {
    // `git diff HEAD` is EMPTY for an untracked file (git does not diff untracked
    // paths) — a false "clean" that would let rehash bookkeep over un-reviewed
    // content. Require the file to be tracked first; ls-files --error-unmatch
    // throws (non-zero) when it is not, falling through to the refuse path.
    execFileSync(
      'git', ['ls-files', '--error-unmatch', '--', artifactRelPath],
      { cwd: dir, stdio: ['ignore', 'ignore', 'ignore'] },
    );
    const out = execFileSync(
      'git', ['diff', 'HEAD', '--', artifactRelPath],
