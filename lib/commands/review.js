  ...
  if ((scope === 'phase2' || scope === 'all') && reqRaw) {
    ...
    if (designRaw) {
      ...
      if (reqs) {
        ...
        // 3.3 #5 (UX-PRO-0707): one line, not one full-sentence warning per FR — `complete 2`
        // used to print N near-identical paragraphs. List the ids; the human looks up detail.
        if (unaddressed.length) {
          const ids = unaddressed.map(fr => fr.id).join(', ');
          const s = unaddressed.length > 1;
          warnings.push(
            `${unaddressed.length} MUST FR${s ? 's' : ''} not referenced by id in 02_SYSTEM_DESIGN.md: ${ids} — ` +
            `confirm the design addresses ${s ? 'them' : 'it'} (id-presence check only, not full design).`
          );
        }
      }
    }
