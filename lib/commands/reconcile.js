 ...
 */
/**
 * Render a refusal's file list, capped. Since the git readers stopped overflowing at 1 MiB
 * (VERIFY-REF-PRERUN-0911), a large untracked tree reaches this list instead of being
 * silently dropped — thousands of lines would bury the one instruction that matters.
 */
export function formatPathList(files, max = 50) {
  const shown = files.slice(0, max).map(f => `  ✗ ${f}`);
  if (files.length > max) shown.push(`  … and ${files.length - max} more`);
  return shown.join('\n');
}
...
function cmdReconcileResolve({ dir, config, err }) {
  ...
  if (method === 'git') {
    ...
    if (uncommitted.length) {
      const list = formatPathList(uncommitted);
      process.stderr.write(
