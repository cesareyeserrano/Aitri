...
import { getBlockingBugs, assertBugsReadable } from './bug.js';
...
function gitChangedFiles(dir, baseRef) {
  // execFileSync (no shell) — baseRef is interpolated; a shell string would be RCE (R3-15).
...
function uncommittedBehavioralChanges(dir) {
  ...
  for (const line of out.split('\n')) {
    ...
    const f = p.replace(/\\/g, '/');
    if (!f) continue;
    if (isAitriStatePath(f, config)) continue;
...
function cmdReconcileResolve({ dir, config, err }) {
  ...
  // Mechanical gate 2: no open critical/high bugs. B8: refuse over an unreadable
  // BUGS.json — a parse failure would empty this gate (false green).
...
export function cmdReconcile({ dir, args = [], err }) {
  ...
  if (files.length === 0) {
    ...
    const { pendingFiles: _drop, ...rest } = ns;
  ...
  const knownPending    = Array.isArray(ns.pendingFiles) ? ns.pendingFiles : null;
