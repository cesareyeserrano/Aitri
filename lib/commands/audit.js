 ...
 *   aitri audit coverage — idea→FR completeness audit (ADR-048)
...
import { loadConfig, saveConfig, readArtifact, ideaPath as resolveIdeaPath, layoutEmissionVars } from '../state.js';
...
function cmdAuditCoverage({ dir, err }) {
  ...
  const briefing = render('phases/auditCoverage', {
    ...
    IDEA:                 idea,
    IDEA_FILE:            layoutEmissionVars(config).IDEA_FILE,
