...
export function buildSecurityNfrSummary(dir, config) {
  ...
  try {
    const nfrs = (JSON.parse(raw).non_functional_requirements || [])
      ...
      // 3.3 #3 (UX-PRO-0707): a security NFR that DECLARES itself not-applicable (e.g.
      // "Not applicable: offline single-user tool, no network, no secrets, no PII") is an
      // exclusion decision, not a promise to protect — it must not trigger the adversarial-
      // audit nudge. Conservative: only drop NFRs whose text OPENS with the exclusion idiom,
      // so a real NFR that merely mentions the phrase in passing still counts.
      ...
      .filter(n => !/^\s*(not[\s-]*applicable|n\/?a\b|does not apply|no aplica)/i.test(`${n.requirement || n.title || ''}`));
