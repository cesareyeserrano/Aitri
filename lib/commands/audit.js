...
import { loadConfig, saveConfig, readArtifact, seedPath, layoutEmissionVars, hashArtifact } from '../state.js';
...
import { activeSecurityNfrs, securityNfrText } from '../requirements.js';
...
export function buildSecurityNfrSummary(dir, config) {
  ...
  if (!raw) return null;
  try {
    // 3.3 #3 (UX-PRO-0707): an NFR that DECLARES itself not-applicable is an exclusion
    // decision, not a promise to protect — it must not trigger the adversarial-audit
    // nudge. The classification (category match + exclusion idiom) is the SSoT in
    // lib/requirements.js, shared with the phase-1/phase-2 gates and the verify nudge
    // (SEC-THREADING-0722). Per-entry guards live there too, so a null/malformed entry
    // in non_functional_requirements no longer nullifies the whole summary (pre-rc.7 a
    // null entry threw inside the filter and the catch suppressed the resume nudge for
    // the entire project — the unsafe direction).
    const nfrs = activeSecurityNfrs(JSON.parse(raw));
    if (nfrs.length === 0) return null;
    return nfrs.map(n => `${n.id} [${n.category}] ${securityNfrText(n)}`).join('\n');
  } catch {
    return null;
  }
}
...
/**
 * Freshness state of the requirements coverage audit for THIS version of the
 * requirements (UPLAN-0703 D5). "Fresh" = an `audit requirements` was invoked for the
 * CURRENT content of 01_REQUIREMENTS.json AND a report file exists.
 *
 * Freshness is keyed to a CONTENT HASH of the requirements, not a timestamp/mtime:
 *   - `AUDIT_REPORT.md` is shared by three audit types (code/requirements/security append
 *     to the same file) — an mtime on it would let a security audit fake requirements-audit
 *     freshness.
 *   - git clone/checkout resets file mtimes — the exact failure the `.aitri` timestamp
 *     exists to escape (ADR-048). A committed content hash survives the clone.
 * So the stamp (`coverageAuditReqHash`) is written at `audit requirements` invocation and
 * compared against the current requirements hash here.
 *
 * Honest ceiling: the stamp is written at INVOCATION. Aitri never verifies the agent
 * actually performed the audit or wrote a meaningful "Requirements Coverage" section — an
 * empty section still reads as fresh. This is placement + visibility, not proof.
 *
 * Returns { hasRequirements, audited, fresh }: `audited` is "ever run for any version"
 * (drives never-run vs stale wording); `fresh` is "run for THIS version".
 */
export function requirementsAuditState(dir, config) {
  const reqRaw = readArtifact(dir, '01_REQUIREMENTS.json', config.artifactsDir || '');
  if (!reqRaw) return { hasRequirements: false, audited: false, fresh: false };
  const currentHash = hashArtifact(reqRaw);
  const stampedHash = config.coverageAuditReqHash || null;
  const audited     = !!config.coverageAuditLastAt || !!stampedHash;
  const reportExists = fs.existsSync(auditReportPath(dir, config));
  const fresh = reportExists && !!stampedHash && stampedHash === currentHash;
  return { hasRequirements: true, audited, fresh };
}
...
function cmdAuditCoverage({ dir, err, featureRoot, scopeName }) {
  ...
  // D5 (UPLAN-0703): stamp a CONTENT HASH of the requirements the audit was invoked
  // against — the freshness signal that survives git clone (mtime resets) and is not
  // faked by the other audit types that share AUDIT_REPORT.md. See requirementsAuditState.
  const reqRawForHash = readArtifact(dir, '01_REQUIREMENTS.json', config.artifactsDir || '');
  if (reqRawForHash) config.coverageAuditReqHash = hashArtifact(reqRawForHash);
  saveConfig(dir, config);
