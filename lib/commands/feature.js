...
export function cmdFeature({ dir, args, err, rootDir }) {
  ...
  switch (sub) {
    case 'run-phase': {
      // Materialize FEATURE_IDEA.md → IDEA.md so phase 1 can find its required input
...
function featureInit(featureDir, name, projectDir, rootDir, err) {
  ...
  console.log(`✅ Feature "${name}" initialized: ${featureDir}`);
  console.log(`\nWhat to do now:`);
  console.log(`  1. Edit: ${ideaPath}`);
  console.log(`     Fill in the feature sections.`);
  console.log(`\nWhen ready:`);
  console.log(`  aitri feature run-phase ${name} 1   Generate feature requirements`);
  console.log(`  aitri feature status ${name}         Check feature pipeline state`);
}
