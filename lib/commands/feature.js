...
const USAGE = `Usage:
  ...
  aitri feature backlog <name> <add|list|show|note|update|done> ...           The feature's own backlog
...
export function cmdFeature({ dir, args, err, rootDir }) {
  ...
  // Charset gate: a name with spaces (or other shell-fragile chars) passes the traversal
  // guard above but breaks every follow-up command Aitri prints — `feature run-phase my feature 1`
  // reparses as name="my", so the success message's own next steps fail. Restrict to a
  // slug-safe charset and suggest the slugified form.
  if (!/^[A-Za-z0-9._-]+$/.test(name)) {
    const slug = name.trim().replace(/\s+/g, '-').replace(/[^A-Za-z0-9._-]/g, '');
    err(
      `Invalid feature name "${name}" — use only letters, digits, dot, dash, underscore (no spaces).` +
      (slug ? `\n  Try: aitri feature ${sub} ${slug}` : '')
    );
  }
...
function featureInit(featureDir, name, projectDir, rootDir, err) {
  ...
  // FEATURE-INDEX-0909: the tree view gains the new feature immediately (with its ordinal).
  try { writeFeaturesIndex(projectDir, buildProjectSnapshot(projectDir)); } catch { /* derived view */ }
