...
import { loadConfig, saveConfig, hashArtifact, artifactPath, appendEvent, clearDriftPhase, hasDrift, cascadeInvalidate, writeLastSession, stampReconcileBaseline, clearCascadePending, writeFrSnapshot, readArtifact, ideaPath as resolveIdeaPath } from '../state.js';
 ...
 */
function archiveIdeaIntoRequirements(dir, artPath, config) {
...
export function cmdApprove({ dir, args, err, featureRoot, scopeName }) {
  ...
  if (phase === 1 && !wasAlreadyApproved) {
    ...
    if (fs.existsSync(ideaPath)) {
      ...
      archivedIdea = archiveIdeaIntoRequirements(dir, artPath, config);
    }
  }
