...
export function buildAgentInterviewBriefing({ questions, target = 'idea', dir, depthNote } = {}) {
  ...
  const lines = [
    ...
    '2. You may draft a first pass of the fields below from their answer and any context.',
    '3. Then go field by field through the REQUIRED FIELDS and CONFIRM each with the user:',
    '   show your draft value and ask "is this right, or correct it?". Ask about blank or',
    '   vague fields explicitly. Confirmation is per field — not one blanket "looks good?".',
    ...
    isIdea
      ? '5. Only after walking the fields, write IDEA.md to: ' + ideaPath
      : '5. Only after walking the fields, produce 00_DISCOVERY.md from these answers as the',
  ];
  if (!isIdea) {
  ...
  lines.push('REQUIRED FIELDS (confirm each with the user — do not silently infer):');
