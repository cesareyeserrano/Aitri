...
import { loadConfig, ideaPath as resolveIdeaPath } from '../state.js';
...
function makeStdinReadLine() {
  return function readLine(prompt) {
    ...
    let chunk;
    try { chunk = readStdinSync(4096); } catch { return ''; }
...
/**
 * Build a structured briefing telling an agent to conduct the interview itself.
 * Used when stdin is not a TTY (agent mode) — Aitri cannot prompt the user, so the
 * agent runs the interview. Shared by the wizard (`target: 'idea'` → writes IDEA.md)
 * and `run-phase discovery --guided` (`target: 'discovery'` → feeds 00_DISCOVERY.md).
 * Exported for testability and reuse — one builder, one field set, two closing actions.
 */
export function buildAgentInterviewBriefing({ questions, target = 'idea', dir, depthNote } = {}) {
  const isIdea = target === 'idea';
  const ideaPath = resolveIdeaPath(dir || '.', loadConfig(dir || '.'));

  const lines = [
    isIdea
      ? '── Aitri Wizard — Agent Mode ────────────────────────────────────────'
      : '── Aitri Discovery (--guided) — Agent Mode ──────────────────────────',
    'stdin is not a TTY. You are running as an agent, so YOU must conduct the',
    'interview with the user — Aitri cannot prompt them directly.',
    '',
    'The seed below is the single highest-value input in the whole pipeline.',
    'Getting it from the user (not from your own inference) is the point of this',
    'command. Do NOT shortcut it into zero questions.',
    '',
    'INSTRUCTIONS:',
    '1. Open with: "Tell me about the project — what problem does it solve and who is it for?"',
    '2. Conduct a CONVERSATION, not a form. From their answer and any provided context,',
    '   present back a short brief: what you understand, what you doubt, and the open',
    '   questions THIS project raises (contradictions, unknowns, risks) — ranked by impact.',
    '   Ask those; iterate as the answers change your understanding. A contradiction is',
    '   presented to the user to resolve — never silently harmonized. Match depth to the',
    '   project: a trivial seed needs one confirmation pass, not manufactured rounds.',
    '3. Before closing, walk the REQUIRED FIELDS below as the COVERAGE CHECKLIST: anything',
    '   the conversation left blank or vague, ask about explicitly now. The fields close',
    '   the conversation — they are not the interview script.',
    '4. A field the user did not actually confirm is an ASSUMPTION — mark it inline as',
    isIdea
      ? '   "[ASSUMPTION] <value>" in IDEA.md so Phase 1 records it as a tracked gap, not a'
      : '   "[ASSUMPTION] <value>" so the discovery records it under Evidence gaps, not a',
    '   silent guess. Never present an inferred value as if the user had stated it.',
    isIdea
      ? '5. Only after the checklist is covered, write IDEA.md to: ' + ideaPath
      : '5. Only after the checklist is covered, produce 00_DISCOVERY.md from these answers as the',
  ];
  if (!isIdea) {
    lines.push('   PRIMARY SOURCE. The discovery briefing is printed below this block — follow it,');
    lines.push('   but synthesize from the answers you gathered; do not invent beyond them.');
  }

  lines.push('');
  if (depthNote) { lines.push(`DEPTH: ${depthNote}`); lines.push(''); }
  lines.push('REQUIRED FIELDS (coverage checklist — close the conversation with these; do not run them as the interview script):');
  ...
  if (isIdea) {
    ...
    lines.push('');
    lines.push('The discovery briefing follows below — produce 00_DISCOVERY.md from it, grounded');
    lines.push('in the answers you gathered above.');
  }
...
/**
 * Convenience: the agent-mode briefing for `run-phase discovery --guided` when stdin
 * is not a TTY. Same QUICK_QUESTIONS field set as the live interview, framed to feed
 * 00_DISCOVERY.md instead of writing IDEA.md.
 */
export function buildDiscoveryInterviewBriefing() {
  return buildAgentInterviewBriefing({ questions: QUICK_QUESTIONS, target: 'discovery' });
}
...
function printAgentBriefing(dir, depth) {
  ...
  console.log(buildAgentInterviewBriefing({ questions, target: 'idea', dir, depthNote: depth }));
}
